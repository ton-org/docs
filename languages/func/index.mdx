...
The method first creates the [message](/foundations/transaction) to send `msg` by using a [cell builder](/tvm/builders-and-slices), since the message is encoded as a [cell](/foundations/cell-serialization/cells). The procedure is:
...
Developers can use the compiled bitcode, structured as a [bag of cells](/foundations/cell-serialization/boc) like all data in the TON blockchain, to test smart contracts, send messages, or execute it in a local TVM instance.
