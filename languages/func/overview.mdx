...
and the `amount` to send. It does not return a value, indicated by the [`()`](/languages/func/types#tensor-types) to the left of the method name `send_money`.
...
`send_money` first creates the [message](/foundations/messages/ordinary-tx) `msg` to send, employing a [cell builder](/tvm/builders-and-slices), since all messages are encoded as [cells](/foundations/serialization/cells). It proceeds as the following:
...
1. [`store_slice`](/languages/func/stdlib#store-slice) stores the receiving address
1. [`store_coins`](/languages/func/stdlib#store-coins) records the amount to send
1. [`end_cell`](/languages/func/stdlib#end-cell) finalizes the builder and turns it into a cell

Finally, the [`send_raw_message`](/languages/func/stdlib#send-raw-message) function sends the message, where the parameter `64` describes a [sending mode](/foundations/messages/internal).

## Compiler

The compiler converts FunC programs into [Fift](/languages/fift) assembly code. The Fift assembly code is then compiled down to the
[TON Virtual Machine](/tvm/overview) bitcode by the [Fift](/languages/fift) compiler.
...
To compile FunC files to bitcode, the `func-js` NPM package is the way to go.
It requires [Node.js](https://nodejs.org/en/download) v22 or later.

The `func-js` package integrates the FunC and [Fift](/languages/fift) compilers
to produce bitcode directly from the FunC source code, without manually invoking Fift.

To install `func-js`, run the following command in the project root folder:
...
<Aside>
  The recommended place for the FunC standard library file is the project root folder as well. Download `smartcont_lib.zip` from the [GitHub repository](https://github.com/ton-blockchain/ton/releases/tag/v2025.07), extract it, and copy `stdlib.fc` to the project root.
</Aside>
...
Prebuilt FunC compiler binaries for Windows, macOS (Intel or Arm64), and Ubuntu are available on the [GitHub](https://github.com/ton-blockchain/ton/releases/tag/v2025.07).
...
Rename the executable, for example, to `func`, for easier use on the command line, and add it to your system’s `PATH`.
...
To compile the generated Fift file  `output.fif` further down to TVM bitcode, use the Fift compiler.
