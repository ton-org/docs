...
Specifying an [atomic type](./types#atomic-types), such as an `int`, `cell`, or `builder`, will make the assembler function capture the top value from the stack.
...
Specifying a [tensor type](./types#tensor-types) as a return type, such as `(int, int)`, will cause the assembler function to take as many elements from the stack as the number of components in the tensor type. If the tensor type has nested tensor types, like `((int, int), int)`, it is interpreted as if it was the flattened tensor type `(int, int, int)`.
...
<Aside>
  When manually rearranging arguments, they are evaluated in the new order. To overwrite this behavior see [`#pragma compute-asm-ltr`](./compiler-directives#%23pragma-compute-asm-ltr).
</Aside>
