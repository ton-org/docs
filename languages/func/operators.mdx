...
<Aside>
  In expressions, all operators should be separated from their arguments, otherwise the expression is interpreted as an [identifier](./literals#identifiers).
...
The `~` operator also behaves as expected on `-1` and `0`, which are the [emulated values](./types#no-boolean-type) for `true` and `false`:
...
The slash-percent sign (division and modulo) operator `/%` returns a two element [tensor](./types#tensor-types), where the first element is the result
...
See more about decomposition notation in the [variable declaration](./expressions#variable-declaration) section.
...
(`?`), then an [expression](./expressions) to execute (the "consequent" expression) if the condition evaluates to an integer different from `0`, followed by a colon (`:`),
...
The assignment operator `=` is used to assign the result of an [expression](./expressions) to a variable. After execution, the `=` operator returns the value assigned to the variable.
...
The assignment operator can also occur in [variable declarations](./expressions#variable-declaration), which are also expressions:
