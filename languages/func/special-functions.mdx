...
FunC, or more specifically, the Fift assembler, reserves several function names with predefined [IDs](./functions#method-id-specifier):
...
The `recv_external` function handles inbound [**external messages**](/foundations/messages/external). It allows declarations similar to those for [`recv_internal`](#receive-internal):
...
The `run_ticktock` triggers at inbound [**tick and tock messages**](/foundations/messages/tick-tock). It allows the following possible declarations:
...
The `split_prepare` triggers for inbound [**split prepare messages**](/foundations/messages/split-prepare). Even though the `split_prepare` name is currently reserved, split prepare messages are currently not in use.

### Split install

The `split_install` triggers for inbound [**split install messages**](/foundations/messages/split-install). Even though the `split_install` name is currently reserved, split install messages are currently unavailable.
