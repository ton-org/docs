...
<Aside>
  ...
  The declarations of `recv_internal` with fewer arguments consume slightly less gas, because each unused value in the stack is automatically dropped from the stack once the TVM finishes execution, without the need to spend gas by explicitly executing a DROP instruction when `recv_internal` finishes.
</Aside>
