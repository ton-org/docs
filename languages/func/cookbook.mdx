...
If a tuple contains various data types `[cell, slice, int, tuple, ...]`, we may need to check the value and cast it accordingly before processing.
...
forall X -> () resolve_type (X value) impure {
    ...
    ;; We need to check what is its type and cast it.
...
() main () {
    ...
    ;; Let us suppose that we do not know beforehand 
    ;; the type of the last element in the tuple.
    ;; So, when extracting the last element, we use the
    ;; keyword var, because we do not know the type
    ;; of the value.
    var value = t~tpop();
...
Since tuples can contain different data types, we must check and cast values dynamically.
...
(int) are_tuples_equal? (tuple t1, tuple t2) {
    ...
    while (i > 0 & equal?) {
        ...
        ;; we need to check if their values 
        ;; are equal or not.
...
You can use the [`cons`](/languages/func/stdlib#cons) function to add an element at the front of the provided list. For example,
...
Given a tree of cells, we can use any [tree traversal algorithm](https://en.wikipedia.org/wiki/Tree_traversal)
...
More concretely, if `s` is the current stack, we append an element `x` by constructing the tuple `[s, x]`.
...
So, we define the function `tpop` as an [assembler function](/languages/func/asm-functions) that wraps the `TPOP` instruction:
...
For example, if we are sure that tuple `t` has only integer elements, we can call the function using [modifying notation](/languages/func/expressions#modifying-notation):
