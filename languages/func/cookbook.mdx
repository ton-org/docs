...
Dictionaries are useful for managing large datasets. 
The built-in methods `dict_get_min?` and `dict_get_max` retrieve the minimum and maximum key values, while `dict_get_next?` allows dictionary iteration.

```func
;; Initialize an example dictionary.
;; keys will be unsigned 256-bit integers.
;; Values are slices 
cell d = new_dict();
...
- [`"<ascii string>"` compile-time builtin](./literals#string-without-suffix), where `<ascii string>` is an ASCII string.
- [`Dictonaries primitives`](./stdlib/#dictionaries-primitives)
- [`dict_get_max?()`](./stdlib/#dict_get_max)
- [`dict_get_min?()`](./stdlib/#dict_get_min)
- [`dict_get_next?()`](./stdlib/#dict_get_next)
- [`dict_set()`](./stdlib/#dict_set)
...
- The `tlen` assembly function is declared [here](./functions#assembler-function-body-definition). You can read more about it and explore a [list of all assembler commands](/tvm/instructions).
