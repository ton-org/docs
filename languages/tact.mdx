...
- a convenient `unboc` disassembler of a contract's code compiled into a [bag of cells](/foundations/cell-serialization/boc) `.boc` format.
...
- BoC Disassembler (`unboc`) — Disassembler for [`.boc`](/foundations/cell-serialization/boc) files. Ships as a standalone binary with the compiler. You can invoke it by running `npx unboc` in your Tact projects.
