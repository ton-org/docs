...
A block is defined using the special words `{` and `}`. All words listed between `{` and `}` constitute the body of a new block, which is then pushed into the stack as a value of type `WordDef`. A block may be stored as a definition of a new Fift word by means of the word `:` as explained in the Words section, or executed by means of the word `execute`:
...
The word `'` recovers the current definition of a word. Namely, the construct `' <word-name>` pushes the execution token equivalent to the current definition of the word `<word-name>`. For instance, the following two lines are equivalent:
...
```fift
// Define word ?. that pushes string "true" if 
