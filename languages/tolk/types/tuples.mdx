...
In [TVM](/tvm/overview), a tuple in the form `[T1, T2, ..., T255]` is a dynamic container that stores from 0 to 255 elements in a [single stack slot](/languages/tolk/types/overall-tvm-stack). Tuples cannot be [serialized to cells](/languages/tolk/types/overall-serialization) and are only present on the stack during the compute phase. Yet they may be returned from get methods because contract getters operate directly on the stack.
