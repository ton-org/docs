...
[`match`](/languages/tolk/syntax/pattern-matching) on enums requires coverage all cases:
...
<Aside
  type="caution"
>
  Operator `as` can produce invalid values, for example `100 as Color`. In this case, operator `==` returns `false`, and an exhaustive `match` throws exception [5](/tvm/exit-codes#5:-integer-out-of-expected-range).
</Aside>
