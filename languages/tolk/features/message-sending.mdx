...
Tolk has a high-level function `createMessage`. In practice, you'll immediately `send` its result:
...
Now, from a minter, you want to send a message to a wallet. But since you are not sure whether the wallet already exists on-chain, you attach its code+data: if a wallet doesn't exist, it's immediately initialized with that code. So, where should you send a message to? What is **destination**? The answer is: **destination is the wallet's `StateInit`**. You need to send a message to a `walletInitialState` because, in TON, the address of a contract is — by definition — a hash of its initial state:
...
In FunC, you had to manually calculate whether it's safe to embed body (you did it _on paper_ or dynamically). In Tolk, you just pass `body`, and the compiler does all calculations for you:
...
Interestingly, whether the body is **small** is determined **AT COMPILE TIME — no runtime checks are needed**. How? Thanks to generics! Here's how `createMessage` is declared:
...
Hence, omitting `body` in `createMessage` defaults `TBody` to `void`.
...
Moreover, following this pattern requires you to give **a name** to a variable. Advice is not to name it "m" or "msg," but to give a descriptive name like "excessesMsg" or "transferMsg":
