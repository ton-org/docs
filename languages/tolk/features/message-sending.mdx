...
1. Integrated with auto-serialization of the `body`
1. Automatically detects, body ref or not
1. Also used for deployment (passing code+data of a contract)
...
it also contains fixed\_prefix\_length (automatically set by the compiler if `toShard`), ticktock info, a library cell; moreover, code and data are nullable.
