...
It is treated as a single string and inserted as-is into Fift output. It may contain `//` comments valid for Fift.
...
Both the input and output sides may be combined: `asm(<INPUT_ORDER> -> <RETURN_ORDER>)`. Reordering is mostly used with `mutate` variables.

## `mutate` and `self` in assembler functions

The `mutate` keyword, described in the [mutability](/languages/tolk/syntax/mutability) section, implicitly returns updated values through the stack for both regular and `asm` functions.
