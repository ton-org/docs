...
The Tolk compiler generates bitecode from clear, idiomatic code. Extracting variables or simple methods should not increase gas consumption.
...
- For Blueprint users, it is an intermediate result that can be accessed in the build directory.
  ...
  To view Fift assembler in Blueprint, run `npm build` or `blueprint build` in the project.
