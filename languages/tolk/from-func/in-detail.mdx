...
In Tolk, spaces are not required. `2+2` evaluates to `4`, and `3+~x` is interpreted as `3 + (~x)`, and so on.
...
Thus, in FunC's standard library, `loadAddress` returns `slice` and `storeAddress` accepts `slice`.

Tolk introduces a dedicated `address` type meaning "internal address". It remains a TVM slice at runtime, but differs from an **abstract slice** in terms of the type system:
...
There is also a type `any_address` to store _internal, external, or none_ address.

**Embedding a const address into a contract**

Use the built-in `address()` function. In FunC, this was done using the postfix `"..."a`, which returned a slice.
...
Remember that `address` is "workchain + hash". Validate untrusted input:
