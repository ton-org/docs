...
| `address`                | addr\_std (internal, 267 bits)           | `STSTDADDR`                         |
| `address?`               | addr\_none or addr\_std (2/267 bits)     | `STOPTSTDADDR`                      |
...
- (3) To pack or unpack a union, say, `Msg1 | Msg2 | Msg3`, we need serialization prefixes. For structures, you can specify them manually
...
1. read uint64 (slice.loadUint 64)
1. read three refs (slice.loadRef); do not unpack them: we just have pointers to cells
...
1. or repack your data by splitting into multiple cells
...
Continue reading here: [Universal createMessage](/languages/tolk/from-func/create-message).
...
Continue reading here: [lazy loading, partial loading](/languages/tolk/from-func/lazy-loading).
