...
Here we can see that the contract tried to load a 64‑bit integer (`LDU 64`) from the slice, but there was not enough data, so [exit code 9](/tvm/exit-codes#9%3A-cell-underflow) occurred.
...
Here, the stack contains of two values:
