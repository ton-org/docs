...
Availability depends on the language you use. In Tolk, use functions from the globally available `debug` object. In FunC, these methods are available globally in `stdlib.fc`. In Tact, use `dumpStack` for `DUMPSTK` and [dump function](https://docs.tact-lang.org/ref/core-debug/#dump) for the other two. Tact also prints the exact line where `dump` is called, so you can quickly find it in your code.
