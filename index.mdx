---
title: "Welcome to TON Blockchain documentation - TON Docs"
description: "Meet the next generation of TON documentation. AI-native, beautiful out-of-the-box, and built for developers."
mode: "custom"
---

export const Journey = ({ title, description, children }) => {
  return (
    <div className="card block font-normal group mt-4 px-8 pt-8 pb-6 ring-2 ring-transparent rounded-2xl bg-white dark:bg-background-dark border border-gray-950/10 dark:border-white/10 w-full">
      <h3 className="mb-2 text-balance text-xl sm:text-3xl font-semibold tracking-tight text-gray-900 dark:text-gray-200">
        {title}
      </h3>
      <p className="text-pretty text-lg prose prose-gray dark:prose-invert">
        {description}
      </p>
      <div className="mt-4 overflow-x-auto">
        <div className="flex flex-row gap-4 py-2">
          {children}
        </div>
      </div>
    </div>
  );
};

export const JourneyStep = ({ title, icon, iconType, href, cta, children }) => {
  return (
    <div className="flex-none w-64 max-h-64">
      <a className="card block font-normal group ring-2 ring-transparent rounded-2xl bg-white dark:bg-background-dark border border-gray-950/10 dark:border-white/10 overflow-hidden w-full h-full cursor-pointer hover:!border-primary dark:hover:!border-primary-light" {...(href && { href })}>
        <div className="px-6 py-5 h-full flex flex-col" data-component-part="card-content-container">
          {icon && (
            <div className="h-6 w-6 mb-4 fill-gray-800 dark:fill-gray-100 text-gray-800 dark:text-gray-100 shrink-0" data-component-part="card-icon">
              {/* @ts-ignore */}
              <Icon icon={icon} iconType={iconType} color="#0098EA" size={16} />
            </div>
          )}
          <div className="flex-1 flex flex-col min-h-0">
            <h2 className="not-prose font-semibold text-base text-gray-800 dark:text-white shrink-0" contentEditable="false" data-component-part="card-title">{title}</h2>
            <div className="prose mt-1 font-normal text-sm leading-6 text-gray-600 dark:text-gray-400 flex-1 overflow-hidden" data-component-part="card-content"><span data-as="p">{children}</span></div>
            {cta && (
              <div className="mt-8 shrink-0" data-component-part="card-cta"><button className="text-left text-gray-600 gap-2 dark:text-gray-400 text-sm font-medium flex flex-row items-center hover:text-primary dark:hover:text-primary-light">{cta}<svg width="3" height="24" viewBox="0 -9 3 24" className="rotate-0 overflow-visible h-6"><path d="M0 0L3 3L0 6" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"></path></svg></button></div>
            )}
          </div>
        </div>
      </a>
    </div>
  );
}

{/* "primary": "#0098EA", "light": "#F7F9FB", "dark": "#1E2337" */}

<div className="relative w-full box-border flex flex-col mx-auto px-6 max-w-xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl">
  {/* hero */}
  <div className="relative isolate">
    {/* color splash */}
    <div aria-hidden="true" className="absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-60">
      <div
        style={{
          clipPath:
            'polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)',
        }}
        className="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#2d83ec] to-[#1ac9ff] opacity-30 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem]"
      />
    </div>
    {/* two parts */}
    <div className="py-16 sm:py-24 lg:py-32 flex flex-col lg:flex-row lg:items-center lg:gap-16 xl:gap-24">
      {/* left */}
      <div className="w-full lg:flex-[2] mx-auto lg:mx-0 lg:max-w-2xl">
        <div className="hidden sm:mb-8 sm:flex">
          <div className="relative rounded-full px-3 py-1 text-sm/6 text-gray-700 dark:text-gray-400 ring-1 ring-gray-400/30 dark:ring-gray-600/30 hover:ring-gray-600/30 dark:hover:ring-gray-500/30 dark:hover:brightness-[1.25]">
            Announcing public beta release!&nbsp;
            <a href="TODO: link to TON Dev channel" className="font-semibold text-primary dark:text-primary-light">
              <span aria-hidden="true" className="absolute inset-0" />
              Read more <span aria-hidden="true">&rarr;</span>
            </a>
          </div>
        </div>
        <div className="text-left">
          <h1 className="text-balance text-5xl sm:text-7xl font-semibold tracking-tight text-gray-900 dark:text-gray-200">
            Welcome to<br />TON&nbsp;Blockchain<br />documentation
          </h1>
          <p className="mt-8 max-w-xl text-pretty text-2xl prose prose-gray dark:prose-invert">
            Meet the AI-native, beautiful and judicious documentation made for developers, builders, integrators, and <b>you</b>.
          </p>
          <div className="mt-8 flex items-center gap-x-6">
            <a
              href="#navigation"
              className="rounded-md bg-primary hover:bg-primary/60 px-3.5 py-2.5 text-lg font-semibold text-white shadow-sm"
            >
              I know where to go
            </a>
            <a href="#journeys" className="font-semibold text-lg prose-gray dark:prose-invert">
              Show me the way <span aria-hidden="true">→</span>
            </a>
          </div>
        </div>
      </div>
      {/* right part */}
      <div className="hidden lg:flex w-full">
        <img
          src="/resources/logo/ton.svg"
          alt="TON logo"
          className="w-full"
          noZoom="true"
        />
      </div>
    </div>
    {/* color splash */}
    <div
      aria-hidden="true"
      className="absolute inset-x-0 top-[calc(100%-13rem)] -z-10 transform-gpu overflow-hidden blur-3xl sm:top-[calc(100%-40rem)]"
    >
      <div
        style={{
          clipPath:
            'polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)',
        }}
        className="relative left-[calc(50%+3rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 bg-gradient-to-tr from-[#1ac9ff] to-[#2d83ec] opacity-30 sm:left-[calc(50%+24rem)] sm:w-[72.1875rem]"
      />
    </div>
  </div>

  {/* sections & direct links */}
  <section id="navigation" tabIndex={-1} className="relative mt-16 py-12 sm:py-24 flex flex-col gap-8">
    <div className="text-left">
      <h2 className="text-balance text-3xl sm:text-5xl font-semibold tracking-tight text-gray-900 dark:text-gray-200">
        Quick section navigation
      </h2>
      <p className="mt-4 text-pretty text-xl prose prose-gray dark:prose-invert">
        Jump to the desired part of the documentation right away. If you're not sure, <a href="#journeys" className="font-normal text-xl">proceed below</a>.
      </p>
    </div>
    {/* @ts-ignore */}
    <Columns cols={4}>
      {/* @ts-ignore */}
      <Card
        title="Step-by-step guides"
        icon="shoe-prints"
        iconType="solid"
        href="/guidebook/first-dapp"
        cta="Your first dApp"
      />
      {/* @ts-ignore */}
      <Card
        title="Ecosystem"
        icon="leaf"
        iconType="solid"
        href="/ecosystem/overview"
        cta="Overview"
      />
      {/* @ts-ignore */}
      <Card
        title="Standard contracts"
        icon="copy"
        iconType="solid"
        href="/standard/overview"
        cta="Overview"
      />
      {/* @ts-ignore */}
      <Card
        title="Contract patterns"
        icon="file-signature"
        iconType="solid"
        href="/techniques/carry-value"
        cta="Carry-value pattern"
      />
      {/* @ts-ignore */}
      <Card
        title="Languages"
        icon="code"
        iconType="solid"
        href="/language/tolk"
        cta="Tolk"
      />
      {/* @ts-ignore */}
      <Card
        title="TON Virtual Machine"
        icon="binary"
        iconType="solid"
        href="/tvm/overview"
        cta="Overview"
      />
      {/* @ts-ignore */}
      <Card
        title="Blockchain foundations"
        icon="house"
        iconType="solid"
        href="/ton/overview"
        cta="Overview"
      />
      {/* @ts-ignore */}
      <Card
        title="Web3 services"
        icon="chart-network"
        iconType="solid"
        href="/services/overview"
        cta="Overview"
      />
      {/* @ts-ignore */}
    </Columns>
  </section>

  {/* journeys */}
  <section id="journeys" tabIndex={-1} className="relative mt-16 py-12 sm:py-24 flex flex-col gap-8">
    <div className="text-left">
      <h2 className="text-balance text-3xl sm:text-5xl font-semibold tracking-tight text-gray-900 dark:text-gray-200">
        Choose your path
      </h2>
      <p className="mt-4 text-pretty text-xl prose prose-gray dark:prose-invert">
        Embark on a journey tailored to your role, experience, and intent.
        {/* <br />To read the documentation top to bottom instead, <a href="/start-here" className="font-normal text-xl">start here</a>. */}
      </p>
    </div>
    <Journey title="Freshman" description="Learn the fundamentals of TON Blockchain. Perfectly suited for newcomers who explore the web3 world through TON.">
      <JourneyStep
        title="How to read this documentation?"
        icon="1"
        iconType="solid"
        href="/start-here"
        cta="Start here"
      >
      </JourneyStep>
      <JourneyStep
        title="What is blockchain?"
        icon="2"
        iconType="solid"
        href="/ton/glossary#blockchain"
        cta="Continue"
      >
        {/* Understand the fundamentals of blockchain technology and decentralization. */}
      </JourneyStep>
      <JourneyStep
        title="What is a smart contract on TON?"
        icon="3"
        iconType="solid"
        href="/ton/glossary#smart-contract"
        cta="Continue"
      >
      </JourneyStep>
      <JourneyStep
        title="Writing your first smart contract"
        icon="4"
        iconType="solid"
        href="/guidebook/first-smart-contract"
        cta="Continue"
      >
      </JourneyStep>
      <JourneyStep
        title="What is a dApp?"
        icon="5"
        iconType="solid"
        href="/ton/glossary#decentralized-application-dapp"
        cta="Continue"
      >
      </JourneyStep>
      <JourneyStep
        title="Your first dApp on TON"
        icon="6"
        iconType="solid"
        href="/guidebook/first-dapp"
        cta="Continue"
      >
      </JourneyStep>
    </Journey>
    <Journey
      title="Nomad"
      description="For those coming from another blockchain. Study how things work on TON in comparison and go through hands-on guides for migration."
    >
      <JourneyStep
        title="Coming from Ethereum"
        icon="ethereum"
        iconType="solid"
        href="/guidebook/from-ethereum"
      >
        Compare the execution model and the ecosystem of TON
      </JourneyStep>
      <JourneyStep
        title="Your first TON dApp"
        icon="mobile-notch"
        iconType="solid"
        href="/guidebook/from-ethereum"
      >
        From nothing to deployed and working contract with a web frontend
      </JourneyStep>
    </Journey>
    <Journey
      title="Ace"
      description="For amateurs, experts, and learners alike. Take a deep dive into the low-level details, or do a quick reference check and return to your project."
    >
      <JourneyStep
        title="Blueprint"
        href="/ecosystem/blueprint/overview"
      >
        Development environment
      </JourneyStep>
      <JourneyStep
        title="TVM Exit codes"
        href="/tvm/exit-codes"
      >
        Complete list of compute phase exit codes
      </JourneyStep>
      <JourneyStep
        title="TVM Instructions"
        href="/tvm/instructions"
      >
        Complete list of instructions and opcodes
      </JourneyStep>
      <JourneyStep
        title="RPC providers"
        href="/ecosystem/rpc/overview"
      >
        TON Center v2 and v3, and others
      </JourneyStep>
      <JourneyStep
        title="TON Connect"
        href="/ecosystem/ton-connect"
      >
        Integrate TON into dApps and wallets
      </JourneyStep>
      <JourneyStep
        title="Blockchain nodes"
        href="/ecosystem/node/overview"
      >
        Pick the right kind and setup your node
      </JourneyStep>
    </Journey>
    <div className="text-center">
      <p className="mt-4 text-pretty text-lg prose prose-gray dark:prose-invert">
        Looking for something else? Search documentation <kbd>Ctrl + I</kbd> and ask AI right there,
        or open the assistant panel by pressing <kbd>Ctrl + I</kbd>. If nothing was found, <a href="#community" className="font-normal text-lg">ask the community</a> of TON developers, builders and enthusiasts.
      </p>
    </div>
  </section>

  {/* community */}
  <section id="community" tabIndex={-1} className="relative mt-16 py-12 sm:py-24 flex flex-col gap-8">
    <div className="text-left">
      <h2 className="text-balance text-3xl sm:text-5xl font-semibold tracking-tight text-gray-900 dark:text-gray-200">
        Join the community
      </h2>
      <p className="mt-4 text-pretty text-xl prose prose-gray dark:prose-invert">
        Troubleshoot issues or discuss best practices with other community members.
      </p>
    </div>
    {/* @ts-ignore */}
    <Columns cols={4}>
      {/* @ts-ignore */}
      <Card
        title="Get support"
        icon="life-ring"
        iconType="solid"
        href="/get-support"
      >
        Learn how to get help on the dedicated page
        {/* @ts-ignore */}
      </Card>
      <Card
        title="Contribute"
        icon="handshake"
        iconType="solid"
        href="/contribute"
      >
        Create educational materials and enrich the ecosystem
        {/* @ts-ignore */}
      </Card>
      {/* @ts-ignore */}
      <Card
        title="Telegram"
        icon="telegram"
        iconType="solid"
        arrow="true"
        href="https://t.me/addlist/1r5Vcb8eljk5Yzcy"
      >
        Add the folder with many developer chats
        {/* @ts-ignore */}
      </Card>
      {/* @ts-ignore */}
      <Card
        title="TON Talents"
        icon="user-group"
        iconType="solid"
        arrow="true"
        href="https://ton.org/en/talents"
      >
        Seek skilled professionals and agencies
        {/* @ts-ignore */}
      </Card>
      {/* @ts-ignore */}
    </Columns>
  </section>
</div>
