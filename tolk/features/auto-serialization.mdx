...
Tolk allows overriding serialization behavior when the required encoding cannot be represented using existing types.
Custom serializers can be defined for type aliases, structures, enums, and generic types:
...
The same works for structures: `fun SomeStruct.packToBuilder` and `fun SomeStruct.unpackFromSlice`.
Even if a struct/alias is generic: `fun GenericType<T>.packToBuilder`.
