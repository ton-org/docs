...
Tolk provides a dedicated [`string` type](/languages/tolk/types/strings) backed by snaked cells. Use [`StringBuilder`](/languages/tolk/types/strings#stringbuilder) for concatenation:
...
For fixed-size binary data, use `bitsN` or `bytesN` types.
