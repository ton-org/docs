...
The `import` statement can accept not only relative paths, but also `@aliases`:
...
This is similar to path mappings in TypeScript (`paths` in `tsconfig.json`).

In Tolk, mappings are specified at compiler invocation.
...
When using Blueprint — in `ContractName.compile.ts`:
...
Unlike `paths` in TypeScript, Tolk does not use file masks: each alias is specified independently. Aliases can point to absolute or relative directories:
