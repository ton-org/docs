...
- `@custom(<anything>)` or `@custom.xxx` or `@custom.xxx(<anything>)` is a custom annotation (arbitrary name after a dot) that is not analyzed by the compiler.
- `@test(<anything>)` or `@test.xxx` or `@test.xxx(<anything>)` is similar to `@custom`, just skipped.
