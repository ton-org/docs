...
- Serialized as a reference (a string is a snaked cell)
- TL-B: `^Cell`
- Stored using `STREF`
- Loaded using `LDREF`
...
- Serializable when `T` is serializable (e.g., `array<int8>`, `array<bool?>`).
- Binary format: `uint8` length + chained cell references (snake refs), each containing a `bool` flag for the next reference and serialized elements.

For example, `array<uint8> [1, 2, 3, 4, 5]` can be packed into a single chunk (all 5 elements in a ref), resulting in binary:

- `uint8` = 5 (len)
- `bool` true (means "has ref")
- ref `cell`
  - `bool` false (means "no next ref")
  - `0x0102030405`

It's not the only option. It may be packed as "ref 1 ref 2345", or "ref 12 + ref 34 + ref "5", and so on. For example, "ref 123 + ref 45" will be:

- `uint8` = 5 (len)
- `bool` true (means "has ref")
- ref `cell`
  - `bool` true (means "has next ref")
  - `0x010203`
  - ref `cell`
    - `bool` false (means "no next ref")
    - `0x0405`

An empty array is `uint8` 0 + `bool` false — nine zero bits.

The compiler pre-calculates chunk size to fit as many elements into a cell as possible.
But client-side libraries can follow a more simple strategy — for example, write into a cell until there is space, then create a nested ref, etc.
...
- Cannot be serialized. Prefer using `array<T>` with serializable `T` instead of `array<unknown>` (aka `tuple`).
