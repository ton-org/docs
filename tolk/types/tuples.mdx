...
>
  ...
  For example, <code>{'array<' + 'int>'}</code> is a single stack entry containing multiple integers. In contrast, a tensor `(int, int, int)` signifies three separate integers that occupy individual stack entries or are serialized sequentially.
</Aside>
...
Specify a type manually to force `T`, especially for initially empty arrays:
...
The syntax `array<int> [...]` is similar to creating an object with `Point {...}`, where a type hint may be omitted if clear from context.
...
Every array has clearly named methods; an IDE suggests them after a dot:
...
Additional methods such as `set()` and others are [available in the standard library](/languages/tolk/features/standard-library#arrays).
...
An array can contain from 0 to 255 elements. It occupies one stack slot regardless of its size.
Accessing the first 16 elements consumes less gas than greater indices, because for `i >= 16` an additional instruction is required.
...
Lisp lists allow accessing only the front element (head). There is no `get(i)` or cheap `size()`:
...
Several helper methods are available in the standard library, such as `calculateSize` and `calculateConcatenation`. These have O(N) complexity: the longer the list, the higher the gas consumption.
