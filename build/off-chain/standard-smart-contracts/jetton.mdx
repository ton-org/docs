...
> THIS PAGE IN TON DOCS: https://docs.ton.org/v3/guidelines/dapps/asset-processing/jettons

## Best practices on Jetton processing
...
[Highload wallet v3](/documentation/smart-contracts/contracts-specs/highload-wallet#highload-wallet-v3): v3 is the latest solution on TON Blockchain and the gold standard for jetton withdrawals. It enables batched withdrawals.
...
<Info>
TON transactions are irreversible after just one confirmation. To clearly understand this process, readers should be familiar with the basic principles of asset processing described in [this section of our documentation](/documentation/dapps/assets/overview). It is particularly important to understand [contracts](/documentation/smart-contracts/addresses#everything-is-a-smart-contract), [wallets](/guidelines/smart-contracts/howto/wallet), [messages](/documentation/smart-contracts/message-management/messages-and-transactions), and the deployment process.
</Info>
...
If they are non-mintable, no additional tokens can be minted. To mint jettons for the first time, refer to the [mint your first jetton](/guidelines/dapps/tutorials/mint-your-first-token) page.
...
<Accordion title="Retrieve wallet address with TON Center API" icon="fa-info-circle" iconColor="blue">
  Run `get_wallet_address(slice owner_address)` through `/runGetMethod` method from the [TON Center API](https://toncenter.com/api/v3/#/default/run_get_method_api_v3_runGetMethod_post). In real cases (not test ones) it is important to always check that wallet indeed is attributed to desired jetton Master. Check the JavaScript code example below for more.
</Accordion>
...
<Tip>
For more examples, read the [TON cookbook](/guidelines/dapps/cookbook#tep-74-jettons-standard).
</Tip>
...
<Accordion title="Retrieve Jetton wallet data via TON Center API directly" icon="fa-info-circle" iconColor="blue">
...
<Accordion title="Retrieve Jetton wallet data via TON Center API through TonWeb" icon="fa-info-circle" iconColor="blue">
...
<Tip>
Read more about messages [here](/documentation/smart-contracts/message-management/messages-and-transactions).
</Tip>
...
You may also encounter the error [`cskip_no_gas`](/documentation/tvm/tvm-overview#when-the-compute-phase-is-skipped), which indicates that the tokens were successfully transferred, but no other computations were performed. This is a common situation when the `forward_ton_amount` value is 1 nanoton.

<Tip>
Check [best practices](/guidelines/dapps/asset-processing/jettons#best-practices) for the _"send jettons with comments"_ example.
</Tip>
...
1. Find the correct [smart contract address](/guidelines/dapps/asset-processing/jettons#jetton-master-smart-contract).
2. Get [metadata](/guidelines/dapps/asset-processing/jettons#retrieving-jetton-data).
3. Check for [scams](/guidelines/dapps/asset-processing/jettons#jetton-master-smart-contract).
...
This should not be confused with the `sender` field in the [body](/guidelines/dapps/asset-processing/jettons#message-2) of the `Transfer Notification`.

1. Get the jetton master address for the new jetton wallet by \[retrieving the wallet data] (/guidelines/dapps/asset-processing/jettons#retrieving-data-for-a-specific-jetton-wallet).
...
1. [Prepare a list of accepted jettons](/guidelines/dapps/asset-processing/jettons#adding-new-jettons-for-asset-processing-and-initial-verification) (jetton master addresses).
2. Deploy hot wallet (using v3R2 if no jetton withdrawals are expected; highload v3 - if jetton withdrawals are expected). [Wallet deployment](/guidelines/dapps/asset-processing/payments-processing#wallet-deployment).
...
3. Retrieve the jetton master address for each jetton wallet using [retrieving wallet data](/guidelines/dapps/asset-processing/jettons#retrieving-data-for-a-specific-jetton-wallet).
...
5. Retrieve the list of recent unprocessed transactions using the hot wallet account and repeat (sorting each transaction one at a time). See: [Check contract transactions](/guidelines/dapps/asset-processing/payments-processing#check-contracts-transactions).
...
9. Read other message body data, including `query_id`, `amount`, `sender`, `forward_payload`. [jetton contract message layouts](/guidelines/dapps/asset-processing/jettons#message-layouts), [Tonweb example](https://github.com/toncenter/examples/blob/9f20f7104411771793dfbbdf07f0ca4860f12de2/deposits-jettons-single-wallet.js#L105).
...
2. Deploy a hot wallet (using v3R2 if no jetton withdrawals are expected; highload v3 - if jetton withdrawals are expected). [Wallet deployment](/guidelines/dapps/asset-processing/payments-processing#wallet-deployment).

#### Creating deposits

1. Accept a request to create a new deposit for the user.
2. Generate a new subwallet address (/unprocessedR2) based on the hot wallet seed. [Creating a subwallet in Tonweb](#creating-a-subwallet-in-tonweb)
...
<Aside>
**It is recommended that you** read and **understand** the [how jetton transfer works](/guidelines/dapps/asset-processing/jettons#overview) and [how to send jettons with a comment](/guidelines/dapps/asset-processing/jettons#jetton-off-chain-processing) articles before reading this section.
...
See: [Jetton contract message layouts](/guidelines/dapps/asset-processing/jettons#message-layouts).
...
2. Initiate hot wallet deployment. Highload v3 is recommended. [Deploy wallet](/guidelines/dapps/asset-processing/payments-processing#wallet-deployment)
...
7. Create a [message](/guidelines/dapps/asset-processing/jettons#message-0).
...
11. Get a list of the latest unprocessed transactions in the hot wallet account and retry it.
    Learn more here: [Check contract transactions](/guidelines/dapps/asset-processing/payments-processing#check-contracts-transactions),
...
For ready-to-test examples, check [SDKs](/guidelines/dapps/asset-processing/jettons#sdks)  and run them. The following code snippets help understand jetton processing with  practical examples.
...
You can find a list of SDKs for different languages ​​(JS, Python, Golang, C#, Rust, etc.) list [here](/guidelines/dapps/apis-sdks/sdk).

## See also

- [Payments processing](/guidelines/dapps/asset-processing/payments-processing)
- [NFT processing on TON](/guidelines/dapps/asset-processing/nft-processing/nfts)
- [Metadata parsing on TON](/guidelines/dapps/asset-processing/nft-processing/metadata-parsing)
