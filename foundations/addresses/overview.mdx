...
For the so-called **Intermediate** and **DNS** addresses, see the [Hypercube Routing](/foundations/shards) and [DNS: .ton domains](/services/dns) pages, respectively.
...
In the currently used workchains, an account id is defined as the result of applying a hash function to a [`StateInit`](/foundations/messages/state-init) structure that stores initial code and data of a smart contract.
