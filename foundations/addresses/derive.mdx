...
Even though the general algorithm is practically the same for all cases, the details can still be non-trivial. Once StateInit is available, composing the address is straightforward; the task is to compose StateInit.

Understand the difference between StateInit and the current state of the account. StateInit is the initial state, which an account stores at deployment. After that, state can be changed by the smart contract itself — for example, by updating the data. Since the address depends on StateInit, to derive the address it is necessary to compose the initial state rather than the current one.
