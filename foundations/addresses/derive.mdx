...
Even though the general algorithm is practically the same for all cases, the details can still be non-trivial. The non‑trivial step is composing the `StateInit`; building the address from it is straightforward.
