...
The Reliable Large Datagram Protocol (RLDP) operates on top of [ADNL UDP](/foundations/network/adnl-udp) and is designed for transferring large data blocks. It uses Forward Error Correction (FEC) instead of acknowledgment packets, enabling more efficient data transfer at the cost of increased traffic.
...
All structures are serialized, wrapped in `adnl.message.custom`, and transmitted over [ADNL UDP](/foundations/network/adnl-udp).
...
The decoded result is `rldp.answer` containing `http.response`. If `no_payload` is false, retrieve the body by sending `http.getNextPayloadPart` requests with incrementing `seqno` and `max_chunk_size` of 128 KB (131072 bytes) until `last` is true.
