...
- If `no_payload` is `false`, the body must be fetched separately: send `http.getNextPayloadPart` requests with incrementing `seqno` and `max_chunk_size` of 128 KiB (131,072 bytes), each wrapped in a new `rldp.query` transfer, until the returned `http.payloadPart` has `last` set to `true`.
