...
The Reliable Large Datagram Protocol (RLDP) operated on top of [ADNL UDP](https://old-docs.ton.org/v3/documentation/network/protocols/adnl/udp) and was designed for transferring large data blocks. It used Forward Error Correction (FEC) instead of acknowledgment packets, enabling more efficient data transfer at the cost of increased traffic.

<Aside type="note">
  With the introduction of the [sub-second upgrade](/ecosystem/sub-second), blockchain nodes now use [QUIC](https://en.wikipedia.org/wiki/QUIC) instead of RLDP for internal communication.
</Aside>
...
All structures were serialized, wrapped in `adnl.message.custom`, and transmitted over [ADNL UDP](https://old-docs.ton.org/v3/documentation/network/protocols/adnl/udp).
...
This allowed the receiver to recover lost packets without requesting retransmission — fewer packets were needed than if raw data segments were simply resent.
