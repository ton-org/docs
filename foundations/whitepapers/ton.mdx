...
- $\text{STORE}(\text{key},\text{value})$ — Asks the node to keep *value* as a value for key *key*. For TON DHT, the $\text{STORE}$ queries are slightly more complicated (cf. [3.2.10](#3-2-10-storing-values-in-ton-dht)).
