...
- A distributed file storage technology (*TON Storage*; cf. [4.1.6](#4-1-6-example%3A-keeping-files-off-chain%3B-ton-storage)), accessible through *TON Network*, used by the TON Blockchain to store archive copies of blocks and status data (snapshots), but also available for storing arbitrary files for users or other services running on the platform, with torrent-like access technology.
...
- A platform for arbitrary services (*TON Services*; cf. [Chapter 4](#4-ton-services-and-applications)), residing in and available through *TON Network* and *TON Proxy*, with formalized interfaces (cf. [4.3.14](#4-3-14-public-interfaces-of-a-smart-contract)) enabling browser-like or smartphone application interaction. These formal interfaces and persistent service entry points can be published in the TON Blockchain (cf. [4.3.17](#4-3-17-locating-user-interfaces-via-ton-dns)); actual nodes providing service at any given moment can be looked up through the *TON DHT* starting from information published in the TON Blockchain (cf. [3.2.12](#3-2-12-locating-services)). Services may create smart contracts in the TON Blockchain to offer some guarantees to their clients (cf. [4.1.5](#4-1-5-mixed-services%3A-partly-off-chain%2C-partly-on-chain)).
...
$$
\text{evtrans}' : \text{Transaction} \times \text{State} \to \text{State}^? \tag{3}
$$
...
We might prefer a curried version of $\text{evtrans}'$:

$$
\text{evtrans} : \text{Transaction} \to \text{State} \to \text{State}^? \tag{4}
$$

Because a block is essentially a list of transactions, the block evaluation function

$$
\text{evblock} : \text{Block} \to \text{State} \to \text{State}^? \tag{5}
$$

can be derived from $\text{evtrans}$. It takes a block $B : \text{Block}$ and the previous blockchain state $s : \text{State}$ (which might include the hash of the previous block) and computes the next blockchain state $s' = \text{evblock}(B)(s) : \text{State}$, which is either a true state or a special value $\bot$ indicating that the next state cannot be computed (i.e., that the block is invalid if evaluated from the starting state given—for example, the block includes a transaction trying to debit an empty account.)

#### 2.2.7. Block sequence numbers

Each block $B$ in the blockchain can be referred to by its *sequence number* $\text{blkseqno}(B)$, starting from zero for the very first block, and incremented by one whenever passing to the next block. More formally,

$$
\text{blkseqno}(B) = \text{blkseqno}\bigl(\text{blkprev}(B)\bigr) + 1 \tag{6}
$$
...
Final remark: in order to make the probability statement of (8) really rigorous, one must introduce a probability distribution on the set $\text{Bytes}^*$ of all byte sequences. A way of doing this is by assuming all byte sequences of the same length $l$ equiprobable, and setting the probability of observing a sequence of length $l$ equal to $p^l - p^{l+1}$ for some $p \to 1^-$. Then (8) should be understood as a limit of conditional probability $P\bigl(\text{Hash}(s) = \text{Hash}(s') | s \neq s'\bigr)$ when $p$ tends to one from below.
...
$$
\text{accountid} : \text{Account} = \text{uint}_{256} = \mathbf{2}^{256} \tag{9}
$$

Here $\text{Account}$ is the account *type*, while $\text{accountid} : \text{Account}$ is a specific variable of type $\text{Account}$.
...
However, the bit length $l$ of an account ID must be fixed during the creation of the workchain (in the masterchain), and it must be at least 64, because the first 64 bits of $\text{accountid}$ are used for sharding and message routing.
...
$$
\text{balance} : \text{Account} \to \text{uint}_{128} \tag{10}
$$
...
$$
\text{storage} : \mathbf{2}^{256} \dashrightarrow \mathbf{2}^{256} \tag{11}
$$
...
Because we have more than one smart contract, distinguished by $\text{accountid}$, each having its separate persistent storage, we must actually have a hashmap

$$
\text{Storage} : \text{Account} \dashrightarrow (\mathbf{2}^{256} \dashrightarrow \mathbf{2}^{256}) \tag{12}
$$

mapping $\text{accountid}$ of a smart contract into its persistent storage.
...
$$
\text{Hashmap}(n,X) : \text{Type} \tag{13}
$$

corresponding to a data structure encoding a (partial) map $\mathbf{2}^n \dashrightarrow X$. We can also write

$$
\text{Hashmap}(n : \mathbb{N})(X : \text{Type}) : \text{Type} \tag{14}
$$

or

$$
\text{Hashmap} : \mathbb{N} \to \text{Type} \to \text{Type} \tag{15}
$$
...
$$
h[i] \equiv \text{hget}(h)(i) : X^? \quad \text{for any } i : \mathbf{2}^n, h : \text{Hashmap}(n,X) \tag{16}
$$
...
$$
\text{height}(\text{leaf}(x)) = 0 \tag{17}
$$

$$
\text{height}\bigl(\text{node}(l,s_l,r,s_r)\bigr) = \text{height}(l) + \text{len}(s_l) = \text{height}(r) + \text{len}(s_r) \tag{18}
$$

$$
\text{height}\bigl(\text{root}(n,s_0,t)\bigr) = \text{len}(s_0) + \text{height}(t) = n \tag{19}
$$
...
$$
\text{Hash}\bigl(\text{node}(l,s_l,r,s_r)\bigr) := \text{Hash}\bigl(\text{Hash}(l).\text{Hash}(r).\text{code}(s_l).\text{code}(s_r)\bigr) \tag{21}
$$

$$
\text{Hash}\bigl(\text{root}(n,s_0,t)\bigr) := \text{Hash}\bigl(\text{code}(n).\text{code}(s_0).\text{Hash}(t)\bigr) \tag{22}
$$
...
$$
\text{ShardchainState} := (\text{Account} \dashrightarrow \text{AccountState}) \tag{21}
$$
...
Note that by signing a block $B$ the validator asserts that the block is valid given the original state $s$ (i.e., that the result of (23) is not the value $\bot$ indicating that the next state cannot be computed). In this way, the validator must perform minimal formal checks of the cells of the original state that are accessed during the evaluation of the (23).
...
$$
d_K(x,y) := (x \oplus y) \quad \text{interpreted as an unsigned 256-bit integer} \tag{24}
$$
