...
When the length of a bitstring is a multiple of four, we subdivide it into groups of four bits and represent each group by one of sixteen hexadecimal digits `0`-`9`, `A`-`F` in the usual manner: $\texttt{0}_{16}\leftrightarrow\texttt{0000}$, $\texttt{1}_{16}\leftrightarrow\texttt{0001}$, $\ldots$, $\texttt{F}_{16}\leftrightarrow\texttt{1111}$. The resulting hexadecimal string is our equivalent representation for the original binary string.
...
If a function or primitive requires $m$ arguments $x_1$, $\ldots$, $x_m$, they are pushed by the caller into the stack in the same order, starting from $x_1$. Therefore, when the function or primitive is invoked, its first argument $x_1$ is in $\texttt{s}(m-1)$, its second argument $x_2$ is in $\texttt{s}(m-2)$, and so on. The last argument $x_m$ is in `s0` (i.e., at the top of the stack). It is the called function or primitive's responsibility to remove its arguments from the stack.
...
For example, the "divide with remainder" primitive $\texttt{DIVMOD}$ needs to return two values, the quotient $q$ and the remainder $r$. Therefore, $\texttt{DIVMOD}$ pushes $q$ and $r$ into the stack, in that order, so that the quotient is available thereafter at `s1` and the remainder at `s0`. The net effect of $\texttt{DIVMOD}$ is to divide the original value of `s1` by the original value of `s0`, and return the quotient in `s1` and the remainder in `s0`. In this particular case the depth of the stack and the values of all other "stack registers" remain unchanged, because $\texttt{DIVMOD}$ takes two arguments and returns two results. In general, the values of other "stack registers" that lie in the stack below the arguments passed and the values returned are shifted according to the change of the depth of the stack.

In principle, some primitives and user-defined functions might return a variable number of result values. In this respect, the remarks above about variadic functions (cf. [function argument order section]) apply: the total number of result values and their types should be determined by the values near the top of the stack. (For example, one might push the return values $y_1$, $\ldots$, $y_k$, and then push their total number $k$ as an integer. The caller would then determine the total number of returned values by inspecting `s0`.)
...
Alternatively, one can describe $\texttt{DIV}$ as a primitive that runs on a stack $S'$ of depth $n\geq2$, divides `s1` by `s0`, and returns the floor-rounded quotient as `s0` of the new stack $S''$ of depth $n-1$. The new value of $\texttt{s}(i)$ equals the old value of $\texttt{s}(i+1)$ for $1\leq i<n-1$. These descriptions are equivalent, but saying that `DIV` transforms $x$ $y$ into $\lfloor x/y\rfloor$, or $\ldots$ $x$ $y$ into $\ldots$ $\lfloor x/y\rfloor$, is more concise.

The stack notation is extensively used throughout Appendix [opcodes section], where all currently defined TVM primitives are listed.
...
Let us present a classification of basic operations with dictionaries (i.e., values $D$ of type $\mathit{HashmapE}$ $n$ $X$):

- GET$(D,k)$ — Given $D$:$\mathit{HashmapE}(n,X)$ and a key $k:n\cdot$`bit`, returns the corresponding value $D[k]:X^?$ kept in $D$.

- SET$(D,k,x)$ — Given $D$:$\mathit{HashmapE}(n,X)$, a key $k:n\cdot$`bit`, and a value $x:X$, sets $D'[k]$ to $x$ in a copy $D'$ of $D$, and returns the resulting dictionary $D'$ (cf. [2.3.4](#2-3-4-transparency-of-the-implementation-stack-values-are-values-not-references)).
...
- EMPTY$(n)$ — Creates an empty dictionary $D$:$\mathit{HashmapE}(n,X)$.
...
- GETSUBDICT$(D,l,k_0)$ — Given $D$:$\mathit{HashmapE}(n,X)$ and some $l$-bit string $k_0:l\cdot$`bit` for $0\leq l\leq n$, returns subdictionary $D'=D/k_0$ of $D$, consisting of keys beginning with $k_0$. The result $D'$ may be of either type $\mathit{HashmapE}(n,X)$ or type $\mathit{HashmapE}(n-l,X)$.

- REPLACESUBDICT$(D,l,k_0,D')$ — Given $D$:$\mathit{HashmapE}(n,X)$, $0\leq l\leq n$, $k_0:l\cdot$`bit`, and $D'$:$\mathit{HashmapE}(n-l,X)$, replaces with $D'$ the subdictionary $D/k_0$ of $D$ consisting of keys beginning with $k_0$, and returns the resulting dictionary $D''$:$\mathit{HashmapE}(n,X)$. Some variants of REPLACESUBDICT may also return the old value of the subdictionary $D/k_0$ in question.

- DELETESUBDICT$(D,l,k_0)$ — Equivalent to REPLACESUBDICT with $D'$ being an empty dictionary.

- SPLIT$(D)$ — Given $D$:$\mathit{HashmapE}(n,X)$, returns $D_0:=D/0$ and $D_1:=D/1$:$\mathit{HashmapE}(n-1,X)$, the two subdictionaries of $D$ consisting of all keys beginning with $0$ and $1$, respectively.

- MERGE$(D_0,D_1)$ — Given $D_0$ and $D_1$:$\mathit{HashmapE}(n-1,X)$, computes $D$:$\mathit{HashmapE}(n,X)$, such that $D/0=D_0$ and $D/1=D_1$.
...
- TREEREDUCE$(D,o,f,g)$ — Given $D$:$\mathit{HashmapE}(n,X)$, a value $o:X$, and two functions $f:X\to Y$ and $g:Y\times Y\to Y$, performs a "tree reduction" of $D$ by first applying $f$ to all the leaves, and then using $g$ to compute the value corresponding to a fork starting from the values assigned to its children.<sup>[19](#fn19)</sup>
...
If the `code` is empty (i.e., contains no bits of data and no references), or if a (rarely needed) explicit subroutine return ($\texttt{RET}$) instruction is encountered, the current continuation is discarded, and the "return continuation" from control register `c0` is loaded into `cc` instead (this process is discussed in more detail starting in [4.1.6](#4-1-6-switching-to-another-continuation-jmp-and-ret)).<sup>[20](#fn20)</sup> Then the execution continues by parsing operations from the new current continuation.
...
The process of switching to another continuation $c$ may be performed by such instructions as $\texttt{JMPX}$ (which takes $c$ from the stack) or $\texttt{RET}$ (which uses `c0` as $c$). This process is slightly more complex than simply setting the value of `cc` to $c$: before doing this, either all values or the top $n$ values in the current stack are moved to the stack of the continuation $c$, and only then is the remainder of the current stack discarded.
...
After the new stack is computed, the values of control registers present in $c$.`save` are restored accordingly, and the current codepage `cp` is also set to $c$.`cp`. Only then does TVM set `cc` equal to the new $c$ and begin its execution.<sup>[22](#fn22)</sup>
...
TVM has some primitives to set and inspect the values of control registers. The most important of them are $\texttt{PUSH c}(i)$ (pushes the current value of `c`$(i)$ into the stack) and $\texttt{POP c}(i)$ (sets the value of `c`$(i)$ from the stack, if the supplied value is of the correct type). However, there is also a modified version of the latter instruction, called $\texttt{POPSAVE c}(i)$, which saves the old value of `c`$(i)$ (for $i>0$) into the continuation at `c0` as described in [4.3.2](#4-3-2-allowed-operations-with-continuations) before setting the new value.
...
One can *compose* two continuations $c$ and $c'$ simply by setting $c$.`c0` or $c$.`c1` to $c'$. This creates a new continuation denoted by $c\circ_0c'$ or $c\circ_1c'$, which differs from $c$ in its savelist. (Recall that if the savelist of $c$ already has an entry corresponding to the control register in question, such an operation silently does nothing as explained in [4.3.2](#4-3-2-allowed-operations-with-continuations)).
...
Two basic primitives for composing continuations are $\texttt{COMPOS}$ (also known as $\texttt{SETCONT c0}$ and $\texttt{BOOLAND}$) and $\texttt{COMPOSALT}$ (also known as $\texttt{SETCONT c1}$ and $\texttt{BOOLOR}$), which take $c$ and $c'$ from the stack, set $c$.`c0` or $c$.`c1` to $c'$, and return the resulting continuation $c''=c\circ_0c'$ or $c\circ_1c'$. All other continuation composition operations can be expressed in terms of these two primitives.
...
For instance, $\texttt{EXECUTE}$ can be described as `cc`$\leftarrow c\circ_0$`cc`, with continuation $c$ taken from the original stack. Similarly, $\texttt{JMPX}$ is `cc`$\leftarrow c$, and $\texttt{RET}$ (also known as $\texttt{RETTRUE}$ in a boolean circuit context) is `cc`$\leftarrow$`c0`. Other interesting primitives include $\texttt{THENRET}$ ($c'\leftarrow c\circ_0$`c0`) and $\texttt{ATEXIT}$ (`c0`$\leftarrow c\circ_0$`c0`).

Finally, some "experimental" primitives also involve `c1` and $\circ_1$. For example:

- $\texttt{RETALT}$ or $\texttt{RETFALSE}$ does `cc`$\leftarrow$`c1`.
...
- $\texttt{INVERT}$ does `c0`$\leftrightarrow$`c1`; if the two continuations in `c0` and `c1` represent the two branches we should select depending on some boolean expression, $\texttt{INVERT}$ negates this expression on the outer level.
- $\texttt{INVERTCONT}$ does $c$.`c0`$\leftrightarrow c$.`c1` to a continuation $c$ taken from the stack.
- Variants of $\texttt{ATEXIT}$ include $\texttt{ATEXITALT}$ (`c1`$\leftarrow c\circ_1$`c1`) and $\texttt{SETEXITALT}$ (`c1`$\leftarrow (c\circ_0$`c0`$)\circ_1$`c1`).
- $\texttt{BOOLEVAL}$ takes a continuation $c$ from the stack and does `cc`$\leftarrow \bigl((c\circ_0($`PUSH -1`$))\circ_1($`PUSH 0`$)\bigr)\circ_0$`cc`. If $c$ represents a boolean circuit, the net effect is to evaluate it and push either $-1$ or $0$ into the stack before continuing.
...
The exception handling itself consists in a control transfer to the exception handler—i.e., the continuation specified in control register `c2`, with $v$ and $n$ supplied as the two arguments to this continuation, as if a $\texttt{JMP}$ to `c2` had been requested with $n''=2$ arguments (cf. [4.1.7](#4-1-7-determining-the-number-n-of-arguments-passed-to-the-next-continuation-c) and [4.1.6](#4-1-6-switching-to-another-continuation-jmp-and-ret)). As a consequence, $v$ and $n$ end up in the top of the stack of the exception handler. The remainder of the old stack is discarded.

Notice that if the continuation in `c2` has a value for `c2` in its savelist, it will be used to set up the new value of `c2` before executing the exception handler. In particular, if the exception handler invokes $\texttt{THROWANY}$, it will re-throw the original exception with the restored value of `c2`. This trick enables the exception handler to handle only some exceptions, and pass the rest to an outer exception handler.
...
A $\texttt{TRY}$ primitive can be used to implement C++-like exception handling. This primitive accepts two continuations, $c$ and $c'$. It stores the old value of `c2` into the savelist of $c'$, sets `c2` to $c'$, and executes $c$ just as $\texttt{EXECUTE}$ would, but additionally saving the old value of `c2` into the savelist of the new `c0` as well. Usually a version of the $\texttt{TRY}$ primitive with an explicit number of arguments $n''$ passed to the continuation $c$ is used.
...
Every ordinary continuation contains a 16-bit *codepage* field `cp` (cf. [4.1.1](#4-1-1-ordinary-continuations)), which determines the codepage that will be used to execute its code. If a continuation is created by a `PUSHCONT` (cf. [4.2.3](#4-2-3-constant%2C-or-literal%2C-continuations)) or similar primitive, it usually inherits the current codepage (i.e., the codepage of `cc`).<sup>[25](#fn25)</sup>
...
As an exception to the above rule, some codepages may accept some values of `cc.code` that are too short to be valid instruction encodings as additional variants of $\texttt{NOP}$, thus effectively using the same procedure for them as for an empty `cc.code`. Such bitstrings may be used for padding the code near its end.

For example, if binary string `00000000` (i.e., `x00`, cf. [1.0.3](#1-0-3-emphasizing-that-a-string-is-a-hexadecimal-representation-of-a-bitstring)) is used in a codepage to encode $\texttt{NOP}$, its proper prefixes cannot encode any instructions. So this codepage may accept `0`, `00`, `000`, ..., `0000000` as variants of $\texttt{NOP}$ if this is all that is left in `cc.code`, instead of generating an invalid opcode exception.
...
For instance, if stack manipulation instructions constitute approximately half of all instructions in a typical TVM program, one should allocate approximately half of the opcode space for encoding stack manipulation instructions. One might reserve the first bytes ("opcodes") `0x00`-`0x7f` for such instructions. If a quarter of these instructions are $\texttt{XCHG}$, it would make sense to reserve `0x00`-`0x1f` for $\texttt{XCHG}$s. Similarly, if half of all $\texttt{XCHG}$s involve the top of stack `s0`, it would make sense to use `0x00`-`0x0f` to encode $\texttt{XCHG s0,s}(i)$.
...
- `00` — $\texttt{NOP}$, does nothing.
- `01` — $\texttt{XCHG s1}$, also known as $\texttt{SWAP}$.
- `0`$i$ — $\texttt{XCHG s}(i)$ or $\texttt{XCHG s0,s}(i)$, interchanges the top of the stack with $\texttt{s}(i)$, $1\leq i\leq 15$.
- `10`$ij$ — $\texttt{XCHG s}(i)\texttt{,s}(j)$, $1\leq i<j\leq15$, interchanges $\texttt{s}(i)$ with $\texttt{s}(j)$.
- `11`$ii$ — $\texttt{XCHG s0,s}(ii)$, with $0\leq ii\leq255$.
- `1`$i$ — $\texttt{XCHG s1,s}(i)$, $2\leq i\leq 15$.
- `2`$i$ — $\texttt{PUSH s}(i)$, $0\leq i\leq 15$, pushes a copy of the old $\texttt{s}(i)$ into the stack.
- `20` — $\texttt{PUSH s0}$, also known as $\texttt{DUP}$.
- `21` — $\texttt{PUSH s1}$, also known as $\texttt{OVER}$.
- `3`$i$ — $\texttt{POP s}(i)$, $0\leq i\leq 15$, pops the old top-of-stack value into the old $\texttt{s}(i)$.
- `30` — $\texttt{POP s0}$, also known as $\texttt{DROP}$, discards the top-of-stack value.
- `31` — $\texttt{POP s1}$, also known as $\texttt{NIP}$.
...
- `4`$ijk$ — $\texttt{XCHG3 s}(i)\texttt{,s}(j)\texttt{,s}(k)$, equivalent to $\texttt{XCHG s2,s}(i)$; $\texttt{XCHG s1, s}(j)$; $\texttt{XCHG s0,s}(k)$, with $0\leq i,j,k\leq 15$.
- `50`$ij$ — $\texttt{XCHG2 s}(i)\texttt{,s}(j)$, equivalent to $\texttt{XCHG s1,s}(i)$; $\texttt{XCHG s}(j)$.
- `51`$ij$ — $\texttt{XCPU s}(i)\texttt{,s}(j)$, equivalent to $\texttt{XCHG s}(i)$; $\texttt{PUSH s}(j)$.
- `52`$ij$ — $\texttt{PUXC s}(i)\texttt{,s}(j-1)$, equivalent to $\texttt{PUSH s}(i)$; $\texttt{SWAP}$; $\texttt{XCHG s}(j)$.
- `53`$ij$ — $\texttt{PUSH2 s}(i)\texttt{,s}(j)$, equivalent to $\texttt{PUSH s}(i)$; $\texttt{PUSH s}(j+1)$.
- `540`$ijk$ — $\texttt{XCHG3 s}(i)\texttt{,s}(j)\texttt{,s}(k)$ (long form).
- `541`$ijk$ — $\texttt{XC2PU s}(i)\texttt{,s}(j)\texttt{,s}(k)$, equivalent to $\texttt{XCHG2 s}(i)\texttt{,s}(j)$; $\texttt{PUSH s}(k)$.
- `542`$ijk$ — $\texttt{XCPUXC s}(i)\texttt{,s}(j)\texttt{,s}(k-1)$, equivalent to $\texttt{XCHG s1,s}(i)$; $\texttt{PUXC s}(j)\texttt{,s}(k-1)$.
- `543`$ijk$ — $\texttt{XCPU2 s}(i)\texttt{,s}(j)\texttt{,s}(k)$, equivalent to $\texttt{XCHG s}(i)$; $\texttt{PUSH2 s}(j)\texttt{, s}(k)$.
- `544`$ijk$ — $\texttt{PUXC2 s}(i)\texttt{,s}(j-1)\texttt{,s}(k-1)$, equivalent to $\texttt{PUSH s}(i)$; $\texttt{XCHG s2}$; $\texttt{XCHG2 s}(j)\texttt{,s}(k)$.
- `545`$ijk$ — $\texttt{PUXCPU s}(i)\texttt{,s}(j-1)\texttt{,s}(k-1)$, equivalent to $\texttt{PUXC s}(i)\texttt{,s}(j-1)$; $\texttt{PUSH s}(k)$.
- `546`$ijk$ — $\texttt{PU2XC s}(i)\texttt{,s}(j-1)\texttt{,s}(k-2)$, equivalent to $\texttt{PUSH s}(i)$; $\texttt{SWAP}$; $\texttt{PUXC s}(j)\texttt{,s}(k-1)$.
- `547`$ijk$ — $\texttt{PUSH3 s}(i)\texttt{,s}(j)\texttt{,s}(k)$, equivalent to $\texttt{PUSH s}(i)$; $\texttt{PUSH2 s}(j+1)\texttt{,s}(k+1)$.
- `54C_` — unused.

### A.2.3. Exotic stack manipulation primitives

- `55`$ij$ — $\texttt{BLKSWAP}$ $i+1$,$j+1$, permutes two blocks $\texttt{s}(j+i+1)\ldots\texttt{s}(j+1)$ and $\texttt{s}(j)\ldots\texttt{s0}$, for $0\leq i,j\leq15$. Equivalent to $\texttt{REVERSE}$ $i+1$,$j+1$; $\texttt{REVERSE}$ $j+1$,0; $\texttt{REVERSE}$ $i+j+2$,0.
- `5513` — $\texttt{ROT2}$ or $\texttt{2ROT}$ ($a$ $b$ $c$ $d$ $e$ $f$ -- $c$ $d$ $e$ $f$ $a$ $b$), rotates the three topmost pairs of stack entries.
- `550`$i$ — $\texttt{ROLL}$ $i+1$, rotates the top $i+1$ stack entries. Equivalent to $\texttt{BLKSWAP 1}$,$i+1$.
- `55`$i$`0` — $\texttt{ROLLREV}$ $i+1$ or $\texttt{-ROLL}$ $i+1$, rotates the top $i+1$ stack entries in the other direction. Equivalent to $\texttt{BLKSWAP}$ $i+1$,1.
- `56`$ii$ — $\texttt{PUSH s}(ii)$ for $0\leq ii\leq 255$.
- `57`$ii$ — $\texttt{POP s}(ii)$ for $0\leq ii\leq 255$.
- `58` — $\texttt{ROT}$ ($a$ $b$ $c$ -- $b$ $c$ $a$), equivalent to $\texttt{BLKSWAP 1,2}$ or to $\texttt{XCHG2 s2,s1}$.
- `59` — $\texttt{ROTREV}$ or $\texttt{-ROT}$ ($a$ $b$ $c$ -- $c$ $a$ $b$), equivalent to $\texttt{BLKSWAP 2,1}$ or to $\texttt{XCHG2 s2,s2}$.
- `5A` — $\texttt{SWAP2}$ or $\texttt{2SWAP}$ ($a$ $b$ $c$ $d$ -- $c$ $d$ $a$ $b$), equivalent to $\texttt{BLKSWAP 2,2}$ or to $\texttt{XCHG2 s3,s2}$.
- `5B` — $\texttt{DROP2}$ or $\texttt{2DROP}$ ($a$ $b$ -- ), equivalent to $\texttt{DROP}$; $\texttt{DROP}$.
- `5C` — $\texttt{DUP2}$ or $\texttt{2DUP}$ ($a$ $b$ -- $a$ $b$ $a$ $b$), equivalent to $\texttt{PUSH2 s1,s0}$.
- `5D` — $\texttt{OVER2}$ or $\texttt{2OVER}$ ($a$ $b$ $c$ $d$ -- $a$ $b$ $c$ $d$ $a$ $b$), equivalent to $\texttt{PUSH2 s3,s2}$.
- `5E`$ij$ — $\texttt{REVERSE}$ $i+2$,$j$, reverses the order of $\texttt{s}(j+i+1)\ldots\texttt{s}(j)$ for $0\leq i,j\leq 15$; equivalent to a sequence of $\lfloor i/2\rfloor+1$ $\texttt{XCHG}$s.
- `5F0`$i$ — $\texttt{BLKDROP}$ $i$, equivalent to $\texttt{DROP}$ performed $i$ times.
- `5F`$ij$ — $\texttt{BLKPUSH}$ $i$,$j$, equivalent to $\texttt{PUSH s}(j)$ performed $i$ times, $1\leq i\leq 15$, $0\leq j\leq 15$.
- `60` — $\texttt{PICK}$ or $\texttt{PUSHX}$, pops integer $i$ from the stack, then performs $\texttt{PUSH s}(i)$.
- `61` — $\texttt{ROLLX}$, pops integer $i$ from the stack, then performs $\texttt{BLKSWAP 1}$,$i$.
- `62` — $\texttt{-ROLLX}$ or $\texttt{ROLLREVX}$, pops integer $i$ from the stack, then performs $\texttt{BLKSWAP}$ $i$,1.
- `63` — $\texttt{BLKSWX}$, pops integers $i$,$j$ from the stack, then performs $\texttt{BLKSWAP}$ $i$,$j$.
- `64` — $\texttt{REVX}$, pops integers $i$,$j$ from the stack, then performs $\texttt{REVERSE}$ $i$,$j$.
- `65` — $\texttt{DROPX}$, pops integer $i$ from the stack, then performs $\texttt{BLKDROP}$ $i$.
- `66` — $\texttt{TUCK}$ ($a$ $b$ - $b$ $a$ $b$), equivalent to $\texttt{SWAP}$; $\texttt{OVER}$ or to $\texttt{XCPU s1,s1}$.
- `67` — $\texttt{XCHGX}$, pops integer $i$ from the stack, then performs $\texttt{XCHG s}(i)$.
- `68` — $\texttt{DEPTH}$, pushes the current depth of the stack.
- `69` — $\texttt{CHKDEPTH}$, pops integer $i$ from the stack, then checks whether there are at least $i$ elements, generating a stack underflow exception otherwise.
- `6A` — $\texttt{ONLYTOPX}$, pops integer $i$ from the stack, then removes all but the top $i$ elements.
- `6B` — $\texttt{ONLYX}$, pops integer $i$ from the stack, then leaves only the bottom $i$ elements. Approximately equivalent to $\texttt{DEPTH}$; $\texttt{SWAP}$; $\texttt{SUB}$; $\texttt{DROPX}$.
- `6C00`--`6C0F` — reserved for stack operations.
- `6C`$ij$ — $\texttt{BLKDROP2}$ $i$,$j$, drops $i$ stack elements under the top $j$ elements, where $1\leq i\leq15$ and $0\leq j\leq 15$. Equivalent to $\texttt{REVERSE}$ $i+j$,0; $\texttt{BLKDROP}$ $i$; $\texttt{REVERSE}$ $j$,0.
...
- `6D` — $\texttt{NULL}$ or $\texttt{PUSHNULL}$ ( -- $\bot$), pushes the only value of type *Null*.
- `6E` — $\texttt{ISNULL}$ ($x$ -- $?$), checks whether $x$ is a *Null*, and returns $-1$ or $0$ accordingly.

### A.3.2. Tuple primitives

- `6F0`$n$ — $\texttt{TUPLE}$ $n$ ($x_1$ $\ldots$ $x_n$ -- $t$), creates a new *Tuple* $t=(x_1,\ldots,x_n)$ containing $n$ values $x_1$, $\ldots$, $x_n$, where $0\leq n\leq 15$.
- `6F00` — $\texttt{NIL}$ ( -- $t$), pushes the only *Tuple* $t=()$ of length zero.
- `6F01` — $\texttt{SINGLE}$ ($x$ -- $t$), creates a singleton $t:=(x)$, i.e., a *Tuple* of length one.
- `6F02` — $\texttt{PAIR}$ or $\texttt{CONS}$ ($x$ $y$ -- $t$), creates pair $t:=(x,y)$.
- `6F03` — $\texttt{TRIPLE}$ ($x$ $y$ $z$ -- $t$), creates triple $t:=(x,y,z)$.
- `6F1`$k$ — $\texttt{INDEX}$ $k$ ($t$ -- $x$), returns the $k$-th element of a *Tuple* $t$, where $0\leq k\leq 15$. In other words, returns $x_{k+1}$ if $t=(x_1,\ldots,x_n)$. If $k\geq n$, throws a range check exception.
- `6F10` — $\texttt{FIRST}$ or $\texttt{CAR}$ ($t$ -- $x$), returns the first element of a *Tuple*.
- `6F11` — $\texttt{SECOND}$ or $\texttt{CDR}$ ($t$ -- $y$), returns the second element of a *Tuple*.
- `6F12` — $\texttt{THIRD}$ ($t$ -- $z$), returns the third element of a *Tuple*.
- `6F2`$n$ — $\texttt{UNTUPLE}$ $n$ ($t$ -- $x_1$ $\ldots$ $x_n$), unpacks a *Tuple* $t=(x_1,\ldots,x_n)$ of length equal to $0\leq n\leq 15$. If $t$ is not a *Tuple*, of if $|t|\neq n$, a type check exception is thrown.
- `6F21` — $\texttt{UNSINGLE}$ ($t$ -- $x$), unpacks a singleton $t=(x)$.
- `6F22` — $\texttt{UNPAIR}$ or $\texttt{UNCONS}$ ($t$ -- $x$ $y$), unpacks a pair $t=(x,y)$.
- `6F23` — $\texttt{UNTRIPLE}$ ($t$ -- $x$ $y$ $z$), unpacks a triple $t=(x,y,z)$.
- `6F3`$k$ — $\texttt{UNPACKFIRST}$ $k$ ($t$ -- $x_1$ $\ldots$ $x_k$), unpacks first $0\leq k\leq 15$ elements of a *Tuple* $t$. If $|t|<k$, throws a type check exception.
- `6F30` — $\texttt{CHKTUPLE}$ ($t$ -- ), checks whether $t$ is a *Tuple*.
- `6F4`$n$ — $\texttt{EXPLODE}$ $n$ ($t$ -- $x_1$ $\ldots$ $x_m$ $m$), unpacks a *Tuple* $t=(x_1,\ldots,x_m)$ and returns its length $m$, but only if $m\leq n\leq 15$. Otherwise throws a type check exception.
- `6F5`$k$ — $\texttt{SETINDEX}$ $k$ ($t$ $x$ -- $t'$), computes *Tuple* $t'$ that differs from $t$ only at position $t'_{k+1}$, which is set to $x$. In other words, $|t'|=|t|$, $t'_i=t_i$ for $i\neq k+1$, and $t'_{k+1}=x$, for given $0\leq k\leq15$. If $k\geq|t|$, throws a range check exception.
- `6F50` — $\texttt{SETFIRST}$ ($t$ $x$ -- $t'$), sets the first component of *Tuple* $t$ to $x$ and returns the resulting *Tuple* $t'$.
- `6F51` — $\texttt{SETSECOND}$ ($t$ $x$ -- $t'$), sets the second component of *Tuple* $t$ to $x$ and returns the resulting *Tuple* $t'$.
- `6F52` — $\texttt{SETTHIRD}$ ($t$ $x$ -- $t'$), sets the third component of *Tuple* $t$ to $x$ and returns the resulting *Tuple* $t'$.
- `6F6`$k$ — $\texttt{INDEXQ}$ $k$ ($t$ -- $x$), returns the $k$-th element of a *Tuple* $t$, where $0\leq k\leq 15$. In other words, returns $x_{k+1}$ if $t=(x_1,\ldots,x_n)$. If $k\geq n$, or if $t$ is *Null*, returns a *Null* instead of $x$.
- `6F7`$k$ — $\texttt{SETINDEXQ}$ $k$ ($t$ $x$ -- $t'$), sets the $k$-th component of *Tuple* $t$ to $x$, where $0\leq k<16$, and returns the resulting *Tuple* $t'$. If $|t|\leq k$, first extends the original *Tuple* to length $k+1$ by setting all new components to *Null*. If the original value of $t$ is *Null*, treats it as an empty *Tuple*. If $t$ is not *Null* or *Tuple*, throws an exception. If $x$ is *Null* and either $|t|\leq k$ or $t$ is *Null*, then always returns $t'=t$ (and does not consume tuple creation gas).
- `6F80` — $\texttt{TUPLEVAR}$ ($x_1$ $\ldots$ $x_n$ $n$ -- $t$), creates a new *Tuple* $t$ of length $n$ similarly to $\texttt{TUPLE}$, but with $0\leq n\leq 255$ taken from the stack.
- `6F81` — $\texttt{INDEXVAR}$ ($t$ $k$ -- $x$), similar to $\texttt{INDEX}$ $k$, but with $0\leq k\leq 254$ taken from the stack.
- `6F82` — $\texttt{UNTUPLEVAR}$ ($t$ $n$ -- $x_1$ $\ldots$ $x_n$), similar to $\texttt{UNTUPLE}$ $n$, but with $0\leq n\leq 255$ taken from the stack.
- `6F83` — $\texttt{UNPACKFIRSTVAR}$ ($t$ $n$ -- $x_1$ $\ldots$ $x_n$), similar to $\texttt{UNPACKFIRST}$ $n$, but with $0\leq n\leq 255$ taken from the stack.
- `6F84` — $\texttt{EXPLODEVAR}$ ($t$ $n$ -- $x_1$ $\ldots$ $x_m$ $m$), similar to $\texttt{EXPLODE}$ $n$, but with $0\leq n\leq 255$ taken from the stack.
- `6F85` — $\texttt{SETINDEXVAR}$ ($t$ $x$ $k$ -- $t'$), similar to $\texttt{SETINDEX}$ $k$, but with $0\leq k\leq 254$ taken from the stack.
- `6F86` — $\texttt{INDEXVARQ}$ ($t$ $k$ -- $x$), similar to $\texttt{INDEXQ}$ $n$, but with $0\leq k\leq 254$ taken from the stack.
- `6F87` — $\texttt{SETINDEXVARQ}$ ($t$ $x$ $k$ -- $t'$), similar to $\texttt{SETINDEXQ}$ $k$, but with $0\leq k\leq 254$ taken from the stack.
- `6F88` — $\texttt{TLEN}$ ($t$ -- $n$), returns the length of a *Tuple*.
- `6F89` — $\texttt{QTLEN}$ ($t$ -- $n$ or $-1$), similar to $\texttt{TLEN}$, but returns $-1$ if $t$ is not a *Tuple*.
- `6F8A` — $\texttt{ISTUPLE}$ ($t$ -- $?$), returns $-1$ or $0$ depending on whether $t$ is a *Tuple*.
- `6F8B` — $\texttt{LAST}$ ($t$ -- $x$), returns the last element $t_{|t|}$ of a non-empty *Tuple* $t$.
- `6F8C` — $\texttt{TPUSH}$ or $\texttt{COMMA}$ ($t$ $x$ -- $t'$), appends a value $x$ to a *Tuple* $t=(x_1,\ldots,x_n)$, but only if the resulting *Tuple* $t'=(x_1,\ldots,x_n,x)$ is of length at most 255. Otherwise throws a type check exception.
- `6F8D` — $\texttt{TPOP}$ ($t$ -- $t'$ $x$), detaches the last element $x=x_n$ from a non-empty *Tuple* $t=(x_1,\ldots,x_n)$, and returns both the resulting *Tuple* $t'=(x_1,\ldots,x_{n-1})$ and the original last element $x$.
- `6FA0` — $\texttt{NULLSWAPIF}$ ($x$ -- $x$ or $\bot$ $x$), pushes a *Null* under the topmost *Integer* $x$, but only if $x\neq0$.
- `6FA1` — $\texttt{NULLSWAPIFNOT}$ ($x$ -- $x$ or $\bot$ $x$), pushes a *Null* under the topmost *Integer* $x$, but only if $x=0$. May be used for stack alignment after quiet primitives such as $\texttt{PLDUXQ}$.
- `6FA2` — $\texttt{NULLROTRIF}$ ($x$ $y$ -- $x$ $y$ or $\bot$ $x$ $y$), pushes a *Null* under the second stack entry from the top, but only if the topmost *Integer* $y$ is non-zero.
- `6FA3` — $\texttt{NULLROTRIFNOT}$ ($x$ $y$ -- $x$ $y$ or $\bot$ $x$ $y$), pushes a *Null* under the second stack entry from the top, but only if the topmost *Integer* $y$ is zero. May be used for stack alignment after quiet primitives such as $\texttt{LDUXQ}$.
- `6FA4` — $\texttt{NULLSWAPIF2}$ ($x$ -- $x$ or $\bot$ $\bot$ $x$), pushes two *Null*s under the topmost *Integer* $x$, but only if $x\neq0$. Equivalent to $\texttt{NULLSWAPIF}$; $\texttt{NULLSWAPIF}$.
- `6FA5` — $\texttt{NULLSWAPIFNOT2}$ ($x$ -- $x$ or $\bot$ $\bot$ $x$), pushes two *Null*s under the topmost *Integer* $x$, but only if $x=0$. Equivalent to $\texttt{NULLSWAPIFNOT}$; $\texttt{NULLSWAPIFNOT}$.
- `6FA6` — $\texttt{NULLROTRIF2}$ ($x$ $y$ -- $x$ $y$ or $\bot$ $\bot$ $x$ $y$), pushes two *Null*s under the second stack entry from the top, but only if the topmost *Integer* $y$ is non-zero. Equivalent to $\texttt{NULLROTRIF}$; $\texttt{NULLROTRIF}$.
- `6FA7` — $\texttt{NULLROTRIFNOT2}$ ($x$ $y$ -- $x$ $y$ or $\bot$ $\bot$ $x$ $y$), pushes two *Null*s under the second stack entry from the top, but only if the topmost *Integer* $y$ is zero. Equivalent to $\texttt{NULLROTRIFNOT}$; $\texttt{NULLROTRIFNOT}$.
- `6FB`$ij$ — $\texttt{INDEX2}$ $i$,$j$ ($t$ -- $x$), recovers $x=(t_{i+1})_{j+1}$ for $0\leq i,j\leq 3$. Equivalent to $\texttt{INDEX}$ $i$; $\texttt{INDEX}$ $j$.
- `6FB4` — $\texttt{CADR}$ ($t$ -- $x$), recovers $x=(t_2)_1$.
- `6FB5` — $\texttt{CDDR}$ ($t$ -- $x$), recovers $x=(t_2)_2$.
- `6FE_`$ijk$ — $\texttt{INDEX3}$ $i$,$j$,$k$ ($t$ -- $x$), recovers $x=\bigl((t_{i+1})_{j+1}\bigr)_{k+1}$ for $0\leq i,j,k\leq3$. Equivalent to $\texttt{INDEX2}$ $i$,$j$; $\texttt{INDEX}$ $k$.
- `6FD4` — $\texttt{CADDR}$ ($t$ -- $x$), recovers $x=\bigl((t_2)_2\bigr)_1$.
- `6FD5` — $\texttt{CDDDR}$ ($t$ -- $x$), recovers $x=\bigl((t_2)_2\bigr)_2$.
...
- `7`$i$ — $\texttt{PUSHINT}$ $x$ with $-5\leq x\leq 10$, pushes integer $x$ into the stack; here $i$ equals four lower-order bits of $x$ (i.e., $i=x\bmod 16$).
- `70` — $\texttt{ZERO}$, $\texttt{FALSE}$, or $\texttt{PUSHINT 0}$, pushes a zero.
- `71` — $\texttt{ONE}$ or $\texttt{PUSHINT 1}$.
- `72` — $\texttt{TWO}$ or $\texttt{PUSHINT 2}$.
- `7A` — $\texttt{TEN}$ or $\texttt{PUSHINT}$ 10.
- `7F` — $\texttt{TRUE}$ or $\texttt{PUSHINT -1}$.
- `80`$xx$ — $\texttt{PUSHINT}$ $xx$ with $-128\leq xx\leq127$.
- `81`$xxxx$ — $\texttt{PUSHINT}$ $xxxx$ with $-2^{15}\leq xxxx<2^{15}$ a signed 16-bit big-endian integer.
- `81FC18` — $\texttt{PUSHINT}$ $-1000$.
- `82`$lxxx$ — $\texttt{PUSHINT}$ $xxx$, where 5-bit $0\leq l\leq30$ determines the length $n=8l+19$ of signed big-endian integer $xxx$. The total length of this instruction is $l+4$ bytes or $n+13=8l+32$ bits.
- `821005F5E100` — $\texttt{PUSHINT}$ $10^8$.
- `83`$xx$ — $\texttt{PUSHPOW2}$ $xx+1$, (quietly) pushes $2^{xx+1}$ for $0\leq xx\leq255$.
- `83FF` — $\texttt{PUSHNAN}$, pushes a $\texttt{NaN}$.
- `84`$xx$ — $\texttt{PUSHPOW2DEC}$ $xx+1$, pushes $2^{xx+1}-1$ for $0\leq xx\leq 255$.
- `85`$xx$ — $\texttt{PUSHNEGPOW2}$ $xx+1$, pushes $-2^{xx+1}$ for $0\leq xx\leq 255$.
...
- `88` — $\texttt{PUSHREF}$, pushes the first reference of `cc.code` into the stack as a *Cell* (and removes this reference from the current continuation).
- `89` — $\texttt{PUSHREFSLICE}$, similar to $\texttt{PUSHREF}$, but converts the cell into a *Slice*.
- `8A` — $\texttt{PUSHREFCONT}$, similar to $\texttt{PUSHREFSLICE}$, but makes a simple ordinary *Continuation* out of the cell.
- `8B`$x$`sss` — $\texttt{PUSHSLICE sss}$, pushes the (prefix) subslice of `cc.code` consisting of its first $8x+4$ bits and no references (i.e., essentially a bitstring), where $0\leq x\leq15$. A completion tag is assumed, meaning that all trailing zeroes and the last binary one (if present) are removed from this bitstring. If the original bitstring consists only of zeroes, an empty slice will be pushed.
- `8B08` — $\texttt{PUSHSLICE x8\_}$, pushes an empty slice (bitstring `` `' ``).
- `8B04` — $\texttt{PUSHSLICE x4\_}$, pushes bitstring `` `0' ``.
- `8B0C` — $\texttt{PUSHSLICE xC\_}$, pushes bitstring `` `1' ``.
- `8B0C` — $\texttt{PUSHSLICE xC\_}$, pushes bitstring `` `1' ``.
- `8C` $r$ $xx$ `ssss` — $\texttt{PUSHSLICE ssss}$, pushes the (prefix) subslice of `cc.code` consisting of its first $1\leq r+1\leq 4$ references and up to first $8xx+1$ bits of data, with $0\leq xx\leq 31$. A completion tag is also assumed.
- `8C01` is equivalent to $\texttt{PUSHREFSLICE}$.
- `8D` $r$ $xx$ `sssss` — $\texttt{PUSHSLICE sssss}$, pushes the subslice of `cc.code` consisting of $0\leq r\leq 4$ references and up to $8xx+6$ bits of data, with $0\leq xx\leq 127$. A completion tag is assumed.
- `8DE_` — unused (reserved).
- `8F_` $r$ $xx$ `cccc` — $\texttt{PUSHCONT cccc}$, where `cccc` is the simple ordinary continuation made from the first $0\leq r\leq 3$ references and the first $0\leq xx\leq 127$ bytes of `cc.code`.
- `9` $x$ `ccc` — $\texttt{PUSHCONT ccc}$, pushes an $x$-byte continuation for $0\leq x\leq 15$.
...
- `A0` — $\texttt{ADD}$ ($x$ $y$ -- $x+y$), adds together two integers.
- `A1` — $\texttt{SUB}$ ($x$ $y$ -- $x-y$).
- `A2` — $\texttt{SUBR}$ ($x$ $y$ -- $y-x$), equivalent to $\texttt{SWAP}$; $\texttt{SUB}$.
- `A3` — $\texttt{NEGATE}$ ($x$ -- $-x$), equivalent to $\texttt{MULCONST}$ $-1$ or to $\texttt{ZERO}$; $\texttt{SUBR}$. Notice that it triggers an integer overflow exception if $x=-2^{256}$.
- `A4` — $\texttt{INC}$ ($x$ -- $x+1$), equivalent to $\texttt{ADDCONST 1}$.
- `A5` — $\texttt{DEC}$ ($x$ -- $x-1$), equivalent to $\texttt{ADDCONST}$ $-1$.
- `A6`$cc$ — $\texttt{ADDCONST}$ $cc$ ($x$ -- $x+cc$), $-128\leq cc\leq127$.
- `A7`$cc$ — $\texttt{MULCONST}$ $cc$ ($x$ -- $x\cdot cc$), $-128\leq cc\leq127$.
- `A8` — $\texttt{MUL}$ ($x$ $y$ -- $xy$).

### A.5.2. Division

The general encoding of a $\texttt{DIV}$, $\texttt{DIVMOD}$, or $\texttt{MOD}$ operation is `A9`$mscdf$, with an optional pre-multiplication and an optional replacement of the division or multiplication by a shift. Variable one- or two-bit fields $m$, $s$, $c$, $d$, and $f$ are as follows:
...
- $0\leq f\leq2$ — Rounding mode: $0$—floor, $1$—nearest integer, $2$—ceiling (cf. [2.1.3](#2-1-3-division)).

Examples:

- `A904` — $\texttt{DIV}$ ($x$ $y$ -- $q:=\lfloor x/y\rfloor$).
- `A905` — $\texttt{DIVR}$ ($x$ $y$ -- $q':=\lfloor x/y+1/2\rfloor$).
- `A906` — $\texttt{DIVC}$ ($x$ $y$ -- $q'':=\lceil x/y\rceil$).
- `A908` — $\texttt{MOD}$ ($x$ $y$ -- $r$), where $q:=\lfloor x/y\rfloor$, $r:=x\bmod y:=x-yq$.
- `A90C` — $\texttt{DIVMOD}$ ($x$ $y$ -- $q$ $r$), where $q:=\lfloor x/y\rfloor$, $r:=x-yq$.
- `A90D` — $\texttt{DIVMODR}$ ($x$ $y$ -- $q'$ $r'$), where $q':=\lfloor x/y+1/2\rfloor$, $r':=x-yq'$.
- `A90E` — $\texttt{DIVMODC}$ ($x$ $y$ -- $q''$ $r''$), where $q'':=\lceil x/y\rceil$, $r'':=x-yq''$.
- `A924` — same as $\texttt{RSHIFT}$: ($x$ $y$ -- $\lfloor x\cdot 2^{-y}\rfloor$) for $0\leq y\leq 256$.
- `A934`$tt$ — same as $\texttt{RSHIFT}$ $tt+1$: ($x$ -- $\lfloor x\cdot 2^{-tt-1}\rfloor$).
- `A938`$tt$ — $\texttt{MODPOW2}$ $tt+1$: ($x$ -- $x\bmod 2^{tt+1}$).
- `A985` — $\texttt{MULDIVR}$ ($x$ $y$ $z$ -- $q'$), where $q'=\lfloor xy/z+1/2\rfloor$.
- `A988` — $\texttt{MULMOD}$ ($x$ $y$ $z$ -- $r$), where $r=xy\bmod z=xy-qz$, $q=\lfloor xy/z\rfloor$. This operation always succeeds for $z\neq0$ and returns the correct value of $r$, even if the intermediate result $xy$ or the quotient $q$ do not fit into 257 bits.
- `A98C` — $\texttt{MULDIVMOD}$ ($x$ $y$ $z$ -- $q$ $r$), where $q:=\lfloor x\cdot y/z\rfloor$, $r:=x\cdot y\bmod z$ (same as $\texttt{*/MOD}$ in Forth).
- `A9A4` — $\texttt{MULRSHIFT}$ ($x$ $y$ $z$ -- $\lfloor xy\cdot2^{-z}\rfloor$) for $0\leq z\leq 256$.
- `A9A5` — $\texttt{MULRSHIFTR}$ ($x$ $y$ $z$ -- $\lfloor xy\cdot2^{-z}+1/2\rfloor$) for $0\leq z\leq 256$.
- `A9B4`$tt$ — $\texttt{MULRSHIFT}$ $tt+1$ ($x$ $y$ -- $\lfloor xy\cdot 2^{-tt-1}\rfloor$).
- `A9B5`$tt$ — $\texttt{MULRSHIFTR}$ $tt+1$ ($x$ $y$ -- $\lfloor xy\cdot 2^{-tt-1}+1/2\rfloor$).
- `A9C4` — $\texttt{LSHIFTDIV}$ ($x$ $y$ $z$ -- $\lfloor 2^zx/y\rfloor$) for $0\leq z\leq 256$.
- `A9C5` — $\texttt{LSHIFTDIVR}$ ($x$ $y$ $z$ -- $\lfloor 2^zx/y+1/2\rfloor$) for $0\leq z\leq 256$.
- `A9D4`$tt$ — $\texttt{LSHIFTDIV}$ $tt+1$ ($x$ $y$ -- $\lfloor 2^{tt+1}x/y\rfloor$).
- `A9D5`$tt$ — $\texttt{LSHIFTDIVR}$ $tt+1$ ($x$ $y$ -- $\lfloor 2^{tt+1}x/y+1/2\rfloor$).
...
- `AA`$cc$ — $\texttt{LSHIFT}$ $cc+1$ ($x$ -- $x\cdot2^{cc+1}$), $0\leq cc\leq255$.
- `AA00` — $\texttt{LSHIFT 1}$, equivalent to $\texttt{MULCONST 2}$ or to Forth's $\texttt{2*}$.
- `AB`$cc$ — $\texttt{RSHIFT}$ $cc+1$ ($x$ -- $\lfloor x\cdot2^{-cc-1}\rfloor$), $0\leq cc\leq255$.
- `AC` — $\texttt{LSHIFT}$ ($x$ $y$ -- $x\cdot 2^y$), $0\leq y\leq 1023$.
- `AD` — $\texttt{RSHIFT}$ ($x$ $y$ -- $\lfloor x\cdot 2^{-y}\rfloor$), $0\leq y\leq 1023$.
- `AE` — $\texttt{POW2}$ ($y$ -- $2^y$), $0\leq y\leq1023$, equivalent to $\texttt{ONE}$; $\texttt{SWAP}$; $\texttt{LSHIFT}$.
- `AF` — reserved.
- `B0` — $\texttt{AND}$ ($x$ $y$ -- $x\&y$), bitwise "and" of two signed integers $x$ and $y$, sign-extended to infinity.
- `B1` — $\texttt{OR}$ ($x$ $y$ -- $x\vee y$), bitwise "or" of two integers.
- `B2` — $\texttt{XOR}$ ($x$ $y$ -- $x\oplus y$), bitwise "xor" of two integers.
- `B3` — $\texttt{NOT}$ ($x$ -- $x\oplus-1=-1-x$), bitwise "not" of an integer.
- `B4`$cc$ — $\texttt{FITS}$ $cc+1$ ($x$ -- $x$), checks whether $x$ is a $cc+1$-bit signed integer for $0\leq cc\leq 255$ (i.e., whether $-2^{cc}\leq x<2^{cc}$). If not, either triggers an integer overflow exception, or replaces $x$ with a $\texttt{NaN}$ (quiet version).
- `B400` — $\texttt{FITS 1}$ or $\texttt{CHKBOOL}$ ($x$ -- $x$), checks whether $x$ is a "boolean value" (i.e., either 0 or -1).
- `B5`$cc$ — $\texttt{UFITS}$ $cc+1$ ($x$ -- $x$), checks whether $x$ is a $cc+1$-bit unsigned integer for $0\leq cc\leq 255$ (i.e., whether $0\leq x<2^{cc+1}$).
- `B500` — $\texttt{UFITS 1}$ or $\texttt{CHKBIT}$, checks whether $x$ is a binary digit (i.e., zero or one).
- `B600` — $\texttt{FITSX}$ ($x$ $c$ -- $x$), checks whether $x$ is a $c$-bit signed integer for $0\leq c\leq 1023$.
- `B601` — $\texttt{UFITSX}$ ($x$ $c$ -- $x$), checks whether $x$ is a $c$-bit unsigned integer for $0\leq c\leq 1023$.
- `B602` — $\texttt{BITSIZE}$ ($x$ -- $c$), computes smallest $c\geq0$ such that $x$ fits into a $c$-bit signed integer ($-2^{c-1}\leq c<2^{c-1}$).
- `B603` — $\texttt{UBITSIZE}$ ($x$ -- $c$), computes smallest $c\geq0$ such that $x$ fits into a $c$-bit unsigned integer ($0\leq x<2^c$), or throws a range check exception.
- `B608` — $\texttt{MIN}$ ($x$ $y$ -- $x$ or $y$), computes the minimum of two integers $x$ and $y$.
- `B609` — $\texttt{MAX}$ ($x$ $y$ -- $x$ or $y$), computes the maximum of two integers $x$ and $y$.
- `B60A` — $\texttt{MINMAX}$ or $\texttt{INTSORT2}$ ($x$ $y$ -- $x$ $y$ or $y$ $x$), sorts two integers. Quiet version of this operation returns two $\texttt{NaN}$s if any of the arguments are $\texttt{NaN}$s.
- `B60B` — $\texttt{ABS}$ ($x$ -- $|x|$), computes the absolute value of an integer $x$.
...
- `B7`$xx$ — $\texttt{QUIET}$ prefix, transforming any arithmetic operation into its "quiet" variant, indicated by prefixing a `Q` to its mnemonic. Such operations return $\texttt{NaN}$s instead of throwing integer overflow exceptions if the results do not fit in *Integer*s, or if one of their arguments is a $\texttt{NaN}$. Notice that this does not extend to shift amounts and other parameters that must be within a small range (e.g., 0--1023). Also notice that this does not disable type-checking exceptions if a value of a type other than *Integer* is supplied.
- `B7A0` — $\texttt{QADD}$ ($x$ $y$ -- $x+y$), always works if $x$ and $y$ are *Integer*s, but returns a $\texttt{NaN}$ if the addition cannot be performed.
- `B7A8` — $\texttt{QMUL}$ ($x$ $y$ -- $xy$), returns the product of $x$ and $y$ if $-2^{256}\leq xy<2^{256}$. Otherwise returns a $\texttt{NaN}$, even if $x=0$ and $y$ is a $\texttt{NaN}$.
- `B7A904` — $\texttt{QDIV}$ ($x$ $y$ -- $\lfloor x/y\rfloor$), returns a $\texttt{NaN}$ if $y=0$, or if $y=-1$ and $x=-2^{256}$, or if either of $x$ or $y$ is a $\texttt{NaN}$.
- `B7A98C` — $\texttt{QMULDIVMOD}$ ($x$ $y$ $z$ -- $q$ $r$), where $q:=\lfloor x\cdot y/z\rfloor$, $r:=x\cdot y\bmod z$. If $z=0$, or if at least one of $x$, $y$, or $z$ is a $\texttt{NaN}$, both $q$ and $r$ are set to $\texttt{NaN}$. Otherwise the correct value of $r$ is always returned, but $q$ is replaced with $\texttt{NaN}$ if $q<-2^{256}$ or $q\geq2^{256}$.
- `B7B0` — $\texttt{QAND}$ ($x$ $y$ -- $x\&y$), bitwise "and" (similar to $\texttt{AND}$), but returns a $\texttt{NaN}$ if either $x$ or $y$ is a $\texttt{NaN}$ instead of throwing an integer overflow exception. However, if one of the arguments is zero, and the other is a $\texttt{NaN}$, the result is zero.
- `B7B1` — $\texttt{QOR}$ ($x$ $y$ -- $x\vee y$), bitwise "or". If $x=-1$ or $y=-1$, the result is always $-1$, even if the other argument is a $\texttt{NaN}$.
- `B7B507` — $\texttt{QUFITS 8}$ ($x$ -- $x'$), checks whether $x$ is an unsigned byte (i.e., whether $0\leq x<2^8$), and replaces $x$ with a $\texttt{NaN}$ if this is not the case; leaves $x$ intact otherwise (i.e., if $x$ is an unsigned byte or a $\texttt{NaN}$).

## A.6  Comparison primitives

### A.6.1. Integer comparison

All integer comparison primitives return integer $-1$ ("true") or $0$ ("false") to indicate the result of the comparison. We do not define their "boolean circuit" counterparts, which would transfer control to `c0` or `c1` depending on the result of the comparison. If needed, such instructions can be simulated with the aid of $\texttt{RETBOOL}$.

Quiet versions of integer comparison primitives are also available, encoded with the aid of the $\texttt{QUIET}$ prefix (`B7`). If any of the integers being compared are $\texttt{NaN}$s, the result of a quiet comparison will also be a $\texttt{NaN}$ ("undefined"), instead of a $-1$ ("yes") or $0$ ("no"), thus effectively supporting ternary logic.

- `B8` — $\texttt{SGN}$ ($x$ -- $\text{sgn}(x)$), computes the sign of an integer $x$: $-1$ if $x<0$, $0$ if $x=0$, $1$ if $x>0$.
- `B9` — $\texttt{LESS}$ ($x$ $y$ -- $x<y$), returns $-1$ if $x<y$, $0$ otherwise.
- `BA` — $\texttt{EQUAL}$ ($x$ $y$ -- $x=y$), returns $-1$ if $x=y$, $0$ otherwise.
- `BB` — $\texttt{LEQ}$ ($x$ $y$ -- $x\leq y$).
- `BC` — $\texttt{GREATER}$ ($x$ $y$ -- $x>y$).
- `BD` — $\texttt{NEQ}$ ($x$ $y$ -- $x\neq y$), equivalent to $\texttt{EQUAL}$; $\texttt{NOT}$.
- `BE` — $\texttt{GEQ}$ ($x$ $y$ -- $x\geq y$), equivalent to $\texttt{LESS}$; $\texttt{NOT}$.
- `BF` — $\texttt{CMP}$ ($x$ $y$ -- $\text{sgn}(x-y)$), computes the sign of $x-y$: $-1$ if $x<y$, $0$ if $x=y$, $1$ if $x>y$. No integer overflow can occur here unless $x$ or $y$ is a $\texttt{NaN}$.
- `C0` $yy$ — $\texttt{EQINT}$ $yy$ ($x$ -- $x=yy$) for $-2^7\leq yy<2^7$.
- `C000` — $\texttt{ISZERO}$, checks whether an integer is zero. Corresponds to Forth's $\texttt{0=}$.
- `C1` $yy$ — $\texttt{LESSINT}$ $yy$ ($x$ -- $x<yy$) for $-2^7\leq yy<2^7$.
- `C100` — $\texttt{ISNEG}$, checks whether an integer is negative. Corresponds to Forth's $\texttt{0<}$.
- `C101` — $\texttt{ISNPOS}$, checks whether an integer is non-positive.
- `C2` $yy$ — $\texttt{GTINT}$ $yy$ ($x$ -- $x>yy$) for $-2^7\leq yy<2^7$.
- `C200` — $\texttt{ISPOS}$, checks whether an integer is positive. Corresponds to Forth's $\texttt{0>}$.
- `C2FF` — $\texttt{ISNNEG}$, checks whether an integer is non-negative.
- `C3` $yy$ — $\texttt{NEQINT}$ $yy$ ($x$ -- $x\neq yy$) for $-2^7\leq yy<2^7$.
- `C4` — $\texttt{ISNAN}$ ($x$ -- $x=$`NaN`), checks whether $x$ is a $\texttt{NaN}$.
- `C5` — $\texttt{CHKNAN}$ ($x$ -- $x$), throws an arithmetic overflow exception if $x$ is a $\texttt{NaN}$.
...
- `C700` — $\texttt{SEMPTY}$ ($s$ -- $s=\emptyset$), checks whether a *Slice* $s$ is empty (i.e., contains no bits of data and no cell references).
- `C701` — $\texttt{SDEMPTY}$ ($s$ -- $s\approx\emptyset$), checks whether *Slice* $s$ has no bits of data.
- `C702` — $\texttt{SREMPTY}$ ($s$ -- $r(s)=0$), checks whether *Slice* $s$ has no references.
- `C703` — $\texttt{SDFIRST}$ ($s$ -- $s_0=1$), checks whether the first bit of *Slice* $s$ is a one.
- `C704` — $\texttt{SDLEXCMP}$ ($s$ $s'$ -- $c$), compares the data of $s$ lexicographically with the data of $s'$, returning $-1$, 0, or 1 depending on the result.
- `C705` — $\texttt{SDEQ}$ ($s$ $s'$ -- $s\approx s'$), checks whether the data parts of $s$ and $s'$ coincide, equivalent to $\texttt{SDLEXCMP}$; $\texttt{ISZERO}$.
- `C708` — $\texttt{SDPFX}$ ($s$ $s'$ -- $?$), checks whether $s$ is a prefix of $s'$.
- `C709` — $\texttt{SDPFXREV}$ ($s$ $s'$ -- $?$), checks whether $s'$ is a prefix of $s$, equivalent to $\texttt{SWAP}$; $\texttt{SDPFX}$.
- `C70A` — $\texttt{SDPPFX}$ ($s$ $s'$ -- $?$), checks whether $s$ is a proper prefix of $s'$ (i.e., a prefix distinct from $s'$).
- `C70B` — $\texttt{SDPPFXREV}$ ($s$ $s'$ -- $?$), checks whether $s'$ is a proper prefix of $s$.
- `C70C` — $\texttt{SDSFX}$ ($s$ $s'$ -- $?$), checks whether $s$ is a suffix of $s'$.
- `C70D` — $\texttt{SDSFXREV}$ ($s$ $s'$ -- $?$), checks whether $s'$ is a suffix of $s$.
- `C70E` — $\texttt{SDPSFX}$ ($s$ $s'$ -- $?$), checks whether $s$ is a proper suffix of $s'$.
- `C70F` — $\texttt{SDPSFXREV}$ ($s$ $s'$ -- $?$), checks whether $s'$ is a proper suffix of $s$.
- `C710` — $\texttt{SDCNTLEAD0}$ ($s$ -- $n$), returns the number of leading zeroes in $s$.
- `C711` — $\texttt{SDCNTLEAD1}$ ($s$ -- $n$), returns the number of leading ones in $s$.
- `C712` — $\texttt{SDCNTTRAIL0}$ ($s$ -- $n$), returns the number of trailing zeroes in $s$.
- `C713` — $\texttt{SDCNTTRAIL1}$ ($s$ -- $n$), returns the number of trailing ones in $s$.
...
- `C8` — $\texttt{NEWC}$ ( -- $b$), creates a new empty *Builder*.
- `C9` — $\texttt{ENDC}$ ($b$ -- $c$), converts a *Builder* into an ordinary *Cell*.
- `CA` $cc$ — $\texttt{STI}$ $cc+1$ ($x$ $b$ -- $b'$), stores a signed $cc+1$-bit integer $x$ into *Builder* $b$ for $0\leq cc\leq 255$, throws a range check exception if $x$ does not fit into $cc+1$ bits.
- `CB` $cc$ — $\texttt{STU}$ $cc+1$ ($x$ $b$ -- $b'$), stores an unsigned $cc+1$-bit integer $x$ into *Builder* $b$. In all other respects it is similar to $\texttt{STI}$.
- `CC` — $\texttt{STREF}$ ($c$ $b$ -- $b'$), stores a reference to *Cell* $c$ into *Builder* $b$.
- `CD` — $\texttt{STBREFR}$ or $\texttt{ENDCST}$ ($b$ $b''$ -- $b$), equivalent to $\texttt{ENDC}$; $\texttt{SWAP}$; $\texttt{STREF}$.
- `CE` — $\texttt{STSLICE}$ ($s$ $b$ -- $b'$), stores *Slice* $s$ into *Builder* $b$.
- `CF00` — $\texttt{STIX}$ ($x$ $b$ $l$ -- $b'$), stores a signed $l$-bit integer $x$ into $b$ for $0\leq l\leq 257$.
- `CF01` — $\texttt{STUX}$ ($x$ $b$ $l$ -- $b'$), stores an unsigned $l$-bit integer $x$ into $b$ for $0\leq l\leq 256$.
- `CF02` — $\texttt{STIXR}$ ($b$ $x$ $l$ -- $b'$), similar to $\texttt{STIX}$, but with arguments in a different order.
- `CF03` — $\texttt{STUXR}$ ($b$ $x$ $l$ -- $b'$), similar to $\texttt{STUX}$, but with arguments in a different order.
- `CF04` — $\texttt{STIXQ}$ ($x$ $b$ $l$ -- $x$ $b$ $f$ or $b'$ $0$), a quiet version of $\texttt{STIX}$. If there is no space in $b$, sets $b'=b$ and $f=-1$. If $x$ does not fit into $l$ bits, sets $b'=b$ and $f=1$. If the operation succeeds, $b'$ is the new *Builder* and $f=0$. However, $0\leq l\leq 257$, with a range check exception if this is not so.
- `CF05` — $\texttt{STUXQ}$ ($x$ $b$ $l$ -- $b'$ $f$).
- `CF06` — $\texttt{STIXRQ}$ ($b$ $x$ $l$ -- $b$ $x$ $f$ or $b'$ $0$).
- `CF07` — $\texttt{STUXRQ}$ ($b$ $x$ $l$ -- $b$ $x$ $f$ or $b'$ $0$).
- `CF08` $cc$ — a longer version of $\texttt{STI}$ $cc+1$.
- `CF09` $cc$ — a longer version of $\texttt{STU}$ $cc+1$.
- `CF0A` $cc$ — $\texttt{STIR}$ $cc+1$ ($b$ $x$ -- $b'$), equivalent to $\texttt{SWAP}$; $\texttt{STI}$ $cc+1$.
- `CF0B` $cc$ — $\texttt{STUR}$ $cc+1$ ($b$ $x$ -- $b'$), equivalent to $\texttt{SWAP}$; $\texttt{STU}$ $cc+1$.
- `CF0C` $cc$ — $\texttt{STIQ}$ $cc+1$ ($x$ $b$ -- $x$ $b$ $f$ or $b'$ $0$).
- `CF0D` $cc$ — $\texttt{STUQ}$ $cc+1$ ($x$ $b$ -- $x$ $b$ $f$ or $b'$ $0$).
- `CF0E` $cc$ — $\texttt{STIRQ}$ $cc+1$ ($b$ $x$ -- $b$ $x$ $f$ or $b'$ $0$).
- `CF0F` $cc$ — $\texttt{STURQ}$ $cc+1$ ($b$ $x$ -- $b$ $x$ $f$ or $b'$ $0$).
- `CF10` — a longer version of $\texttt{STREF}$ ($c$ $b$ -- $b'$).
- `CF11` — $\texttt{STBREF}$ ($b'$ $b$ -- $b''$), equivalent to $\texttt{SWAP}$; $\texttt{STBREFREV}$.
- `CF12` — a longer version of $\texttt{STSLICE}$ ($s$ $b$ -- $b'$).
- `CF13` — $\texttt{STB}$ ($b'$ $b$ -- $b''$), appends all data from *Builder* $b'$ to *Builder* $b$.
- `CF14` — $\texttt{STREFR}$ ($b$ $c$ -- $b'$).
- `CF15` — $\texttt{STBREFR}$ ($b$ $b'$ -- $b''$), a longer encoding of $\texttt{STBREFR}$.
- `CF16` — $\texttt{STSLICER}$ ($b$ $s$ -- $b'$).
- `CF17` — $\texttt{STBR}$ ($b$ $b'$ -- $b''$), concatenates two *Builder*s, equivalent to $\texttt{SWAP}$; $\texttt{STB}$.
- `CF18` — $\texttt{STREFQ}$ ($c$ $b$ -- $c$ $b$ $-1$ or $b'$ $0$).
- `CF19` — $\texttt{STBREFQ}$ ($b'$ $b$ -- $b'$ $b$ $-1$ or $b''$ $0$).
- `CF1A` — $\texttt{STSLICEQ}$ ($s$ $b$ -- $s$ $b$ $-1$ or $b'$ $0$).
- `CF1B` — $\texttt{STBQ}$ ($b'$ $b$ -- $b'$ $b$ $-1$ or $b''$ $0$).
- `CF1C` — $\texttt{STREFRQ}$ ($b$ $c$ -- $b$ $c$ $-1$ or $b'$ $0$).
- `CF1D` — $\texttt{STBREFRQ}$ ($b$ $b'$ -- $b$ $b'$ $-1$ or $b''$ $0$).
- `CF1E` — $\texttt{STSLICERQ}$ ($b$ $s$ -- $b$ $s$ $-1$ or $b''$ $0$).
- `CF1F` — $\texttt{STBRQ}$ ($b$ $b'$ -- $b$ $b'$ $-1$ or $b''$ $0$).
- `CF20` — $\texttt{STREFCONST}$, equivalent to $\texttt{PUSHREF}$; $\texttt{STREFR}$.
- `CF21` — $\texttt{STREF2CONST}$, equivalent to $\texttt{STREFCONST}$; $\texttt{STREFCONST}$.
- `CF23` — $\texttt{ENDXC}$ ($b$ $x$ -- $c$), if $x\neq0$, creates a *special* or *exotic* cell (cf. [3.1.7](#3-1-7-exotic-cells)) from *Builder* $b$. The type of the exotic cell must be stored in the first 8 bits of $b$. If $x=0$, it is equivalent to $\texttt{ENDC}$. Otherwise some validity checks on the data and references of $b$ are performed before creating the exotic cell.
- `CF28` — $\texttt{STILE4}$ ($x$ $b$ -- $b'$), stores a little-endian signed 32-bit integer.
- `CF29` — $\texttt{STULE4}$ ($x$ $b$ -- $b'$), stores a little-endian unsigned 32-bit integer.
- `CF2A` — $\texttt{STILE8}$ ($x$ $b$ -- $b'$), stores a little-endian signed 64-bit integer.
- `CF2B` — $\texttt{STULE8}$ ($x$ $b$ -- $b'$), stores a little-endian unsigned 64-bit integer.
- `CF30` — $\texttt{BDEPTH}$ ($b$ -- $x$), returns the depth of *Builder* $b$. If no cell references are stored in $b$, then $x=0$; otherwise $x$ is one plus the maximum of depths of cells referred to from $b$.
- `CF31` — $\texttt{BBITS}$ ($b$ -- $x$), returns the number of data bits already stored in *Builder* $b$.
- `CF32` — $\texttt{BREFS}$ ($b$ -- $y$), returns the number of cell references already stored in $b$.
- `CF33` — $\texttt{BBITREFS}$ ($b$ -- $x$ $y$), returns the numbers of both data bits and cell references in $b$.
- `CF35` — $\texttt{BREMBITS}$ ($b$ -- $x'$), returns the number of data bits that can still be stored in $b$.
- `CF36` — $\texttt{BREMREFS}$ ($b$ -- $y'$).
- `CF37` — $\texttt{BREMBITREFS}$ ($b$ -- $x'$ $y'$).
- `CF38` $cc$ — $\texttt{BCHKBITS}$ $cc+1$ ($b$ --), checks whether $cc+1$ bits can be stored into $b$, where $0\leq cc\leq 255$.
- `CF39` — $\texttt{BCHKBITS}$ ($b$ $x$ -- ), checks whether $x$ bits can be stored into $b$, $0\leq x\leq 1023$. If there is no space for $x$ more bits in $b$, or if $x$ is not within the range $0\ldots1023$, throws an exception.
- `CF3A` — $\texttt{BCHKREFS}$ ($b$ $y$ -- ), checks whether $y$ references can be stored into $b$, $0\leq y\leq 7$.
- `CF3B` — $\texttt{BCHKBITREFS}$ ($b$ $x$ $y$ -- ), checks whether $x$ bits and $y$ references can be stored into $b$, $0\leq x\leq 1023$, $0\leq y\leq 7$.
- `CF3C` $cc$ — $\texttt{BCHKBITSQ}$ $cc+1$ ($b$ -- $?$), checks whether $cc+1$ bits can be stored into $b$, where $0\leq cc\leq 255$.
- `CF3D` — $\texttt{BCHKBITSQ}$ ($b$ $x$ -- $?$), checks whether $x$ bits can be stored into $b$, $0\leq x\leq 1023$.
- `CF3E` — $\texttt{BCHKREFSQ}$ ($b$ $y$ -- $?$), checks whether $y$ references can be stored into $b$, $0\leq y\leq 7$.
- `CF3F` — $\texttt{BCHKBITREFSQ}$ ($b$ $x$ $y$ -- $?$), checks whether $x$ bits and $y$ references can be stored into $b$, $0\leq x\leq 1023$, $0\leq y\leq 7$.
- `CF40` — $\texttt{STZEROES}$ ($b$ $n$ -- $b'$), stores $n$ binary zeroes into *Builder* $b$.
- `CF41` — $\texttt{STONES}$ ($b$ $n$ -- $b'$), stores $n$ binary ones into *Builder* $b$.
- `CF42` — $\texttt{STSAME}$ ($b$ $n$ $x$ -- $b'$), stores $n$ binary $x$es ($0\leq x\leq1$) into *Builder* $b$.
- `CFC0_` $xysss$ — $\texttt{STSLICECONST}$ $sss$ ($b$ -- $b'$), stores a constant subslice $sss$ consisting of $0\leq x\leq 3$ references and up to $8y+1$ data bits, with $0\leq y\leq 7$. Completion bit is assumed.
- `CF81` — $\texttt{STSLICECONST '0'}$ or $\texttt{STZERO}$ ($b$ -- $b'$), stores one binary zero.
- `CF83` — $\texttt{STSLICECONST '1'}$ or $\texttt{STONE}$ ($b$ -- $b'$), stores one binary one.
- `CFA2` — equivalent to $\texttt{STREFCONST}$.
- `CFA3` — almost equivalent to $\texttt{STSLICECONST '1'}$; $\texttt{STREFCONST}$.
- `CFC2` — equivalent to $\texttt{STREF2CONST}$.
- `CFE2` — $\texttt{STREF3CONST}$.

### A.7.2. Cell deserialization primitives

- `D0` — $\texttt{CTOS}$ ($c$ -- $s$), converts a *Cell* into a *Slice*. Notice that $c$ must be either an ordinary cell, or an exotic cell (cf. [3.1.7](#3-1-7-exotic-cells)) which is automatically *loaded* to yield an ordinary cell $c'$, converted into a *Slice* afterwards.
- `D1` — $\texttt{ENDS}$ ($s$ -- ), removes a *Slice* $s$ from the stack, and throws an exception if it is not empty.
- `D2` $cc$ — $\texttt{LDI}$ $cc+1$ ($s$ -- $x$ $s'$), loads (i.e., parses) a signed $cc+1$-bit integer $x$ from *Slice* $s$, and returns the remainder of $s$ as $s'$.
- `D3` $cc$ — $\texttt{LDU}$ $cc+1$ ($s$ -- $x$ $s'$), loads an unsigned $cc+1$-bit integer $x$ from *Slice* $s$.
- `D4` — $\texttt{LDREF}$ ($s$ -- $c$ $s'$), loads a cell reference $c$ from $s$.
- `D5` — $\texttt{LDREFRTOS}$ ($s$ -- $s'$ $s''$), equivalent to $\texttt{LDREF}$; $\texttt{SWAP}$; $\texttt{CTOS}$.
- `D6` $cc$ — $\texttt{LDSLICE}$ $cc+1$ ($s$ -- $s''$ $s'$), cuts the next $cc+1$ bits of $s$ into a separate *Slice* $s''$.
- `D700` — $\texttt{LDIX}$ ($s$ $l$ -- $x$ $s'$), loads a signed $l$-bit ($0\leq l\leq 257$) integer $x$ from *Slice* $s$, and returns the remainder of $s$ as $s'$.
- `D701` — $\texttt{LDUX}$ ($s$ $l$ -- $x$ $s'$), loads an unsigned $l$-bit integer $x$ from (the first $l$ bits of) $s$, with $0\leq l\leq 256$.
- `D702` — $\texttt{PLDIX}$ ($s$ $l$ -- $x$), preloads a signed $l$-bit integer from *Slice* $s$, for $0\leq l\leq 257$.
- `D703` — $\texttt{PLDUX}$ ($s$ $l$ -- $x$), preloads an unsigned $l$-bit integer from $s$, for $0\leq l\leq 256$.
- `D704` — $\texttt{LDIXQ}$ ($s$ $l$ -- $x$ $s'$ $-1$ or $s$ $0$), quiet version of $\texttt{LDIX}$: loads a signed $l$-bit integer from $s$ similarly to $\texttt{LDIX}$, but returns a success flag, equal to $-1$ on success or to $0$ on failure (if $s$ does not have $l$ bits), instead of throwing a cell underflow exception.
- `D705` — $\texttt{LDUXQ}$ ($s$ $l$ -- $x$ $s'$ $-1$ or $s$ $0$), quiet version of $\texttt{LDUX}$.
- `D706` — $\texttt{PLDIXQ}$ ($s$ $l$ -- $x$ $-1$ or $0$), quiet version of $\texttt{PLDIX}$.
- `D707` — $\texttt{PLDUXQ}$ ($s$ $l$ -- $x$ $-1$ or $0$), quiet version of $\texttt{PLDUX}$.
- `D708` $cc$ — $\texttt{LDI}$ $cc+1$ ($s$ -- $x$ $s'$), a longer encoding for $\texttt{LDI}$.
- `D709` $cc$ — $\texttt{LDU}$ $cc+1$ ($s$ -- $x$ $s'$), a longer encoding for $\texttt{LDU}$.
- `D70A` $cc$ — $\texttt{PLDI}$ $cc+1$ ($s$ -- $x$), preloads a signed $cc+1$-bit integer from *Slice* $s$.
- `D70B` $cc$ — $\texttt{PLDU}$ $cc+1$ ($s$ -- $x$), preloads an unsigned $cc+1$-bit integer from $s$.
- `D70C` $cc$ — $\texttt{LDIQ}$ $cc+1$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), a quiet version of $\texttt{LDI}$.
- `D70D` $cc$ — $\texttt{LDUQ}$ $cc+1$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), a quiet version of $\texttt{LDU}$.
- `D70E` $cc$ — $\texttt{PLDIQ}$ $cc+1$ ($s$ -- $x$ $-1$ or $0$), a quiet version of $\texttt{PLDI}$.
- `D70F` $cc$ — $\texttt{PLDUQ}$ $cc+1$ ($s$ -- $x$ $-1$ or $0$), a quiet version of $\texttt{PLDU}$.
- `D714_` $c$ — $\texttt{PLDUZ}$ $32(c+1)$ ($s$ -- $s$ $x$), preloads the first $32(c+1)$ bits of *Slice* $s$ into an unsigned integer $x$, for $0\leq c\leq 7$. If $s$ is shorter than necessary, missing bits are assumed to be zero. This operation is intended to be used along with $\texttt{IFBITJMP}$ and similar instructions.
- `D718` — $\texttt{LDSLICEX}$ ($s$ $l$ -- $s''$ $s'$), loads the first $0\leq l\leq 1023$ bits from *Slice* $s$ into a separate *Slice* $s''$, returning the remainder of $s$ as $s'$.
- `D719` — $\texttt{PLDSLICEX}$ ($s$ $l$ -- $s''$), returns the first $0\leq l\leq 1023$ bits of $s$ as $s''$.
- `D71A` — $\texttt{LDSLICEXQ}$ ($s$ $l$ -- $s''$ $s'$ $-1$ or $s$ $0$), a quiet version of $\texttt{LDSLICEX}$.
- `D71B` — $\texttt{PLDSLICEXQ}$ ($s$ $l$ -- $s'$ $-1$ or $0$), a quiet version of $\texttt{LDSLICEXQ}$.
- `D71C` $cc$ — $\texttt{LDSLICE}$ $cc+1$ ($s$ -- $s''$ $s'$), a longer encoding for $\texttt{LDSLICE}$.
- `D71D` $cc$ — $\texttt{PLDSLICE}$ $cc+1$ ($s$ -- $s''$), returns the first $0<cc+1\leq 256$ bits of $s$ as $s''$.
- `D71E` $cc$ — $\texttt{LDSLICEQ}$ $cc+1$ ($s$ -- $s''$ $s'$ $-1$ or $s$ $0$), a quiet version of $\texttt{LDSLICE}$.
- `D71F` $cc$ — $\texttt{PLDSLICEQ}$ $cc+1$ ($s$ -- $s''$ $-1$ or $0$), a quiet version of $\texttt{PLDSLICE}$.
- `D720` — $\texttt{SDCUTFIRST}$ ($s$ $l$ -- $s'$), returns the first $0\leq l\leq 1023$ bits of $s$. It is equivalent to $\texttt{PLDSLICEX}$.
- `D721` — $\texttt{SDSKIPFIRST}$ ($s$ $l$ -- $s'$), returns all but the first $0\leq l\leq 1023$ bits of $s$. It is equivalent to $\texttt{LDSLICEX}$; $\texttt{NIP}$.
- `D722` — $\texttt{SDCUTLAST}$ ($s$ $l$ -- $s'$), returns the last $0\leq l\leq 1023$ bits of $s$.
- `D723` — $\texttt{SDSKIPLAST}$ ($s$ $l$ -- $s'$), returns all but the last $0\leq l\leq 1023$ bits of $s$.
- `D724` — $\texttt{SDSUBSTR}$ ($s$ $l$ $l'$ -- $s'$), returns $0\leq l'\leq 1023$ bits of $s$ starting from offset $0\leq l\leq 1023$, thus extracting a bit substring out of the data of $s$.
- `D726` — $\texttt{SDBEGINSX}$ ($s$ $s'$ -- $s''$), checks whether $s$ begins with (the data bits of) $s'$, and removes $s'$ from $s$ on success. On failure throws a cell deserialization exception. Primitive $\texttt{SDPFXREV}$ can be considered a quiet version of $\texttt{SDBEGINSX}$.
- `D727` — $\texttt{SDBEGINSXQ}$ ($s$ $s'$ -- $s''$ $-1$ or $s$ $0$), a quiet version of $\texttt{SDBEGINSX}$.
- `D72A_` $xsss$ — $\texttt{SDBEGINS}$ ($s$ -- $s''$), checks whether $s$ begins with constant bitstring $sss$ of length $8x+3$ (with continuation bit assumed), where $0\leq x\leq 127$, and removes $sss$ from $s$ on success.
- `D72802` — $\texttt{SDBEGINS '0'}$ ($s$ -- $s''$), checks whether $s$ begins with a binary zero.
- `D72806` — $\texttt{SDBEGINS '1'}$ ($s$ -- $s''$), checks whether $s$ begins with a binary one.
- `D72E_` $xsss$ — $\texttt{SDBEGINSQ}$ ($s$ -- $s''$ $-1$ or $s$ $0$), a quiet version of $\texttt{SDBEGINS}$.
- `D730` — $\texttt{SCUTFIRST}$ ($s$ $l$ $r$ -- $s'$), returns the first $0\leq l\leq 1023$ bits and first $0\leq r\leq 4$ references of $s$.
- `D731` — $\texttt{SSKIPFIRST}$ ($s$ $l$ $r$ -- $s'$).
- `D732` — $\texttt{SCUTLAST}$ ($s$ $l$ $r$ -- $s'$), returns the last $0\leq l\leq 1023$ data bits and last $0\leq r\leq 4$ references of $s$.
- `D733` — $\texttt{SSKIPLAST}$ ($s$ $l$ $r$ -- $s'$).
- `D734` — $\texttt{SUBSLICE}$ ($s$ $l$ $r$ $l'$ $r'$ -- $s'$), returns $0\leq l'\leq 1023$ bits and $0\leq r'\leq 4$ references from *Slice* $s$, after skipping the first $0\leq l\leq 1023$ bits and first $0\leq r\leq 4$ references.
- `D736` — $\texttt{SPLIT}$ ($s$ $l$ $r$ -- $s'$ $s''$), splits the first $0\leq l\leq 1023$ data bits and first $0\leq r\leq 4$ references from $s$ into $s'$, returning the remainder of $s$ as $s''$.
- `D737` — $\texttt{SPLITQ}$ ($s$ $l$ $r$ -- $s'$ $s''$ $-1$ or $s$ $0$), a quiet version of $\texttt{SPLIT}$.
- `D739` — $\texttt{XCTOS}$ ($c$ -- $s$ $?$), transforms an ordinary or exotic cell into a *Slice*, as if it were an ordinary cell. A flag is returned indicating whether $c$ is exotic. If that be the case, its type can later be deserialized from the first eight bits of $s$.
- `D73A` — $\texttt{XLOAD}$ ($c$ -- $c'$), loads an exotic cell $c$ and returns an ordinary cell $c'$. If $c$ is already ordinary, does nothing. If $c$ cannot be loaded, throws an exception.
- `D73B` — $\texttt{XLOADQ}$ ($c$ -- $c'$ $-1$ or $c$ $0$), loads an exotic cell $c$ as $\texttt{XLOAD}$, but returns 0 on failure.
- `D741` — $\texttt{SCHKBITS}$ ($s$ $l$ -- ), checks whether there are at least $l$ data bits in *Slice* $s$. If this is not the case, throws a cell deserialisation (i.e., cell underflow) exception.
- `D742` — $\texttt{SCHKREFS}$ ($s$ $r$ -- ), checks whether there are at least $r$ references in *Slice* $s$.
- `D743` — $\texttt{SCHKBITREFS}$ ($s$ $l$ $r$ -- ), checks whether there are at least $l$ data bits and $r$ references in *Slice* $s$.
- `D745` — $\texttt{SCHKBITSQ}$ ($s$ $l$ -- $?$), checks whether there are at least $l$ data bits in *Slice* $s$.
- `D746` — $\texttt{SCHKREFSQ}$ ($s$ $r$ -- $?$), checks whether there are at least $r$ references in *Slice* $s$.
- `D747` — $\texttt{SCHKBITREFSQ}$ ($s$ $l$ $r$ -- $?$), checks whether there are at least $l$ data bits and $r$ references in *Slice* $s$.
- `D748` — $\texttt{PLDREFVAR}$ ($s$ $n$ -- $c$), returns the $n$-th cell reference of *Slice* $s$ for $0\leq n\leq 3$.
- `D749` — $\texttt{SBITS}$ ($s$ -- $l$), returns the number of data bits in *Slice* $s$.
- `D74A` — $\texttt{SREFS}$ ($s$ -- $r$), returns the number of references in *Slice* $s$.
- `D74B` — $\texttt{SBITREFS}$ ($s$ -- $l$ $r$), returns both the number of data bits and the number of references in $s$.
- `D74E_` $n$ — $\texttt{PLDREFIDX}$ $n$ ($s$ -- $c$), returns the $n$-th cell reference of *Slice* $s$, where $0\leq n\leq 3$.
- `D74C` — $\texttt{PLDREF}$ ($s$ -- $c$), preloads the first cell reference of a *Slice*.
- `D750` — $\texttt{LDILE4}$ ($s$ -- $x$ $s'$), loads a little-endian signed 32-bit integer.
- `D751` — $\texttt{LDULE4}$ ($s$ -- $x$ $s'$), loads a little-endian unsigned 32-bit integer.
- `D752` — $\texttt{LDILE8}$ ($s$ -- $x$ $s'$), loads a little-endian signed 64-bit integer.
- `D753` — $\texttt{LDULE8}$ ($s$ -- $x$ $s'$), loads a little-endian unsigned 64-bit integer.
- `D754` — $\texttt{PLDILE4}$ ($s$ -- $x$), preloads a little-endian signed 32-bit integer.
- `D755` — $\texttt{PLDULE4}$ ($s$ -- $x$), preloads a little-endian unsigned 32-bit integer.
- `D756` — $\texttt{PLDILE8}$ ($s$ -- $x$), preloads a little-endian signed 64-bit integer.
- `D757` — $\texttt{PLDULE8}$ ($s$ -- $x$), preloads a little-endian unsigned 64-bit integer.
- `D758` — $\texttt{LDILE4Q}$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), quietly loads a little-endian signed 32-bit integer.
- `D759` — $\texttt{LDULE4Q}$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), quietly loads a little-endian unsigned 32-bit integer.
- `D75A` — $\texttt{LDILE8Q}$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), quietly loads a little-endian signed 64-bit integer.
- `D75B` — $\texttt{LDULE8Q}$ ($s$ -- $x$ $s'$ $-1$ or $s$ $0$), quietly loads a little-endian unsigned 64-bit integer.
- `D75C` — $\texttt{PLDILE4Q}$ ($s$ -- $x$ $-1$ or $0$), quietly preloads a little-endian signed 32-bit integer.
- `D75D` — $\texttt{PLDULE4Q}$ ($s$ -- $x$ $-1$ or $0$), quietly preloads a little-endian unsigned 32-bit integer.
- `D75E` — $\texttt{PLDILE8Q}$ ($s$ -- $x$ $-1$ or $0$), quietly preloads a little-endian signed 64-bit integer.
- `D75F` — $\texttt{PLDULE8Q}$ ($s$ -- $x$ $-1$ or $0$), quietly preloads a little-endian unsigned 64-bit integer.
- `D760` — $\texttt{LDZEROES}$ ($s$ -- $n$ $s'$), returns the count $n$ of leading zero bits in $s$, and removes these bits from $s$.
- `D761` — $\texttt{LDONES}$ ($s$ -- $n$ $s'$), returns the count $n$ of leading one bits in $s$, and removes these bits from $s$.
- `D762` — $\texttt{LDSAME}$ ($s$ $x$ -- $n$ $s'$), returns the count $n$ of leading bits equal to $0\leq x\leq 1$ in $s$, and removes these bits from $s$.
- `D764` — $\texttt{SDEPTH}$ ($s$ -- $x$), returns the depth of *Slice* $s$. If $s$ has no references, then $x=0$; otherwise $x$ is one plus the maximum of depths of cells referred to from $s$.
- `D765` — $\texttt{CDEPTH}$ ($c$ -- $x$), returns the depth of *Cell* $c$. If $c$ has no references, then $x=0$; otherwise $x$ is one plus the maximum of depths of cells referred to from $c$. If $c$ is a *Null* instead of a *Cell*, returns zero.
...
- `D8` — $\texttt{EXECUTE}$ or $\texttt{CALLX}$ ($c$ -- ), *calls* or *executes* continuation $c$ (i.e., `cc`$\leftarrow c\circ_0$`cc`).
- `D9` — $\texttt{JMPX}$ ($c$ -- ), *jumps*, or transfers control, to continuation $c$ (i.e., `cc`$\leftarrow c\circ_0$`c0`, or rather `cc`$\leftarrow(c\circ_0$`c0`$)\circ_1$`c1`). The remainder of the previous current continuation `cc` is discarded.
- `DA` $pr$ — $\texttt{CALLXARGS}$ $p$,$r$ ($c$ -- ), *calls* continuation $c$ with $p$ parameters and expecting $r$ return values, $0\leq p\leq15$, $0\leq r\leq 15$.
- `DB0` $p$ — $\texttt{CALLXARGS}$ $p$,$-1$ ($c$ -- ), *calls* continuation $c$ with $0\leq p\leq15$ parameters, expecting an arbitrary number of return values.
- `DB1` $p$ — $\texttt{JMPXARGS}$ $p$ ($c$ -- ), *jumps* to continuation $c$, passing only the top $0\leq p\leq 15$ values from the current stack to it (the remainder of the current stack is discarded).
- `DB2` $r$ — $\texttt{RETARGS}$ $r$, *returns* to `c0`, with $0\leq r\leq 15$ return values taken from the current stack.
- `DB30` — $\texttt{RET}$ or $\texttt{RETTRUE}$, *returns* to the continuation at `c0` (i.e., performs `cc`$\leftarrow$`c0`). The remainder of the current continuation `cc` is discarded. Approximately equivalent to $\texttt{PUSH c0}$; $\texttt{JMPX}$.
- `DB31` — $\texttt{RETALT}$ or $\texttt{RETFALSE}$, *returns* to the continuation at `c1` (i.e., `cc`$\leftarrow$`c1`). Approximately equivalent to $\texttt{PUSH c1}$; $\texttt{JMPX}$.
- `DB32` — $\texttt{BRANCH}$ or $\texttt{RETBOOL}$ ($f$ -- ), performs $\texttt{RETTRUE}$ if integer $f\neq 0$, or $\texttt{RETFALSE}$ if $f=0$.
- `DB34` — $\texttt{CALLCC}$ ($c$ -- ), *call with current continuation*, transfers control to $c$, pushing the old value of `cc` into $c$'s stack (instead of discarding it or writing it into new `c0`).
- `DB35` — $\texttt{JMPXDATA}$ ($c$ -- ), similar to $\texttt{CALLCC}$, but the remainder of the current continuation (the old value of `cc`) is converted into a *Slice* before pushing it into the stack of $c$.
- `DB36` $pr$ — $\texttt{CALLCCARGS}$ $p$,$r$ ($c$ -- ), similar to $\texttt{CALLXARGS}$, but pushes the old value of `cc` (along with the top $0\leq p\leq 15$ values from the original stack) into the stack of newly-invoked continuation $c$, setting `cc.nargs` to $-1\leq r\leq 14$.
- `DB38` — $\texttt{CALLXVARARGS}$ ($c$ $p$ $r$ -- ), similar to $\texttt{CALLXARGS}$, but takes $-1\leq p,r\leq254$ from the stack. The next three operations also take $p$ and $r$ from the stack, both in the range $-1\ldots254$.
- `DB39` — $\texttt{RETVARARGS}$ ($p$ $r$ -- ), similar to $\texttt{RETARGS}$.
- `DB3A` — $\texttt{JMPXVARARGS}$ ($c$ $p$ $r$ -- ), similar to $\texttt{JMPXARGS}$.
- `DB3B` — $\texttt{CALLCCVARARGS}$ ($c$ $p$ $r$ -- ), similar to $\texttt{CALLCCARGS}$.
- `DB3C` — $\texttt{CALLREF}$, equivalent to $\texttt{PUSHREFCONT}$; $\texttt{CALLX}$.
- `DB3D` — $\texttt{JMPREF}$, equivalent to $\texttt{PUSHREFCONT}$; $\texttt{JMPX}$.
- `DB3E` — $\texttt{JMPREFDATA}$, equivalent to $\texttt{PUSHREFCONT}$; $\texttt{JMPXDATA}$.
- `DB3F` — $\texttt{RETDATA}$, equivalent to $\texttt{PUSH c0}$; $\texttt{JMPXDATA}$. In this way, the remainder of the current continuation is converted into a *Slice* and returned to the caller.

### A.8.2. Conditional control flow primitives

- `DC` — $\texttt{IFRET}$ ($f$ -- ), performs a $\texttt{RET}$, but only if integer $f$ is non-zero. If $f$ is a $\texttt{NaN}$, throws an integer overflow exception.
- `DD` — $\texttt{IFNOTRET}$ ($f$ -- ), performs a $\texttt{RET}$, but only if integer $f$ is zero.
- `DE` — $\texttt{IF}$ ($f$ $c$ -- ), performs $\texttt{EXECUTE}$ for $c$ (i.e., *executes* $c$), but only if integer $f$ is non-zero. Otherwise simply discards both values.
- `DF` — $\texttt{IFNOT}$ ($f$ $c$ -- ), executes continuation $c$, but only if integer $f$ is zero. Otherwise simply discards both values.
- `E0` — $\texttt{IFJMP}$ ($f$ $c$ -- ), jumps to $c$ (similarly to $\texttt{JMPX}$), but only if $f$ is non-zero.
- `E1` — $\texttt{IFNOTJMP}$ ($f$ $c$ -- ), jumps to $c$ (similarly to $\texttt{JMPX}$), but only if $f$ is zero.
- `E2` — $\texttt{IFELSE}$ ($f$ $c$ $c'$ -- ), if integer $f$ is non-zero, executes $c$, otherwise executes $c'$. Equivalent to $\texttt{CONDSELCHK}$; $\texttt{EXECUTE}$.
- `E300` — $\texttt{IFREF}$ ($f$ -- ), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{IF}$, with the optimization that the cell reference is not actually loaded into a *Slice* and then converted into an ordinary *Continuation* if $f=0$.
- `E301` — $\texttt{IFNOTREF}$ ($f$ -- ), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{IFNOT}$.
- `E302` — $\texttt{IFJMPREF}$ ($f$ -- ), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{IFJMP}$.
- `E303` — $\texttt{IFNOTJMPREF}$ ($f$ -- ), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{IFNOTJMP}$.
- `E304` — $\texttt{CONDSEL}$ ($f$ $x$ $y$ -- $x$ or $y$), if integer $f$ is non-zero, returns $x$, otherwise returns $y$. Notice that no type checks are performed on $x$ and $y$.
- `E305` — $\texttt{CONDSELCHK}$ ($f$ $x$ $y$ -- $x$ or $y$), same as $\texttt{CONDSEL}$, but first checks whether $x$ and $y$ have the same type.
- `E308` — $\texttt{IFRETALT}$ ($f$ --), performs $\texttt{RETALT}$ if integer $f\neq0$.
- `E309` — $\texttt{IFNOTRETALT}$ ($f$ --), performs $\texttt{RETALT}$ if integer $f=0$.
- `E30D` — $\texttt{IFREFELSE}$ ($f$ $c$ --), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{SWAP}$; $\texttt{IFELSE}$.
- `E30E` — $\texttt{IFELSEREF}$ ($f$ $c$ --), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{IFELSE}$.
- `E30F` — $\texttt{IFREFELSEREF}$ ($f$ --), equivalent to $\texttt{PUSHREFCONT}$; $\texttt{PUSHREFCONT}$; $\texttt{IFELSE}$.
- `E39_` $n$ — $\texttt{IFBITJMP}$ $n$ ($x$ $c$ -- $x$), checks whether bit $0\leq n\leq 31$ is set in integer $x$, and if so, performs $\texttt{JMPX}$ to continuation $c$. Value $x$ is left in the stack.
- `E3B_` $n$ — $\texttt{IFNBITJMP}$ $n$ ($x$ $c$ -- $x$), jumps to $c$ if bit $0\leq n\leq 31$ is not set in integer $x$.
- `E3D_` $n$ — $\texttt{IFBITJMPREF}$ $n$ ($x$ -- $x$), performs a $\texttt{JMPREF}$ if bit $0\leq n\leq 31$ is set in integer $x$.
- `E3F_` $n$ — $\texttt{IFNBITJMPREF}$ $n$ ($x$ -- $x$), performs a $\texttt{JMPREF}$ if bit $0\leq n\leq 31$ is not set in integer $x$.

### A.8.3. Control flow primitives: loops

- `E4` — $\texttt{REPEAT}$ ($n$ $c$ -- ), executes continuation $c$ $n$ times, if integer $n$ is non-negative. If $n\geq2^{31}$ or $n<-2^{31}$, generates a range check exception.
- `E5` — $\texttt{REPEATEND}$ ($n$ -- ), similar to $\texttt{REPEAT}$, but it is applied to the current continuation `cc`.
- `E6` — $\texttt{UNTIL}$ ($c$ -- ), executes continuation $c$, then pops an integer $x$ from the resulting stack. If $x$ is zero, performs another iteration of this loop.
- `E7` — $\texttt{UNTILEND}$ ( -- ), similar to $\texttt{UNTIL}$, but executes the current continuation `cc` in a loop. When the loop exit condition is satisfied, performs a $\texttt{RET}$.
- `E8` — $\texttt{WHILE}$ ($c'$ $c$ -- ), executes $c'$ and pops an integer $x$ from the resulting stack. If $x$ is zero, exits the loop and transfers control to the original `cc`. If $x$ is non-zero, executes $c$, and then begins a new iteration.
- `E9` — $\texttt{WHILEEND}$ ($c'$ -- ), similar to $\texttt{WHILE}$, but uses the current continuation `cc` as the loop body.
- `EA` — $\texttt{AGAIN}$ ($c$ -- ), similar to $\texttt{REPEAT}$, but executes $c$ infinitely many times. A $\texttt{RET}$ only begins a new iteration of the infinite loop.
- `EB` — $\texttt{AGAINEND}$ ( -- ), similar to $\texttt{AGAIN}$, but performed with respect to the current continuation `cc`.
- `E314` — $\texttt{REPEATBRK}$ ($n$ $c$ -- ), similar to $\texttt{REPEAT}$, but also sets `c1` to the original `cc` after saving the old value of `c1` into the savelist of the original `cc`.
- `E315` — $\texttt{REPEATENDBRK}$ ($n$ -- ), similar to $\texttt{REPEATEND}$, but also sets `c1` to the original `c0` after saving the old value of `c1` into the savelist of the original `c0`.
- `E316` — $\texttt{UNTILBRK}$ ($c$ -- ), similar to $\texttt{UNTIL}$, but also modifies `c1` in the same way as $\texttt{REPEATBRK}$.
- `E317` — $\texttt{UNTILENDBRK}$ ( -- ), equivalent to $\texttt{SAMEALTSAVE}$; $\texttt{UNTILEND}$.
- `E318` — $\texttt{WHILEBRK}$ ($c'$ $c$ -- ), similar to $\texttt{WHILE}$, but also modifies `c1` in the same way as $\texttt{REPEATBRK}$.
- `E319` — $\texttt{WHILEENDBRK}$ ($c$ -- ), equivalent to $\texttt{SAMEALTSAVE}$; $\texttt{WHILEEND}$.
- `E31A` — $\texttt{AGAINBRK}$ ($c$ -- ), similar to $\texttt{AGAIN}$, but also modifies `c1` in the same way as $\texttt{REPEATBRK}$.
- `E31B` — $\texttt{AGAINENDBRK}$ ( -- ), equivalent to $\texttt{SAMEALTSAVE}$; $\texttt{AGAINEND}$.

### A.8.4. Manipulating the stack of continuations

- `EC` $rn$ — $\texttt{SETCONTARGS}$ $r$,$n$ ($x_1$ $x_2$...$x_r$ $c$ -- $c'$), similar to $\texttt{SETCONTARGS}$ $r$, but sets $c$.`nargs` to the final size of the stack of $c'$ plus $n$. In other words, transforms $c$ into a *closure* or a *partially applied function*, with $0\leq n\leq 14$ arguments missing.
- `EC0` $n$ — $\texttt{SETNUMARGS}$ $n$ or $\texttt{SETCONTARGS}$ $0$,$n$ ($c$ -- $c'$), sets $c$.`nargs` to $n$ plus the current depth of $c$'s stack, where $0\leq n\leq 14$. If $c$.`nargs` is already set to a non-negative value, does nothing.
- `EC` $r$ `F` — $\texttt{SETCONTARGS}$ $r$ or $\texttt{SETCONTARGS}$ $r$,$-1$ ($x_1$ $x_2$...$x_r$ $c$ -- $c'$), pushes $0\leq r\leq 15$ values $x_1\ldots x_r$ into the stack of (a copy of) the continuation $c$, starting with $x_1$. If the final depth of $c$'s stack turns out to be greater than $c$.`nargs`, a stack overflow exception is generated.
- `ED0` $p$ — $\texttt{RETURNARGS}$ $p$ ( -- ), leaves only the top $0\leq p\leq 15$ values in the current stack (somewhat similarly to $\texttt{ONLYTOPX}$), with all the unused bottom values not discarded, but saved into continuation `c0` in the same way as $\texttt{SETCONTARGS}$ does.
- `ED10` — $\texttt{RETURNVARARGS}$ ($p$ -- ), similar to $\texttt{RETURNARGS}$, but with Integer $0\leq p\leq 255$ taken from the stack.
- `ED11` — $\texttt{SETCONTVARARGS}$ ($x_1$ $x_2$...$x_r$ $c$ $r$ $n$ -- $c'$), similar to $\texttt{SETCONTARGS}$, but with $0\leq r\leq 255$ and $-1\leq n\leq 255$ taken from the stack.
- `ED12` — $\texttt{SETNUMVARARGS}$ ($c$ $n$ -- $c'$), where $-1\leq n\leq 255$. If $n=-1$, this operation does nothing ($c'=c$). Otherwise its action is similar to $\texttt{SETNUMARGS}$ $n$, but with $n$ taken from the stack.

### A.8.5. Creating simple continuations and closures

- `ED1E` — $\texttt{BLESS}$ ($s$ -- $c$), transforms a *Slice* $s$ into a simple ordinary continuation $c$, with $c$.`code`$=s$ and an empty stack and savelist.
- `ED1F` — $\texttt{BLESSVARARGS}$ ($x_1$...$x_r$ $s$ $r$ $n$ -- $c$), equivalent to $\texttt{ROT}$; $\texttt{BLESS}$; $\texttt{ROTREV}$; $\texttt{SETCONTVARARGS}$.
- `EE` $rn$ — $\texttt{BLESSARGS}$ $r,n$ ($x_1$...$x_r$ $s$ -- $c$), where $0\leq r\leq 15$, $-1\leq n\leq 14$, equivalent to $\texttt{BLESS}$; $\texttt{SETCONTARGS}$ $r,n$. The value of $n$ is represented inside the instruction by the 4-bit integer $n\bmod16$.
- `EE0` $n$ — $\texttt{BLESSNUMARGS}$ $n$ or $\texttt{BLESSARGS 0,}$ $n$ ($s$ -- $c$), also transforms a *Slice* $s$ into a *Continuation* $c$, but sets $c$.`nargs` to $0\leq n\leq 14$.

### A.8.6. Operations with continuation savelists and control registers

- `ED4` $i$ — $\texttt{PUSH c}(i)$ or $\texttt{PUSHCTR c}(i)$ ( -- $x$), pushes the current value of control register `c`$(i)$. If the control register is not supported in the current codepage, or if it does not have a value, an exception is triggered.
- `ED44` — $\texttt{PUSH c4}$ or $\texttt{PUSHROOT}$, pushes the "global data root" cell reference, thus enabling access to persistent smart-contract data.
- `ED5` $i$ — $\texttt{POP c}(i)$ or $\texttt{POPCTR c}(i)$ ($x$ -- ), pops a value $x$ from the stack and stores it into control register `c`$(i)$, if supported in the current codepage. Notice that if a control register accepts only values of a specific type, a type-checking exception may occur.
- `ED54` — $\texttt{POP c4}$ or $\texttt{POPROOT}$, sets the "global data root" cell reference, thus allowing modification of persistent smart-contract data.
- `ED6` $i$ — $\texttt{SETCONT c}(i)$ or $\texttt{SETCONTCTR c}(i)$ ($x$ $c$ -- $c'$), stores $x$ into the savelist of continuation $c$ as `c`$(i)$, and returns the resulting continuation $c'$. Almost all operations with continuations may be expressed in terms of $\texttt{SETCONTCTR}$, $\texttt{POPCTR}$, and $\texttt{PUSHCTR}$.
- `ED7` $i$ — $\texttt{SETRETCTR c}(i)$ ($x$ -- ), equivalent to $\texttt{PUSH c0}$; $\texttt{SETCONTCTR c}(i)$; $\texttt{POP c0}$.
- `ED8` $i$ — $\texttt{SETALTCTR c}(i)$ ($x$ -- ), equivalent to $\texttt{PUSH c1}$; $\texttt{SETCONTCTR c}(i)$; $\texttt{POP c0}$.
- `ED9` $i$ — $\texttt{POPSAVE c}(i)$ or $\texttt{POPCTRSAVE c}(i)$ ($x$ --), similar to $\texttt{POP c}(i)$, but also saves the old value of `c`$(i)$ into continuation `c0`. Equivalent (up to exceptions) to $\texttt{SAVECTR c}(i)$; $\texttt{POP c}(i)$.
- `EDA` $i$ — $\texttt{SAVE c}(i)$ or $\texttt{SAVECTR c}(i)$ ( -- ), saves the current value of `c`$(i)$ into the savelist of continuation `c0`. If an entry for `c`$(i)$ is already present in the savelist of `c0`, nothing is done. Equivalent to $\texttt{PUSH c}(i)$; $\texttt{SETRETCTR}$ $c(i)$.
- `EDB` $i$ — $\texttt{SAVEALT c}(i)$ or $\texttt{SAVEALTCTR c}(i)$ ( -- ), similar to $\texttt{SAVE c}(i)$, but saves the current value of `c`$(i)$ into the savelist of `c1`, not `c0`.
- `EDC` $i$ — $\texttt{SAVEBOTH c}(i)$ or $\texttt{SAVEBOTHCTR c}(i)$ ( -- ), equivalent to $\texttt{DUP}$; $\texttt{SAVE c}(i)$; $\texttt{SAVEALT c}(i)$.
- `EDE0` — $\texttt{PUSHCTRX}$ ($i$ -- $x$), similar to $\texttt{PUSHCTR c}(i)$, but with $i$, $0\leq i\leq255$, taken from the stack. Notice that this primitive is one of the few "exotic" primitives, which are not polymorphic like stack manipulation primitives, and at the same time do not have well-defined types of parameters and return values, because the type of $x$ depends on $i$.
- `EDE1` — $\texttt{POPCTRX}$ ($x$ $i$ -- ), similar to $\texttt{POPCTR c}(i)$, but with $0\leq i\leq255$ from the stack.
- `EDE2` — $\texttt{SETCONTCTRX}$ ($x$ $c$ $i$ -- $c'$), similar to $\texttt{SETCONTCTR c}(i)$, but with $0\leq i\leq 255$ from the stack.
- `EDF0` — $\texttt{COMPOS}$ or $\texttt{BOOLAND}$ ($c$ $c'$ -- $c''$), computes the composition $c\circ_0c'$, which has the meaning of "perform $c$, and, if successful, perform $c'$" (if $c$ is a boolean circuit) or simply "perform $c$, then $c'$". Equivalent to $\texttt{SWAP}$; $\texttt{SETCONT c0}$.
- `EDF1` — $\texttt{COMPOSALT}$ or $\texttt{BOOLOR}$ ($c$ $c'$ -- $c''$), computes the alternative composition $c\circ_1 c'$, which has the meaning of "perform $c$, and, if not successful, perform $c'$" (if $c$ is a boolean circuit). Equivalent to $\texttt{SWAP}$; $\texttt{SETCONT c1}$.
- `EDF2` — $\texttt{COMPOSBOTH}$ ($c$ $c'$ -- $c''$), computes $(c\circ_0c')\circ_1c'$, which has the meaning of "compute boolean circuit $c$, then compute $c'$, regardless of the result of $c$".
- `EDF3` — $\texttt{ATEXIT}$ ($c$ -- ), sets `c0`$\leftarrow c\circ_0$`c0`. In other words, $c$ will be executed before exiting current subroutine.
- `EDF4` — $\texttt{ATEXITALT}$ ($c$ -- ), sets `c1`$\leftarrow c\circ_1$`c1`. In other words, $c$ will be executed before exiting current subroutine by its alternative return path.
- `EDF5` — $\texttt{SETEXITALT}$ ($c$ -- ), sets `c1`$\leftarrow (c\circ_0$`c0`$)\circ_1$`c1`. In this way, a subsequent $\texttt{RETALT}$ will first execute $c$, then transfer control to the original `c0`. This can be used, for instance, to exit from nested loops.
- `EDF6` — $\texttt{THENRET}$ ($c$ -- $c'$), computes $c':=c\circ_0$`c0`
- `EDF7` — $\texttt{THENRETALT}$ ($c$ -- $c'$), computes $c':=c\circ_0$`c1`
- `EDF8` — $\texttt{INVERT}$ ( -- ), interchanges `c0` and `c1`.
- `EDF9` — $\texttt{BOOLEVAL}$ ($c$ -- $?$), performs `cc`$\leftarrow \bigl(c\circ_0(($`PUSH`$\,-1)\circ_0$`cc`$)\bigr)\circ_1(($`PUSH`$\,0)\circ_0$`cc`$)$. If $c$ represents a boolean circuit, the net effect is to evaluate it and push either $-1$ or $0$ into the stack before continuing.
- `EDFA` — $\texttt{SAMEALT}$ ( -- ), sets $c_1:=c_0$. Equivalent to $\texttt{PUSH c0}$; $\texttt{POP c1}$.
- `EDFB` — $\texttt{SAMEALTSAVE}$ ( -- ), sets $c_1:=c_0$, but first saves the old value of $c_1$ into the savelist of $c_0$. Equivalent to $\texttt{SAVE c1}$; $\texttt{SAMEALT}$.

### A.8.7. Dictionary subroutine calls and jumps

- `F0` $n$ — $\texttt{CALL}$ $n$ or $\texttt{CALLDICT}$ $n$ ( -- $n$), calls the continuation in `c3`, pushing integer $0\leq n\leq 255$ into its stack as an argument. Approximately equivalent to $\texttt{PUSHINT}$ $n$; $\texttt{PUSH c3}$; $\texttt{EXECUTE}$.
- `F12_` $n$ — $\texttt{CALL}$ $n$ for $0\leq n<2^{14}$ ( -- $n$), an encoding of $\texttt{CALL}$ $n$ for larger values of $n$.
- `F16_` $n$ — $\texttt{JMP}$ $n$ or $\texttt{JMPDICT}$ $n$ ( -- $n$), jumps to the continuation in `c3`, pushing integer $0\leq n<2^{14}$ as its argument. Approximately equivalent to $\texttt{PUSHINT}$ $n$; $\texttt{PUSH c3}$; $\texttt{JMPX}$.
- `F1A_` $n$ — $\texttt{PREPARE}$ $n$ or $\texttt{PREPAREDICT}$ $n$ ( -- $n$ $c$), equivalent to $\texttt{PUSHINT}$ $n$; $\texttt{PUSH c3}$, for $0\leq n<2^{14}$. In this way, $\texttt{CALL}$ $n$ is approximately equivalent to $\texttt{PREPARE}$ $n$; $\texttt{EXECUTE}$, and $\texttt{JMP}$ $n$ is approximately equivalent to $\texttt{PREPARE}$ $n$; $\texttt{JMPX}$. One might use, for instance, $\texttt{CALLARGS}$ or $\texttt{CALLCC}$ instead of $\texttt{EXECUTE}$ here.

## A.9  Exception generating and handling primitives

### A.9.1. Throwing exceptions

- `F22_` $nn$ — $\texttt{THROW}$ $nn$ ( -- $0$ $nn$), throws exception $0\leq nn\leq 63$ with parameter zero. In other words, it transfers control to the continuation in `c2`, pushing $0$ and $nn$ into its stack, and discarding the old stack altogether.
- `F26_` $nn$ — $\texttt{THROWIF}$ $nn$ ($f$ -- ), throws exception $0\leq nn\leq 63$ with parameter zero only if integer $f\neq0$.
- `F2A_` $nn$ — $\texttt{THROWIFNOT}$ $nn$ ($f$ -- ), throws exception $0\leq nn\leq 63$ with parameter zero only if integer $f=0$.
- `F2C4_` $nn$ — $\texttt{THROW}$ $nn$ for $0\leq nn<2^{11}$, an encoding of $\texttt{THROW}$ $nn$ for larger values of $nn$.
- `F2CC_` $nn$ — $\texttt{THROWARG}$ $nn$ ($x$ -- $x$ $nn$), throws exception $0\leq nn<2^{11}$ with parameter $x$, by copying $x$ and $nn$ into the stack of `c2` and transferring control to `c2`.
- `F2D4_` $nn$ — $\texttt{THROWIF}$ $nn$ ($f$ -- ) for $0\leq nn<2^{11}$.
- `F2DC_` $nn$ — $\texttt{THROWARGIF}$ $nn$ ($x$ $f$ -- ), throws exception $0\leq nn<2^{11}$ with parameter $x$ only if integer $f\neq0$.
- `F2E4_` $nn$ — $\texttt{THROWIFNOT}$ $nn$ ($f$ -- ) for $0\leq nn<2^{11}$.
- `F2EC_` $nn$ — $\texttt{THROWARGIFNOT}$ $nn$ ($x$ $f$ -- ), throws exception $0\leq nn<2^{11}$ with parameter $x$ only if integer $f=0$.
- `F2F0` — $\texttt{THROWANY}$ ($n$ -- $0$ $n$), throws exception $0\leq n<2^{16}$ with parameter zero. Approximately equivalent to $\texttt{PUSHINT 0}$; $\texttt{SWAP}$; $\texttt{THROWARGANY}$.
- `F2F1` — $\texttt{THROWARGANY}$ ($x$ $n$ -- $x$ $n$), throws exception $0\leq n<2^{16}$ with parameter $x$, transferring control to the continuation in `c2`. Approximately equivalent to $\texttt{PUSH c2}$; $\texttt{JMPXARGS 2}$.
- `F2F2` — $\texttt{THROWANYIF}$ ($n$ $f$ -- ), throws exception $0\leq n<2^{16}$ with parameter zero only if $f\neq0$.
- `F2F3` — $\texttt{THROWARGANYIF}$ ($x$ $n$ $f$ -- ), throws exception $0\leq n<2^{16}$ with parameter $x$ only if $f\neq0$.
- `F2F4` — $\texttt{THROWANYIFNOT}$ ($n$ $f$ -- ), throws exception $0\leq n<2^{16}$ with parameter zero only if $f=0$.
- `F2F5` — $\texttt{THROWARGANYIFNOT}$ ($x$ $n$ $f$ -- ), throws exception $0\leq n<2^{16}$ with parameter $x$ only if $f=0$.

### A.9.2. Catching and handling exceptions

- `F2FF` — $\texttt{TRY}$ ($c$ $c'$ -- ), sets `c2` to $c'$, first saving the old value of `c2` both into the savelist of $c'$ and into the savelist of the current continuation, which is stored into $c$.`c0` and $c'$.`c0`. Then runs $c$ similarly to $\texttt{EXECUTE}$. If $c$ does not throw any exceptions, the original value of `c2` is automatically restored on return from $c$. If an exception occurs, the execution is transferred to $c'$, but the original value of `c2` is restored in the process, so that $c'$ can re-throw the exception by $\texttt{THROWANY}$ if it cannot handle it by itself.
- `F3` $pr$ — $\texttt{TRYARGS}$ $p$,$r$ ($c$ $c'$ -- ), similar to $\texttt{TRY}$, but with $\texttt{CALLARGS}$ $p$,$r$ internally used instead of $\texttt{EXECUTE}$. In this way, all but the top $0\leq p\leq 15$ stack elements will be saved into current continuation's stack, and then restored upon return from either $c$ or $c'$, with the top $0\leq r\leq 15$ values of the resulting stack of $c$ or $c'$ copied as return values.

---

## A.10  Dictionary manipulation primitives

TVM's dictionary support is discussed at length in [3.3](#3-3-hashmaps-or-dictionaries). The basic operations with dictionaries are listed in [3.3.10](#3-3-10-basic-dictionary-operations), while the taxonomy of dictionary manipulation primitives is provided in [3.3.8](#3-3-8-taxonomy-of-dictionary-primitives). Here we use the concepts and notation introduced in those sections.
...
- `6D` — $\texttt{NEWDICT}$ ( -- $D$), returns a new empty dictionary. It is an alternative mnemonics for $\texttt{PUSHNULL}$, cf. [A.3.1](#a-3-1-null-primitives).
- `6E` — $\texttt{DICTEMPTY}$ ($D$ -- $?$), checks whether dictionary $D$ is empty, and returns $-1$ or $0$ accordingly. It is an alternative mnemonics for $\texttt{ISNULL}$, cf. [A.3.1](#a-3-1-null-primitives).

### A.10.2. Dictionary serialization and deserialization

- `CE` — $\texttt{STDICTS}$ ($s$ $b$ -- $b'$), stores a Slice-represented dictionary $s$ into Builder $b$. It is actually a synonym for $\texttt{STSLICE}$.
- `F400` — $\texttt{STDICT}$ or $\texttt{STOPTREF}$ ($D$ $b$ -- $b'$), stores dictionary $D$ into Builder $b$, returning the resulting Builder $b'$. In other words, if $D$ is a cell, performs $\texttt{STONE}$ and $\texttt{STREF}$; if $D$ is Null, performs $\texttt{NIP}$ and $\texttt{STZERO}$; otherwise throws a type checking exception.
- `F401` — $\texttt{SKIPDICT}$ or $\texttt{SKIPOPTREF}$ ($s$ -- $s'$), equivalent to $\texttt{LDDICT}$; $\texttt{NIP}$.
- `F402` — $\texttt{LDDICTS}$ ($s$ -- $s'$ $s''$), loads (parses) a (Slice-represented) dictionary $s'$ from Slice $s$, and returns the remainder of $s$ as $s''$. This is a "split function" for all $\mathit{HashmapE}(n,X)$ dictionary types.
- `F403` — $\texttt{PLDDICTS}$ ($s$ -- $s'$), preloads a (Slice-represented) dictionary $s'$ from Slice $s$. Approximately equivalent to $\texttt{LDDICTS}$; $\texttt{DROP}$.
- `F404` — $\texttt{LDDICT}$ or $\texttt{LDOPTREF}$ ($s$ -- $D$ $s'$), loads (parses) a dictionary $D$ from Slice $s$, and returns the remainder of $s$ as $s'$. May be applied to dictionaries or to values of arbitrary $(^Y)^?$ types.
- `F405` — $\texttt{PLDDICT}$ or $\texttt{PLDOPTREF}$ ($s$ -- $D$), preloads a dictionary $D$ from Slice $s$. Approximately equivalent to $\texttt{LDDICT}$; $\texttt{DROP}$.
- `F406` — $\texttt{LDDICTQ}$ ($s$ -- $D$ $s'$ $-1$ or $s$ $0$), a quiet version of $\texttt{LDDICT}$.
- `F407` — $\texttt{PLDDICTQ}$ ($s$ -- $D$ $-1$ or $0$), a quiet version of $\texttt{PLDDICT}$.

### A.10.3. Get dictionary operations

- `F40A` — $\texttt{DICTGET}$ ($k$ $D$ $n$ -- $x$ $-1$ or $0$), looks up key $k$ (represented by a Slice, the first $0\leq n\leq 1023$ data bits of which are used as a key) in dictionary $D$ of type $\mathit{HashmapE}(n,X)$ with $n$-bit keys. On success, returns the value found as a Slice $x$.
- `F40B` — $\texttt{DICTGETREF}$ ($k$ $D$ $n$ -- $c$ $-1$ or $0$), similar to $\texttt{DICTGET}$, but with a $\texttt{LDREF}$; $\texttt{ENDS}$ applied to $x$ on success. This operation is useful for dictionaries of type $\mathit{HashmapE}(n,^Y)$.
- `F40C` — $\texttt{DICTIGET}$ ($i$ $D$ $n$ -- $x$ $-1$ or $0$), similar to $\texttt{DICTGET}$, but with a signed (big-endian) $n$-bit Integer $i$ as a key. If $i$ does not fit into $n$ bits, returns $0$. If $i$ is a $\texttt{NaN}$, throws an integer overflow exception.
- `F40D` — $\texttt{DICTIGETREF}$ ($i$ $D$ $n$ -- $c$ $-1$ or $0$), combines $\texttt{DICTIGET}$ with $\texttt{DICTGETREF}$: it uses signed $n$-bit Integer $i$ as a key and returns a Cell instead of a Slice on success.
- `F40E` — $\texttt{DICTUGET}$ ($i$ $D$ $n$ -- $x$ $-1$ or $0$), similar to $\texttt{DICTIGET}$, but with unsigned (big-endian) $n$-bit Integer $i$ used as a key.
- `F40F` — $\texttt{DICTUGETREF}$ ($i$ $D$ $n$ -- $c$ $-1$ or $0$), similar to $\texttt{DICTIGETREF}$, but with an unsigned $n$-bit Integer key $i$.

### A.10.4. Set/Replace/Add dictionary operations

The mnemonics of the following dictionary primitives are constructed in a systematic fashion according to the regular expression `DICT`[`I`,`U`](`SET`,`REPLACE`,`ADD`)[`GET`][`REF`] depending on the type of the key used (a *Slice* or a signed or unsigned *Integer*), the dictionary operation to be performed, and the way the values are accepted and returned (as *Cell*s or as *Slice*s). Therefore, we provide a detailed description only for some primitives, assuming that this information is sufficient for the reader to understand the precise action of the remaining primitives.

- `F412` — $\texttt{DICTSET}$ ($x$ $k$ $D$ $n$ -- $D'$), sets the value associated with $n$-bit key $k$ (represented by a Slice as in $\texttt{DICTGET}$) in dictionary $D$ (also represented by a Slice) to value $x$ (again a Slice), and returns the resulting dictionary as $D'$.
- `F413` — $\texttt{DICTSETREF}$ ($c$ $k$ $D$ $n$ -- $D'$), similar to $\texttt{DICTSET}$, but with the value set to a reference to Cell $c$.
- `F414` — $\texttt{DICTISET}$ ($x$ $i$ $D$ $n$ -- $D'$), similar to $\texttt{DICTSET}$, but with the key represented by a (big-endian) signed $n$-bit integer $i$. If $i$ does not fit into $n$ bits, a range check exception is generated.
- `F415` — $\texttt{DICTISETREF}$ ($c$ $i$ $D$ $n$ -- $D'$), similar to $\texttt{DICTSETREF}$, but with the key a signed $n$-bit integer as in $\texttt{DICTISET}$.
- `F416` — $\texttt{DICTUSET}$ ($x$ $i$ $D$ $n$ -- $D'$), similar to $\texttt{DICTISET}$, but with $i$ an unsigned $n$-bit integer.
- `F417` — $\texttt{DICTUSETREF}$ ($c$ $i$ $D$ $n$ -- $D'$), similar to $\texttt{DICTISETREF}$, but with $i$ unsigned.
- `F41A` — $\texttt{DICTSETGET}$ ($x$ $k$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$), combines $\texttt{DICTSET}$ with $\texttt{DICTGET}$: it sets the value corresponding to key $k$ to $x$, but also returns the old value $y$ associated with the key in question, if present.
- `F41B` — $\texttt{DICTSETGETREF}$ ($c$ $k$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D'$ $0$), combines $\texttt{DICTSETREF}$ with $\texttt{DICTGETREF}$ similarly to $\texttt{DICTSETGET}$.
- `F41C` — $\texttt{DICTISETGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$), similar to $\texttt{DICTSETGET}$, but with the key represented by a big-endian signed $n$-bit Integer $i$.
- `F41D` — $\texttt{DICTISETGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D'$ $0$), a version of $\texttt{DICTSETGETREF}$ with signed Integer $i$ as a key.
- `F41E` — $\texttt{DICTUSETGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$), similar to $\texttt{DICTISETGET}$, but with $i$ an unsigned $n$-bit integer.
- `F41F` — $\texttt{DICTUSETGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D'$ $0$).
- `F422` — $\texttt{DICTREPLACE}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$), a Replace operation, which is similar to $\texttt{DICTSET}$, but sets the value of key $k$ in dictionary $D$ to $x$ only if the key $k$ was already present in $D$.
- `F423` — $\texttt{DICTREPLACEREF}$ ($c$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$), a Replace counterpart of $\texttt{DICTSETREF}$.
- `F424` — $\texttt{DICTIREPLACE}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$), a version of $\texttt{DICTREPLACE}$ with signed $n$-bit Integer $i$ used as a key.
- `F425` — $\texttt{DICTIREPLACEREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F426` — $\texttt{DICTUREPLACE}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F427` — $\texttt{DICTUREPLACEREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F42A` — $\texttt{DICTREPLACEGET}$ ($x$ $k$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$), a Replace counterpart of $\texttt{DICTSETGET}$: on success, also returns the old value associated with the key in question.
- `F42B` — $\texttt{DICTREPLACEGETREF}$ ($c$ $k$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D$ $0$).
- `F42C` — $\texttt{DICTIREPLACEGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$).
- `F42D` — $\texttt{DICTIREPLACEGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D$ $0$).
- `F42E` — $\texttt{DICTUREPLACEGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$).
- `F42F` — $\texttt{DICTUREPLACEGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $c'$ $-1$ or $D$ $0$).
- `F432` — $\texttt{DICTADD}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$), an Add counterpart of $\texttt{DICTSET}$: sets the value associated with key $k$ in dictionary $D$ to $x$, but only if it is not already present in $D$.
- `F433` — $\texttt{DICTADDREF}$ ($c$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F434` — $\texttt{DICTIADD}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F435` — $\texttt{DICTIADDREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F436` — $\texttt{DICTUADD}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F437` — $\texttt{DICTUADDREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F43A` — $\texttt{DICTADDGET}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$), an Add counterpart of $\texttt{DICTSETGET}$: sets the value associated with key $k$ in dictionary $D$ to $x$, but only if key $k$ is not already present in $D$. Otherwise, just returns the old value $y$ without changing the dictionary.
- `F43B` — $\texttt{DICTADDGETREF}$ ($c$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $c'$ $0$), an Add counterpart of $\texttt{DICTSETGETREF}$.
- `F43C` — $\texttt{DICTIADDGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$).
- `F43D` — $\texttt{DICTIADDGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $c'$ $0$).
- `F43E` — $\texttt{DICTUADDGET}$ ($x$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$).
- `F43F` — $\texttt{DICTUADDGETREF}$ ($c$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $c'$ $0$).

### A.10.5. Builder-accepting variants of Set dictionary operations

The following primitives accept the new value as a Builder $b$ instead of a Slice $x$, which often is more convenient if the value needs to be serialized from several components computed in the stack. (This is reflected by appending a `B` to the mnemonics of the corresponding Set primitives that work with Slices.) The net effect is roughly equivalent to converting $b$ into a Slice by $\texttt{ENDC}$; $\texttt{CTOS}$ and executing the corresponding primitive listed in [A.10.4](#a-10-4-set-replace-add-dictionary-operations).

- `F441` — $\texttt{DICTSETB}$ ($b$ $k$ $D$ $n$ -- $D'$).
- `F442` — $\texttt{DICTISETB}$ ($b$ $i$ $D$ $n$ -- $D'$).
- `F443` — $\texttt{DICTUSETB}$ ($b$ $i$ $D$ $n$ -- $D'$).
- `F445` — $\texttt{DICTSETGETB}$ ($b$ $k$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$).
- `F446` — $\texttt{DICTISETGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$).
- `F447` — $\texttt{DICTUSETGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D'$ $0$).
- `F449` — $\texttt{DICTREPLACEB}$ ($b$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F44A` — $\texttt{DICTIREPLACEB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F44B` — $\texttt{DICTUREPLACEB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F44D` — $\texttt{DICTREPLACEGETB}$ ($b$ $k$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$).
- `F44E` — $\texttt{DICTIREPLACEGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$).
- `F44F` — $\texttt{DICTUREPLACEGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $y$ $-1$ or $D$ $0$).
- `F451` — $\texttt{DICTADDB}$ ($b$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F452` — $\texttt{DICTIADDB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F453` — $\texttt{DICTUADDB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F455` — $\texttt{DICTADDGETB}$ ($b$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$).
- `F456` — $\texttt{DICTIADDGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$).
- `F457` — $\texttt{DICTUADDGETB}$ ($b$ $i$ $D$ $n$ -- $D'$ $-1$ or $D$ $y$ $0$).

### A.10.6. Delete dictionary operations

- `F459` — $\texttt{DICTDEL}$ ($k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$), deletes $n$-bit key, represented by a Slice $k$, from dictionary $D$. If the key is present, returns the modified dictionary $D'$ and the success flag $-1$. Otherwise, returns the original dictionary $D$ and $0$.
- `F45A` — $\texttt{DICTIDEL}$ ($i$ $D$ $n$ -- $D'$ $?$), a version of $\texttt{DICTDEL}$ with the key represented by a signed $n$-bit Integer $i$. If $i$ does not fit into $n$ bits, simply returns $D$ $0$ ("key not found, dictionary unmodified").
- `F45B` — $\texttt{DICTUDEL}$ ($i$ $D$ $n$ -- $D'$ $?$), similar to $\texttt{DICTIDEL}$, but with $i$ an unsigned $n$-bit integer.
- `F462` — $\texttt{DICTDELGET}$ ($k$ $D$ $n$ -- $D'$ $x$ $-1$ or $D$ $0$), deletes $n$-bit key, represented by a Slice $k$, from dictionary $D$. If the key is present, returns the modified dictionary $D'$, the original value $x$ associated with the key $k$ (represented by a Slice), and the success flag $-1$. Otherwise, returns the original dictionary $D$ and $0$.
- `F463` — $\texttt{DICTDELGETREF}$ ($k$ $D$ $n$ -- $D'$ $c$ $-1$ or $D$ $0$), similar to $\texttt{DICTDELGET}$, but with $\texttt{LDREF}$; $\texttt{ENDS}$ applied to $x$ on success, so that the value returned $c$ is a Cell.
- `F464` — $\texttt{DICTIDELGET}$ ($i$ $D$ $n$ -- $D'$ $x$ $-1$ or $D$ $0$), a variant of primitive $\texttt{DICTDELGET}$ with signed $n$-bit integer $i$ as a key.
- `F465` — $\texttt{DICTIDELGETREF}$ ($i$ $D$ $n$ -- $D'$ $c$ $-1$ or $D$ $0$), a variant of primitive $\texttt{DICTIDELGET}$ returning a Cell instead of a Slice.
- `F466` — $\texttt{DICTUDELGET}$ ($i$ $D$ $n$ -- $D'$ $x$ $-1$ or $D$ $0$), a variant of primitive $\texttt{DICTDELGET}$ with unsigned $n$-bit integer $i$ as a key.
- `F467` — $\texttt{DICTUDELGETREF}$ ($i$ $D$ $n$ -- $D'$ $c$ $-1$ or $D$ $0$), a variant of primitive $\texttt{DICTUDELGET}$ returning a Cell instead of a Slice.
...
- `F469` — $\texttt{DICTGETOPTREF}$ ($k$ $D$ $n$ -- $c^?$), a variant of $\texttt{DICTGETREF}$ that returns Null instead of the value $c^?$ if the key $k$ is absent from dictionary $D$.
- `F46A` — $\texttt{DICTIGETOPTREF}$ ($i$ $D$ $n$ -- $c^?$), similar to $\texttt{DICTGETOPTREF}$, but with the key given by signed $n$-bit Integer $i$. If the key $i$ is out of range, also returns Null.
- `F46B` — $\texttt{DICTUGETOPTREF}$ ($i$ $D$ $n$ -- $c^?$), similar to $\texttt{DICTGETOPTREF}$, but with the key given by unsigned $n$-bit Integer $i$.
- `F46D` — $\texttt{DICTSETGETOPTREF}$ ($c^?$ $k$ $D$ $n$ -- $D'$ $\tilde{c}^?$), a variant of both $\texttt{DICTGETOPTREF}$ and $\texttt{DICTSETGETREF}$ that sets the value corresponding to key $k$ in dictionary $D$ to $c^?$ (if $c^?$ is Null, then the key is deleted instead), and returns the old value $\tilde{c}^?$ (if the key $k$ was absent before, returns Null instead).
- `F46E` — $\texttt{DICTISETGETOPTREF}$ ($c^?$ $i$ $D$ $n$ -- $D'$ $\tilde{c}^?$), similar to primitive $\texttt{DICTSETGETOPTREF}$, but using signed $n$-bit Integer $i$ as a key. If $i$ does not fit into $n$ bits, throws a range checking exception.
- `F46F` — $\texttt{DICTUSETGETOPTREF}$ ($c^?$ $i$ $D$ $n$ -- $D'$ $\tilde{c}^?$), similar to primitive $\texttt{DICTSETGETOPTREF}$, but using unsigned $n$-bit Integer $i$ as a key.
...
- `F470` — $\texttt{PFXDICTSET}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F471` — $\texttt{PFXDICTREPLACE}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F472` — $\texttt{PFXDICTADD}$ ($x$ $k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
- `F473` — $\texttt{PFXDICTDEL}$ ($k$ $D$ $n$ -- $D'$ $-1$ or $D$ $0$).
...
- `F474` — $\texttt{DICTGETNEXT}$ ($k$ $D$ $n$ -- $x'$ $k'$ $-1$ or $0$), computes the minimal key $k'$ in dictionary $D$ that is lexicographically greater than $k$, and returns $k'$ (represented by a Slice) along with associated value $x'$ (also represented by a Slice).
- `F475` — $\texttt{DICTGETNEXTEQ}$ ($k$ $D$ $n$ -- $x'$ $k'$ $-1$ or $0$), similar to $\texttt{DICTGETNEXT}$, but computes the minimal key $k'$ that is lexicographically greater than or equal to $k$.
- `F476` — $\texttt{DICTGETPREV}$ ($k$ $D$ $n$ -- $x'$ $k'$ $-1$ or $0$), similar to $\texttt{DICTGETNEXT}$, but computes the maximal key $k'$ lexicographically smaller than $k$.
- `F477` — $\texttt{DICTGETPREVEQ}$ ($k$ $D$ $n$ -- $x'$ $k'$ $-1$ or $0$), similar to $\texttt{DICTGETPREV}$, but computes the maximal key $k'$ lexicographically smaller than or equal to $k$.
- `F478` — $\texttt{DICTIGETNEXT}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$), similar to $\texttt{DICTGETNEXT}$, but interprets all keys in dictionary $D$ as big-endian signed $n$-bit integers, and computes the minimal key $i'$ that is larger than Integer $i$ (which does not necessarily fit into $n$ bits).
- `F479` — $\texttt{DICTIGETNEXTEQ}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).
- `F47A` — $\texttt{DICTIGETPREV}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).
- `F47B` — $\texttt{DICTIGETPREVEQ}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).
- `F47C` — $\texttt{DICTUGETNEXT}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$), similar to $\texttt{DICTGETNEXT}$, but interprets all keys in dictionary $D$ as big-endian unsigned $n$-bit integers, and computes the minimal key $i'$ that is larger than Integer $i$ (which does not necessarily fit into $n$ bits, and is not necessarily non-negative).
- `F47D` — $\texttt{DICTUGETNEXTEQ}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).
- `F47E` — $\texttt{DICTUGETPREV}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).
- `F47F` — $\texttt{DICTUGETPREVEQ}$ ($i$ $D$ $n$ -- $x'$ $i'$ $-1$ or $0$).

### A.10.10. GetMin, GetMax, RemoveMin, RemoveMax operations

- `F482` — $\texttt{DICTMIN}$ ($D$ $n$ -- $x$ $k$ $-1$ or $0$), computes the minimal key $k$ (represented by a Slice with $n$ data bits) in dictionary $D$, and returns $k$ along with the associated value $x$.
- `F483` — $\texttt{DICTMINREF}$ ($D$ $n$ -- $c$ $k$ $-1$ or $0$), similar to $\texttt{DICTMIN}$, but returns the only reference in the value as a Cell $c$.
- `F484` — $\texttt{DICTIMIN}$ ($D$ $n$ -- $x$ $i$ $-1$ or $0$), somewhat similar to $\texttt{DICTMIN}$, but computes the minimal key $i$ under the assumption that all keys are big-endian signed $n$-bit integers. Notice that the key and value returned may differ from those computed by $\texttt{DICTMIN}$ and $\texttt{DICTUMIN}$.
- `F485` — $\texttt{DICTIMINREF}$ ($D$ $n$ -- $c$ $i$ $-1$ or $0$).
- `F486` — $\texttt{DICTUMIN}$ ($D$ $n$ -- $x$ $i$ $-1$ or $0$), similar to $\texttt{DICTMIN}$, but returns the key as an unsigned $n$-bit Integer $i$.
- `F487` — $\texttt{DICTUMINREF}$ ($D$ $n$ -- $c$ $i$ $-1$ or $0$).
- `F48A` — $\texttt{DICTMAX}$ ($D$ $n$ -- $x$ $k$ $-1$ or $0$), computes the maximal key $k$ (represented by a Slice with $n$ data bits) in dictionary $D$, and returns $k$ along with the associated value $x$.
- `F48B` — $\texttt{DICTMAXREF}$ ($D$ $n$ -- $c$ $k$ $-1$ or $0$).
- `F48C` — $\texttt{DICTIMAX}$ ($D$ $n$ -- $x$ $i$ $-1$ or $0$).
- `F48D` — $\texttt{DICTIMAXREF}$ ($D$ $n$ -- $c$ $i$ $-1$ or $0$).
- `F48E` — $\texttt{DICTUMAX}$ ($D$ $n$ -- $x$ $i$ $-1$ or $0$).
- `F48F` — $\texttt{DICTUMAXREF}$ ($D$ $n$ -- $c$ $i$ $-1$ or $0$).
- `F492` — $\texttt{DICTREMMIN}$ ($D$ $n$ -- $D'$ $x$ $k$ $-1$ or $D$ $0$), computes the minimal key $k$ (represented by a Slice with $n$ data bits) in dictionary $D$, removes $k$ from the dictionary, and returns $k$ along with the associated value $x$ and the modified dictionary $D'$.
- `F493` — $\texttt{DICTREMMINREF}$ ($D$ $n$ -- $D'$ $c$ $k$ $-1$ or $D$ $0$), similar to $\texttt{DICTREMMIN}$, but returns the only reference in the value as a Cell $c$.
- `F494` — $\texttt{DICTIREMMIN}$ ($D$ $n$ -- $D'$ $x$ $i$ $-1$ or $D$ $0$), somewhat similar to $\texttt{DICTREMMIN}$, but computes the minimal key $i$ under the assumption that all keys are big-endian signed $n$-bit integers. Notice that the key and value returned may differ from those computed by $\texttt{DICTREMMIN}$ and $\texttt{DICTUREMMIN}$.
- `F495` — $\texttt{DICTIREMMINREF}$ ($D$ $n$ -- $D'$ $c$ $i$ $-1$ or $D$ $0$).
- `F496` — $\texttt{DICTUREMMIN}$ ($D$ $n$ -- $D'$ $x$ $i$ $-1$ or $D$ $0$), similar to $\texttt{DICTREMMIN}$, but returns the key as an unsigned $n$-bit Integer $i$.
- `F497` — $\texttt{DICTUREMMINREF}$ ($D$ $n$ -- $D'$ $c$ $i$ $-1$ or $D$ $0$).
- `F49A` — $\texttt{DICTREMMAX}$ ($D$ $n$ -- $D'$ $x$ $k$ $-1$ or $D$ $0$), computes the maximal key $k$ (represented by a Slice with $n$ data bits) in dictionary $D$, removes $k$ from the dictionary, and returns $k$ along with the associated value $x$ and the modified dictionary $D'$.
- `F49B` — $\texttt{DICTREMMAXREF}$ ($D$ $n$ -- $D'$ $c$ $k$ $-1$ or $D$ $0$).
- `F49C` — $\texttt{DICTIREMMAX}$ ($D$ $n$ -- $D'$ $x$ $i$ $-1$ or $D$ $0$).
- `F49D` — $\texttt{DICTIREMMAXREF}$ ($D$ $n$ -- $D'$ $c$ $i$ $-1$ or $D$ $0$).
- `F49E` — $\texttt{DICTUREMMAX}$ ($D$ $n$ -- $D'$ $x$ $i$ $-1$ or $D$ $0$).
- `F49F` — $\texttt{DICTUREMMAXREF}$ ($D$ $n$ -- $D'$ $c$ $i$ $-1$ or $D$ $0$).

### A.10.11. Special Get dictionary and prefix code dictionary operations, and constant dictionaries

- `F4A0` — $\texttt{DICTIGETJMP}$ ($i$ $D$ $n$ -- ), similar to $\texttt{DICTIGET}$ (cf. [A.10.3](#a-10-3-get-dictionary-operations)), but with $x$ $\texttt{BLESS}$ed into a continuation with a subsequent $\texttt{JMPX}$ to it on success. On failure, does nothing. This is useful for implementing `switch`/`case` constructions.
- `F4A1` — $\texttt{DICTUGETJMP}$ ($i$ $D$ $n$ -- ), similar to $\texttt{DICTIGETJMP}$, but performs $\texttt{DICTUGET}$ instead of $\texttt{DICTIGET}$.
- `F4A2` — $\texttt{DICTIGETEXEC}$ ($i$ $D$ $n$ -- ), similar to $\texttt{DICTIGETJMP}$, but with $\texttt{EXECUTE}$ instead of $\texttt{JMPX}$.
- `F4A3` — $\texttt{DICTUGETEXEC}$ ($i$ $D$ $n$ -- ), similar to $\texttt{DICTUGETJMP}$, but with $\texttt{EXECUTE}$ instead of $\texttt{JMPX}$.
- `F4A6_` $n$ — $\texttt{DICTPUSHCONST}$ $n$ ( -- $D$ $n$), pushes a non-empty constant dictionary $D$ (as a $\mathit{Cell}^?$) along with its key length $0\leq n\leq 1023$, stored as a part of the instruction. The dictionary itself is created from the first of remaining references of the current continuation. In this way, the complete $\texttt{DICTPUSHCONST}$ instruction can be obtained by first serializing `xF4A8_`, then the non-empty dictionary itself (one `1` bit and a cell reference), and then the unsigned 10-bit integer $n$ (as if by a $\texttt{STU 10}$ instruction). An empty dictionary can be pushed by a $\texttt{NEWDICT}$ primitive (cf. [A.10.1](#a-10-1-dictionary-creation)) instead.
- `F4A8` — $\texttt{PFXDICTGETQ}$ ($s$ $D$ $n$ -- $s'$ $x$ $s''$ $-1$ or $s$ $0$), looks up the unique prefix of Slice $s$ present in the prefix code dictionary represented by $\mathit{Cell}^?$ $D$ and $0\leq n\leq 1023$. If found, the prefix of $s$ is returned as $s'$, and the corresponding value (also a Slice) as $x$. The remainder of $s$ is returned as a Slice $s''$. If no prefix of $s$ is a key in prefix code dictionary $D$, returns the unchanged $s$ and a zero flag to indicate failure.
- `F4A9` — $\texttt{PFXDICTGET}$ ($s$ $D$ $n$ -- $s'$ $x$ $s''$), similar to $\texttt{PFXDICTGETQ}$, but throws a cell deserialization failure exception on failure.
- `F4AA` — $\texttt{PFXDICTGETJMP}$ ($s$ $D$ $n$ -- $s'$ $s''$ or $s$), similar to $\texttt{PFXDICTGETQ}$, but on success $\texttt{BLESS}$es the value $x$ into a Continuation and transfers control to it as if by a $\texttt{JMPX}$. On failure, returns $s$ unchanged and continues execution.
- `F4AB` — $\texttt{PFXDICTGETEXEC}$ ($s$ $D$ $n$ -- $s'$ $s''$), similar to $\texttt{PFXDICTGETJMP}$, but $\texttt{EXEC}$utes the continuation found instead of jumping to it. On failure, throws a cell deserialization exception.
- `F4AE_` $n$ — $\texttt{PFXDICTCONSTGETJMP}$ $n$ or $\texttt{PFXDICTSWITCH}$ $n$ ($s$ -- $s'$ $s''$ or $s$), combines $\texttt{DICTPUSHCONST}$ $n$ for $0\leq n\leq 1023$ with $\texttt{PFXDICTGETJMP}$.
- `F4BC` — $\texttt{DICTIGETJMPZ}$ ($i$ $D$ $n$ -- $i$ or nothing), a variant of $\texttt{DICTIGETJMP}$ that returns index $i$ on failure.
- `F4BD` — $\texttt{DICTUGETJMPZ}$ ($i$ $D$ $n$ -- $i$ or nothing), a variant of $\texttt{DICTUGETJMP}$ that returns index $i$ on failure.
- `F4BE` — $\texttt{DICTIGETEXECZ}$ ($i$ $D$ $n$ -- $i$ or nothing), a variant of $\texttt{DICTIGETEXEC}$ that returns index $i$ on failure.
- `F4BF` — $\texttt{DICTUGETEXECZ}$ ($i$ $D$ $n$ -- $i$ or nothing), a variant of $\texttt{DICTUGETEXEC}$ that returns index $i$ on failure.

### A.10.12. SubDict dictionary operations

- `F4B1` — $\texttt{SUBDICTGET}$ ($k$ $l$ $D$ $n$ -- $D'$), constructs a subdictionary consisting of all keys beginning with prefix $k$ (represented by a Slice, the first $0\leq l\leq n\leq 1023$ data bits of which are used as a key) of length $l$ in dictionary $D$ of type $\mathit{HashmapE}(n,X)$ with $n$-bit keys. On success, returns the new subdictionary of the same type $\mathit{HashmapE}(n,X)$ as a Slice $D'$.
- `F4B2` — $\texttt{SUBDICTIGET}$ ($x$ $l$ $D$ $n$ -- $D'$), variant of $\texttt{SUBDICTGET}$ with the prefix represented by a signed big-endian $l$-bit Integer $x$, where necessarily $l\leq257$.
- `F4B3` — $\texttt{SUBDICTUGET}$ ($x$ $l$ $D$ $n$ -- $D'$), variant of $\texttt{SUBDICTGET}$ with the prefix represented by an unsigned big-endian $l$-bit Integer $x$, where necessarily $l\leq256$.
- `F4B5` — $\texttt{SUBDICTRPGET}$ ($k$ $l$ $D$ $n$ -- $D'$), similar to $\texttt{SUBDICTGET}$, but removes the common prefix $k$ from all keys of the new dictionary $D'$, which becomes of type $\mathit{HashmapE}(n-l,X)$.
- `F4B6` — $\texttt{SUBDICTIRPGET}$ ($x$ $l$ $D$ $n$ -- $D'$), variant of $\texttt{SUBDICTRPGET}$ with the prefix represented by a signed big-endian $l$-bit Integer $x$, where necessarily $l\leq257$.
- `F4B7` — $\texttt{SUBDICTURPGET}$ ($x$ $l$ $D$ $n$ -- $D'$), variant of $\texttt{SUBDICTRPGET}$ with the prefix represented by an unsigned big-endian $l$-bit Integer $x$, where necessarily $l\leq256$.
...
- `F800` — $\texttt{ACCEPT}$, sets current gas limit $g_l$ to its maximal allowed value $g_m$, and resets the gas credit $g_c$ to zero (cf. [1.4](#1-4-total-state-of-tvm-scccg)), decreasing the value of $g_r$ by $g_c$ in the process. In other words, the current smart contract agrees to buy some gas to finish the current transaction. This action is required to process external messages, which bring no value (hence no gas) with themselves.
- `F801` — $\texttt{SETGASLIMIT}$ ($g$ -- ), sets current gas limit $g_l$ to the minimum of $g$ and $g_m$, and resets the gas credit $g_c$ to zero. If the gas consumed so far (including the present instruction) exceeds the resulting value of $g_l$, an (unhandled) out of gas exception is thrown before setting new gas limits. Notice that $\texttt{SETGASLIMIT}$ with an argument $g\geq 2^{63}-1$ is equivalent to $\texttt{ACCEPT}$.
- `F802` — $\texttt{BUYGAS}$ ($x$ -- ), computes the amount of gas that can be bought for $x$ nanograms, and sets $g_l$ accordingly in the same way as $\texttt{SETGASLIMIT}$.
- `F804` — $\texttt{GRAMTOGAS}$ ($x$ -- $g$), computes the amount of gas that can be bought for $x$ nanograms. If $x$ is negative, returns 0. If $g$ exceeds $2^{63}-1$, it is replaced with this value.
- `F805` — $\texttt{GASTOGRAM}$ ($g$ -- $x$), computes the price of $g$ gas in nanograms.
- `F806`--`F80E` — Reserved for gas-related primitives.
- `F80F` — $\texttt{COMMIT}$ ( -- ), commits the current state of registers `c4` ("persistent data") and `c5` ("actions") so that the current execution is considered "successful" with the saved values even if an exception is thrown later.
...
- `F810` — $\texttt{RANDU256}$ ( -- $x$), generates a new pseudo-random unsigned 256-bit Integer $x$. The algorithm is as follows: if $r$ is the old value of the random seed, considered as a 32-byte array (by constructing the big-endian representation of an unsigned 256-bit integer), then its $\text{sha512}(r)$ is computed; the first 32 bytes of this hash are stored as the new value $r'$ of the random seed, and the remaining 32 bytes are returned as the next random value $x$.
- `F811` — $\texttt{RAND}$ ($y$ -- $z$), generates a new pseudo-random integer $z$ in the range $0\ldots y-1$ (or $y\ldots-1$, if $y<0$). More precisely, an unsigned random value $x$ is generated as in $\texttt{RAND256U}$; then $z:=\lfloor xy/2^{256}\rfloor$ is computed. Equivalent to $\texttt{RANDU256}$; $\texttt{MULRSHIFT 256}$.
- `F814` — $\texttt{SETRAND}$ ($x$ -- ), sets the random seed to unsigned 256-bit *Integer* $x$.
- `F815` — $\texttt{ADDRAND}$ ($x$ -- ), mixes unsigned 256-bit *Integer* $x$ into the random seed $r$ by setting the random seed to $\text{sha256}$ of the concatenation of two 32-byte strings: the first with the big-endian representation of the old seed $r$, and the second with the big-endian representation of $x$.
...
- `F82` $i$ — $\texttt{GETPARAM}$ $i$ ( -- $x$), returns the $i$-th parameter from the *Tuple* provided at `c7` for $0\leq i<16$. Equivalent to $\texttt{PUSH c7}$; $\texttt{FIRST}$; $\texttt{INDEX}$ $i$. If one of these internal operations fails, throws an appropriate type checking or range checking exception.
- `F823` — $\texttt{NOW}$ ( -- $x$), returns the current Unix time as an *Integer*. If it is impossible to recover the requested value starting from `c7`, throws a type checking or range checking exception as appropriate. Equivalent to $\texttt{GETPARAM 3}$.
- `F824` — $\texttt{BLOCKLT}$ ( -- $x$), returns the starting logical time of the current block. Equivalent to $\texttt{GETPARAM 4}$.
- `F825` — $\texttt{LTIME}$ ( -- $x$), returns the logical time of the current transaction. Equivalent to $\texttt{GETPARAM 5}$.
- `F826` — $\texttt{RANDSEED}$ ( -- $x$), returns the current random seed as an unsigned 256-bit *Integer*. Equivalent to $\texttt{GETPARAM 6}$.
- `F827` — $\texttt{BALANCE}$ ( -- $t$), returns the remaining balance of the smart contract as a *Tuple* consisting of an *Integer* (the remaining Gram balance in nanograms) and a *Maybe Cell* (a dictionary with 32-bit keys representing the balance of "extra currencies"). Equivalent to $\texttt{GETPARAM 7}$. Note that $\texttt{RAW}$ primitives such as $\texttt{SENDRAWMSG}$ do not update this field.
- `F828` — $\texttt{MYADDR}$ ( -- $s$), returns the internal address of the current smart contract as a *Slice* with a $\texttt{MsgAddressInt}$. If necessary, it can be parsed further using primitives such as $\texttt{PARSESTDADDR}$ or $\texttt{REWRITESTDADDR}$. Equivalent to $\texttt{GETPARAM 8}$.
- `F829` — $\texttt{CONFIGROOT}$ ( -- $D$), returns the *Maybe Cell* $D$ with the current global configuration dictionary. Equivalent to $\texttt{GETPARAM 9}$.
- `F830` — $\texttt{CONFIGDICT}$ ( -- $D$ $32$), returns the global configuration dictionary along with its key length (32). Equivalent to $\texttt{CONFIGROOT}$; $\texttt{PUSHINT 32}$.
- `F832` — $\texttt{CONFIGPARAM}$ ($i$ -- $c$ $-1$ or $0$), returns the value of the global configuration parameter with integer index $i$ as a *Cell* $c$, and a flag to indicate success. Equivalent to $\texttt{CONFIGDICT}$; $\texttt{DICTIGETREF}$.
- `F833` — $\texttt{CONFIGOPTPARAM}$ ($i$ -- $c^?$), returns the value of the global configuration parameter with integer index $i$ as a *Maybe Cell* $c^?$. Equivalent to $\texttt{CONFIGDICT}$; $\texttt{DICTIGETOPTREF}$.
- `F820`-`F83F` — Reserved for configuration primitives.

### A.11.5. Global variable primitives

The "global variables" may be helpful in implementing some high-level smart-contract languages. They are in fact stored as components of the *Tuple* at `c7`: the $k$-th global variable simply is the $k$-th component of this *Tuple*, for $1\leq k\leq 254$. By convention, the $0$-th component is used for the "configuration parameters" of [A.11.4](#a-11-4-configuration-primitives), so it is not available as a global variable.

- `F840` — $\texttt{GETGLOBVAR}$ ($k$ -- $x$), returns the $k$-th global variable for $0\leq k<255$. Equivalent to $\texttt{PUSH c7}$; $\texttt{SWAP}$; $\texttt{INDEXVARQ}$ (cf. [A.3.2](#a-3-2-tuple-primitives)).
- `F85_` $k$ — $\texttt{GETGLOB}$ $k$ ( -- $x$), returns the $k$-th global variable for $1\leq k\leq 31$. Equivalent to $\texttt{PUSH c7}$; $\texttt{INDEXQ}$ $k$.
- `F860` — $\texttt{SETGLOBVAR}$ ($x$ $k$ -- ), assigns $x$ to the $k$-th global variable for $0\leq k<255$. Equivalent to $\texttt{PUSH c7}$; $\texttt{ROTREV}$; $\texttt{SETINDEXVARQ}$; $\texttt{POP c7}$.
- `F87_` $k$ — $\texttt{SETGLOB}$ $k$ ($x$ -- ), assigns $x$ to the $k$-th global variable for $1\leq k\leq 31$. Equivalent to $\texttt{PUSH c7}$; $\texttt{SWAP}$; $\texttt{SETINDEXQ}$ $k$; $\texttt{POP c7}$.

### A.11.6. Hashing and cryptography primitives

- `F900` — $\texttt{HASHCU}$ ($c$ -- $x$), computes the representation hash (cf. [3.1.5](#3-1-5-the-representation-hash-of-a-cell)) of a *Cell* $c$ and returns it as a 256-bit unsigned integer $x$. Useful for signing and checking signatures of arbitrary entities represented by a tree of cells.
- `F901` — $\texttt{HASHSU}$ ($s$ -- $x$), computes the hash of a *Slice* $s$ and returns it as a 256-bit unsigned integer $x$. The result is the same as if an ordinary cell containing only data and references from $s$ had been created and its hash computed by $\texttt{HASHCU}$.
- `F902` — $\texttt{SHA256U}$ ($s$ -- $x$), computes SHA256 of the data bits of *Slice* $s$. If the bit length of $s$ is not divisible by eight, throws a cell underflow exception. The hash value is returned as a 256-bit unsigned integer $x$.
- `F910` — $\texttt{CHKSIGNU}$ ($h$ $s$ $k$ -- $?$), checks the Ed25519-signature $s$ of a hash $h$ (a 256-bit unsigned integer, usually computed as the hash of some data) using public key $k$ (also represented by a 256-bit unsigned integer). The signature $s$ must be a *Slice* containing at least 512 data bits; only the first 512 bits are used. The result is $-1$ if the signature is valid, $0$ otherwise. Notice that $\texttt{CHKSIGNU}$ is equivalent to $\texttt{ROT}$; $\texttt{NEWB}$; $\texttt{STU 256}$; $\texttt{ENDB}$; $\texttt{NEWC}$; $\texttt{ROTREV}$; $\texttt{CHKSIGNS}$, i.e., to $\texttt{CHKSIGNS}$ with the first argument $d$ set to 256-bit *Slice* containing $h$. Therefore, if $h$ is computed as the hash of some data, these data are hashed *twice*, the second hashing occurring inside $\texttt{CHKSIGNS}$.
- `F911` — $\texttt{CHKSIGNS}$ ($d$ $s$ $k$ -- $?$), checks whether $s$ is a valid Ed25519-signature of the data portion of *Slice* $d$ using public key $k$, similarly to $\texttt{CHKSIGNU}$. If the bit length of *Slice* $d$ is not divisible by eight, throws a cell underflow exception. The verification of Ed25519 signatures is the standard one, with SHA256 used to reduce $d$ to the 256-bit number that is actually signed.
- `F912`-`F93F` — Reserved for hashing and cryptography primitives.

### A.11.7. Miscellaneous primitives

- `F940` — $\texttt{CDATASIZEQ}$ ($c$ $n$ -- $x$ $y$ $z$ $-1$ or $0$), recursively computes the count of distinct cells $x$, data bits $y$, and cell references $z$ in the dag rooted at *Cell* $c$, effectively returning the total storage used by this dag taking into account the identification of equal cells. The values of $x$, $y$, and $z$ are computed by a depth-first traversal of this dag, with a hash table of visited cell hashes used to prevent visits of already-visited cells. The total count of visited cells $x$ cannot exceed non-negative *Integer* $n$; otherwise the computation is aborted before visiting the $(n+1)$-st cell and a zero is returned to indicate failure. If $c$ is *Null*, returns $x=y=z=0$.
- `F941` — $\texttt{CDATASIZE}$ ($c$ $n$ -- $x$ $y$ $z$), a non-quiet version of $\texttt{CDATASIZEQ}$ that throws a cell overflow exception (8) on failure.
- `F942` — $\texttt{SDATASIZEQ}$ ($s$ $n$ -- $x$ $y$ $z$ $-1$ or $0$), similar to $\texttt{CDATASIZEQ}$, but accepting a *Slice* $s$ instead of a *Cell*. The returned value of $x$ does not take into account the cell that contains the slice $s$ itself; however, the data bits and the cell references of $s$ are accounted for in $y$ and $z$.
- `F943` — $\texttt{SDATASIZE}$ ($s$ $n$ -- $x$ $y$ $z$), a non-quiet version of $\texttt{SDATASIZEQ}$ that throws a cell overflow exception (8) on failure.
...
- `FA03` — $\texttt{STVARINT16}$ ($b$ $x$ -- $b'$), similar to $\texttt{STVARUINT16}$, but serializes a *signed* *Integer* $x$ in the range $-2^{119}\ldots2^{119}-1$.
- `FA04` — $\texttt{LDVARUINT32}$ ($s$ -- $x$ $s'$), loads (deserializes) a $\texttt{VarUInteger 32}$ from *CellSlice* $s$, and returns the deserialized value as an *Integer* $0\leq x<2^{248}$. The expected serialization of $x$ consists of a 5-bit unsigned big-endian integer $l$, followed by an $8l$-bit unsigned big-endian representation of $x$. The net effect is approximately equivalent to $\texttt{LDU 5}$; $\texttt{SWAP}$; $\texttt{SHIFT 3}$; $\texttt{LDUX}$.
- `FA05` — $\texttt{LDVARINT32}$ ($s$ -- $x$ $s'$), deserializes a $\texttt{VarInteger 32}$ from *CellSlice* $s$, and returns the deserialized value as an *Integer* $-2^{247}\leq x<2^{247}$.
- `FA06` — $\texttt{STVARUINT32}$ ($b$ $x$ -- $b'$), serializes an *Integer* $0\leq x<2^{248}$ as a $\texttt{VarUInteger 32}$.
- `FA07` — $\texttt{STVARINT32}$ ($b$ $x$ -- $b'$), serializes an *Integer* $-2^{247}\leq x<2^{247}$ as a $\texttt{VarInteger 32}$.
...
- `FE` $nn$ — $\texttt{DEBUG}$ $nn$, for $0\leq nn<240$, is a two-byte NOP.
- `FEF` $n$ $ssss$ — $\texttt{DEBUGSTR}$ $ssss$, for $0\leq n<16$, is an $(n+3)$-byte NOP, with the $(n+1)$-byte "contents string" $ssss$ skipped as well.
...
- `FE00` — $\texttt{DUMPSTK}$, dumps the stack (at most the top 255 values) and shows the total stack depth.
- `FE0` $n$ — $\texttt{DUMPSTKTOP}$ $n$, $1\leq n<15$, dumps the top $n$ values from the stack, starting from the deepest of them. If there are $d<n$ values available, dumps only $d$ values.
- `FE10` — $\texttt{HEXDUMP}$, dumps `s0` in hexadecimal form, be it a *Slice* or an *Integer*.
- `FE11` — $\texttt{HEXPRINT}$, similar to $\texttt{HEXDUMP}$, except the hexadecimal representation of `s0` is not immediately output, but rather concatenated to an output text buffer.
- `FE12` — $\texttt{BINDUMP}$, dumps `s0` in binary form, similarly to $\texttt{HEXDUMP}$.
- `FE13` — $\texttt{BINPRINT}$, outputs the binary representation of `s0` to a text buffer.
- `FE14` — $\texttt{STRDUMP}$, dumps the *Slice* at `s0` as an UTF-8 string.
- `FE15` — $\texttt{STRPRINT}$, similar to $\texttt{STRDUMP}$, but outputs the string into a text buffer (without carriage return).
- `FE1E` — $\texttt{DEBUGOFF}$, disables all debug output until it is re-enabled by a $\texttt{DEBUGON}$. More precisely, this primitive increases an internal counter, which disables all debug operations (except $\texttt{DEBUGOFF}$ and $\texttt{DEBUGON}$) when strictly positive.
- `FE1F` — $\texttt{DEBUGON}$, enables debug output (in a debug version of TVM).
- `FE2` $n$ — $\texttt{DUMP s}(n)$, $0\leq n<15$, dumps `s`$(n)$.
- `FE3` $n$ — $\texttt{PRINT s}(n)$, $0\leq n<15$, concatenates the text representation of `s`$(n)$ (without any leading or trailing spaces or carriage returns) to a text buffer which will be output before the output of any other debug operation.
- `FEC0`-`FEEF` — Use these opcodes for custom/experimental debug operations.
- `FEF` $nssss$ — $\texttt{DUMPTOSFMT}$ $ssss$, dumps `s0` formatted according to the $(n+1)$-byte string $ssss$. This string might contain (a prefix of) the name of a TL-B type supported by the debugger. If the string begins with a zero byte, simply outputs it (without the first byte) into the debug log. If the string begins with a byte equal to one, concatenates it to a buffer, which will be output before the output of any other debug operation (effectively outputs a string without a carriage return).
- `FEF` $n$ `00` $ssss$ — $\texttt{LOGSTR}$ $ssss$, string $ssss$ is $n$ bytes long.
- `FEF000` — $\texttt{LOGFLUSH}$, flushes all pending debug output from the buffer into the debug log.
- `FEF` $n$ `01` $ssss$ — $\texttt{PRINTSTR}$ $ssss$, string $ssss$ is $n$ bytes long.
...
- `FF` $nn$ — $\texttt{SETCP}$ $nn$, selects TVM codepage $0\leq nn<240$. If the codepage is not supported, throws an invalid opcode exception.
- `FF00` — $\texttt{SETCP0}$, selects TVM (test) codepage zero as described in this document.
...
- `FFF0` — $\texttt{SETCPX}$ ($c$ -- ), selects codepage $c$ with $-2^{15}\leq c<2^{15}$ passed in the top of the stack.
---
...
In order to estimate the consequences of assuming $m=n=16$, we will assume that all our register machines are equipped with a stack, and with one-byte instructions $\texttt{PUSH r}(i)$ and $\texttt{POP r}(i)$, which push or pop a register into/from the stack. For example, the three-address machine code provided in [C.1.2](#c-1-2-three-address-register-machine) destroys the values in registers `r2`, `r3`, `r6`, and `r7`; this means that the code of this function must be augmented by four instructions $\texttt{PUSH r2}$; $\texttt{PUSH r3}$; $\texttt{PUSH r6}$; $\texttt{PUSH r7}$ at the beginning, and by four instructions $\texttt{POP r7}$; $\texttt{POP r6}$; $\texttt{POP r3}$; $\texttt{POP r2}$ right before the $\texttt{RET}$ instruction, in order to restore the original values of these registers from the stack. These four additional $\texttt{PUSH}$/$\texttt{POP}$ pairs would increase the operation count and code size in bytes by $4\times 2=8$. A similar analysis can be done for other register machines as well, leading to [Table 2](#c-2-3-case-m%3Dn%3D16%3A-all-registers-must-be-preserved).
...
As explained in [C.3.3](#c-3-3-three-address-and-two-address-register-machines%2C-preserved-registers-2), the preservation of `r8`--`r15` between subroutine calls does not improve the size of our previously written code, so the one-address machine will use for $m=8$ the same code provided in [C.3.5](#c-3-5-one-address-register-machine%2C-preserved-registers).
