...
A *private key* for Ed25519 is just an arbitrary 256-bit string $k$. A *secret exponent* $a$ and *secret salt* $k''$ are derived from $k$ by first computing $\text{SHA512}(k)$, and then taking the first 256 bits of this SHA512 as the little-endian representation of $a$ (but with bits 255, 2, 1, and 0 cleared, and bit 254 set); the last 256 bits of $\text{SHA512}(k)$ then constitute $k''$.
