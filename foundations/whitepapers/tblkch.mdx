...
Such an assignment can be achieved by first [assigning](#1-4-1-logical-time) logical time $\text{Lt}(e)$ to all atomic events $e\in E$, then setting $\text{Lt}^-(C):=\inf_{e\in C}\text{Lt}(e)$ and $\text{Lt}^+(C):=1+\sup_{e\in C}\text{Lt}(e)$ for any $C\subset E$.
