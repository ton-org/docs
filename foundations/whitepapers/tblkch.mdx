...
3. A signed shardchain block $\tilde B$ is generated from $B$ and these validator signatures as described above [Signed shardchain block](#5-1-8-signed-shardchain-block).
...
3. If $h=1$, the serialization is continued by $l+1$ 32-byte [higher hashes](/foundations/whitepapers/tvm#3-1-6-higher-hashes-of-a-cell) of $c$: $\text{Hash}_1(c)$, ..., $\text{Hash}_{l+1}(c)=\text{Hash}_\infty(c)$.
