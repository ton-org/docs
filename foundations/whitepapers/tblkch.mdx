...
In addition to the transparent and representation hashes of a cell $c$, a sequence of *higher hashes* $\text{Hash}_i(c)$, $i=1,2,\dots$ may be defined, which eventually stabilizes at $\text{Hash}_\infty(c)$. (More detail may be found in [Generalities on cells](/foundations/whitepapers/tvm#3-1-generalities-on-cells).)
...
All of the most important non-split shardchain data structures related to messages are organized as *hashmaps* or *dictionaries* (implemented by means of Patricia trees serialized into a tree of cells as described in [Hashmaps, or dictionaries](/foundations/whitepapers/tvm#3-3-hashmaps%2C-or-dictionaries)), with the following keys:
...
In order to simplify the verification of local conditions, they are made *constructible* (i.e., verifiable in bounded time) by adding some *witness* data structures. For instance, condition $C$ of ([5](#1-3-7-constructive-elimination-of-existence-quantifiers)) may be transformed by adding a new data structure $f:X\to Y$ (a map $f$ from $X$ to $Y$) and imposing the following condition $C'$ instead:
...
Similarly to the transformation described in [Constructive elimination of existence quantifiers](#1-3-7-constructive-elimination-of-existence-quantifiers), condition
...
For instance, *InMsgDescr*, as considered in [Example: consistency condition for *InMsgDescr*](#1-3-8-example%3A-consistency-condition-for-inmsgdescr), can contain both messages processed in the block and transit messages. We might introduce a field in the inbound message description to indicate whether the message is transit or not, and, in the latter case, include a witness field for the transaction processing the message.
...
without insisting that $\text{Lt}(e)$ be the smallest non-negative integer with this property. In such cases we can speak about *relaxed* logical time, as opposed to the *strict* [logical time](#1-4-1-logical-time) defined above. Notice, however, that the condition ([9](#1-4-2-a-relaxed-variant-of-logical-time)) is a fundamental property of logical time and cannot be relaxed further.
...
If the validator election procedure ever needs to be changed, this can be accomplished by first committing a new validator election smart contract into the masterchain, and then changing the ordinary configurable parameter containing the address $\nu$ of the validator election smart contract. This will require two-thirds of the validators to accept the proposal in a vote as described above in [Changing the values of configurable parameters](#1-6-7-changing-the-values-of-configurable-parameters).
...
Notice that a shardchain is always considered a neighbor of itself. This may seem redundant, because we always repeat the next-hop computation described in [Computation of the next-hop address for hypercube routing](#2-1-5-computation-of-the-next-hop-address-for-hypercube-routing) until we obtain a next-hop address outside the current shardchain ([Internal routing](#2-1-11-internal-routing)). However, there are at least two reasons for such an arrangement:
...
- A proof that $m$ has not been included in a preceding block of the same shardchain, using the block header information containing the smallest and the largest logical time of all messages imported into the block ([Checking whether a message has already been delivered to its final destination](#2-3-4-checking-whether-a-message-has-already-been-delivered-to-its-final-destination)–[Checking whether an HR message has already been delivered via IHR to its final destination](#2-3-7-checking-whether-an-hr-message-has-already-been-delivered-via-ihr-to-its-final-destination) for more information).
...
Consider the routing and delivery of the same message $m$ with source $\xi$ and destination $\eta$ as discussed in [An overview of Hypercube Routing](#2-2-12-an-overview-of-hypercube-routing):
...
The two last lines define type `MsgAddress` to be the internal union of types `MsgAddressInt` and `MsgAddressExt` (not to be confused with their external union `Either MsgAddressInt MsgAddressExt` as defined in [Some standard definitions](#3-1-1-some-standard-definitions)), as if the preceding four lines had been repeated with the right-hand side replaced by `MsgAddress`. In this way, type `MsgAddress` has four constructors, and types `MsgAddressInt` and `MsgAddressExt` are both subtypes of `MsgAddress`.
...
Recall ([The TON Coin, or the Gram](/foundations/whitepapers/ton#a-the-ton-coin-or-the-gram)) that the original total supply of Grams is fixed at five billion (i.e., $5\cdot10^{18}<2^{63}$ nanograms), and is expected to grow very slowly. Therefore, all the amounts of Grams encountered in practice will fit in unsigned or even signed 64-bit integers. The validators may use the 64-bit integer representation of Grams in their internal computations; however, the serialization of these values the blockchain is another matter.
...
Each inbound message is described by an instance of the `InMsg` type, which has six constructors corresponding to the cases listed above in [Types and sources of inbound messages](#3-2-1-types-and-sources-of-inbound-messages):
...
Augmented hashmaps are key-value storage structures with $n$-bit keys and values of some type $X$, similar to the ordinary hashmaps described in the [Hashmaps, or dictionaries](/foundations/whitepapers/tvm#3-3-hashmaps,-or-dictionaries). However, each intermediate node of the Patricia tree representing an *augmented* hashmap is augmented by a value of type $Y$.
...
The serialization of augmented hashmaps with $n$-bit keys, values of type $X$, and augmentation values of type $Y$ is given by the following TL-B scheme, which is an extension of the one provided in [Serialization of hashmaps](/foundations/whitepapers/tvm#3-3-3-serialization-of-hashmaps):
...
Now the *InMsgDescr* itself is defined as an augmented hashmap, with 256-bit keys (equal to the representation hashes of imported messages), values of type [`InMsg`](#3-2-2-descriptor-of-an-inbound-message), and [augmentation values of type `ImportFees`](#3-2-7-augmentation-of-inmsgdescr):
...
The fork evaluation and leaf evaluation functions ([Augmented hashmaps, or dictionaries](#3-2-5-augmented-hashmaps%2C-or-dictionaries)) are not included explicitly in the above notation, because the dependent types of TL-B are not expressive enough for this purpose. In words, the fork evaluation function is just the componentwise addition of two `ImportFees` instances, and the leaf evaluation function is defined by the rules listed in [Collecting forwarding and transit fees from imported messages](#3-2-3-collecting-forwarding-and-transit-fees-from-imported-messages) and [Imported value of an inbound message](#3-2-4-imported-value-of-an-inbound-message). In this way, the root of the Patricia tree representing an instance of *InMsgDescr* contains an *ImportFees* instance with the total value imported by all inbound messages, and with the total forwarding fees collected from them.
...
- A message dequeueing record ([`msg_export_deq`](#3-3-2-message-dequeueing-records)) exports no value.
...
The augmentation is the *exported value* of the corresponding message, aggregated by means of the sum, and computed at the leaves as explained in [Exported value of an outbound message](#3-3-4-exported-value-of-an-outbound-message). In this way, the total exported value appears near the root of the Patricia tree representing *OutMsgDescr*.
...
Recall ([Outbound message queue of a shardchain](#1-2-7-outbound-message-queue-of-a-shardchain)) that *OutMsgQueue* is a part of the blockchain state, not of a block. Therefore, a block contains only hash references to its initial and final state, and its newly-created cells.
...
- On some occasions messages are converted into account states, and vice versa. For instance, when a shardchain merge event occurs, and two accounts that are instances of the same large contract need to be merged, one of them is converted into a message sent to the other one ([Merge transactions](#4-2-11-merge-transactions)). Similarly, when a shardchain split event occurs, and an instance of a large smart contract needs to be split into two, this is achieved by a special transaction that creates the new instance by means of a constructor message sent from the previously existing instance to the new one ([Split transactions](#4-2-10-split-transactions)).
...
The split part of the shardchain state ([The Infinite Sharding Paradigm (ISP) applied to blockchain block and state](#1-2-1-the-infinite-sharding-paradigm-isp-applied-to-blockchain-block-and-state) and [Split and non-split part of the shardchain block and state](#1-2-2-split-and-non-split-part-of-the-shardchain-block-and-state)) is given by
...
Each transaction $t$ has a logical time interval $\text{Lt}^\bullet(t)=[\text{Lt}^-(t),\text{Lt}^+(t))$ assigned to it ([Logical time in the TON Blockchain](#1-4-6-logical-time-in-the-ton-blockchain) and [Logical time intervals](#1-4-3-logical-time-intervals)). By convention, a transaction $t$ generating $n$ outbound messages $m_1$, $\ldots$, $m_n$ is assigned a logical time interval of length $n+1$, so that
...
- First a *merge prepare transaction* is issued for $\xi'$, which converts all of its persistent state and balance into a special constructor message with destination $\xi$ ([Account state as a message from an account to its future self](#4-1-7-account-state-as-a-message-from-an-account-to-its-future-self)).
...
Any transaction contains the fields listed in [Generic components of a transaction](#4-2-3-generic-components-of-a-transaction). As a consequence, there are some common components in all transactions:
...
The exclamation mark in the TL-B declaration of a `merkle_update` indicates special processing required for such values. In particular, they must be kept in a separate cell, which must be marked as *exotic* by a bit in its header ([Generalities on cells](/foundations/whitepapers/tvm#3-1-generalities-on-cells)).

A full explanation of the serialization of *TransactionDescr*, which describes one transaction according to its kind listed in [Kinds of transactions](#4-2-4-kinds-of-transactions), can be found in [Transaction descriptions](#4-3-transaction-descriptions).
...
In addition to this dictionary, an *AccountBlock* contains a Merkle update ([Generalities on cells](/foundations/whitepapers/tvm#3-1-generalities-on-cells)) of the total state of the account. If an account did not exist before the block, its state is represented by an `account_none`.
...
All transactions in a block are represented by ([The Infinite Sharding Paradigm (ISP) applied to blockchain block and state](#1-2-1-the-infinite-sharding-paradigm-isp-applied-to-blockchain-block-and-state)):
...
- If $\xi$ is present in *ShardAccountBlock*, its initial and final states as indicated in AccountBlock must match those indicated in the initial and final states of the shardchain block, expressed by instances of *ShardAccounts* ([The combined state of all accounts in a shard](#4-1-9-the-combined-state-of-all-accounts-in-a-shard)).
...
This section presents the specific TL-B schemes for transaction descriptions according to the classification provided in [Kinds of transactions](#4-2-4-kinds-of-transactions).
...
- The original $\texttt{cc}$ (current continuation) is initialized using the cell slice created from the cell $\texttt{code}$, containing the code of the smart contract described in [Smart-contract code](#4-4-1-smart-contract-code).
...
After TVM is initialized as described in [The initial state of TVM](#4-4-4-the-initial-state-of-tvm), its stack is initialized by pushing the arguments to the main() function of the smart contract as follows:
...
An inbound external message is processed similarly to [The initial state of TVM](#4-4-4-the-initial-state-of-tvm) and [The initial stack of TVM for processing an internal message](#4-4-5-the-initial-stack-of-tvm-for-processing-an-internal-message), with the following modifications:
...
The TVM stack for processing tick and tock transactions ([Kinds of transactions](#4-2-4-kinds-of-transactions)) is initialized by pushing the following values:
...
- The state $\tilde S$ of the sibling account, represented by a *StateInit* ([Message layout](#3-1-7-message-layout)).
- A *Slice* containing [*SplitMergeInfo*](#4-3-13-split-prepare-and-install-transactions).
- The 256-bit address $\xi$ of the current account.
- The 256-bit address $\tilde\xi$ of the sibling account.
- An integer $0\leq d\leq 63$, equal to the position of the only bit in which $\xi$ and $\tilde\xi$ differ.
...
- *ShardAccounts*, the [split part of the shardchain state](#1-2-2-split-and-non-split-part-of-the-shardchain-block-and-state) containing the state of [all accounts assigned to this shard](#4-1-9-the-combined-state-of-all-accounts-in-a-shard).
- *OutMsgQueue*, the [output message queue of the shardchain](#3-3-6-structure-of-outmsgqueue).
...
- A hash reference to the most recent masterchain block, indirectly describing the state of the masterchain and, through it, the [state of all other shardchains of the TON Blockchain](#1-5-2-total-state-defined-by-a-shardchain-block).
...
- *InMsgDescr* and *OutMsgDescr*, the inbound and outbound message descriptors ([Structure of *InMsgDescr*](#3-2-8-structure-of-inmsgdescr) and [Structure of *OutMsgDescr*](#3-3-5-structure-of-outmsgdescr)).
- *ShardAccountBlocks*, the [collection of all transactions processed in the block](#4-2-17-collection-of-all-transactions-in-a-block) along with all updates of the states of the accounts assigned to the shard. This is the *split* part of the [shardchain block](#1-2-2-split-and-non-split-part-of-the-shardchain-block-and-state).
...
The shardchain state ([The Infinite Sharding Paradigm (ISP) applied to blockchain block and state](#1-2-1-the-infinite-sharding-paradigm-isp-applied-to-blockchain-block-and-state) and [Components of the shardchain state](#5-1-1-components-of-the-shardchain-state)) is serialized according to the following TL-B scheme:
...
3. A signed shardchain block $\tilde B$ is generated from $B$ and these [validator signatures](#5-1-8-signed-shardchain-block) as described above ([Signed shardchain block](#5-1-8-signed-shardchain-block)).
...
Masterchain blocks are very similar to shardchain blocks of the basic workchain. This section lists some of the modifications needed to obtain the description of a masterchain block from the description of a shardchain block given in [Shardchain block layout](#5-1-shardchain-block-layout).

### 5.2.1 Additional components present in the masterchain state

In addition to the components listed in [Components of the shardchain state](#5-1-1-components-of-the-shardchain-state), the masterchain state must contain:
...
In addition to the components listed in [Components of a shardchain block](#5-1-2-components-of-a-shardchain-block), each masterchain block must contain:
...
The data specific to the masterchain state is collected into *McStateExtra*, already mentioned in [TL-B scheme for the shardchain state](#5-1-4-tl-b-scheme-for-the-shardchain-state):
...
2. Choose an integer $s$, such that $n\leq 2^s$. Represent each cell $c_i$ by an integral number of octets in the standard way ([The layout of a single cell](#1-1-3-the-layout-of-a-single-cell) or [Standard cell representation](/foundations/whitepapers/tvm#3-1-4-standard-cell-representation)), but using unsigned big-endian $s$-bit integer $j$ instead of hash $\text{Hash}(c_j)$ to represent internal references to cell $c_j$ ([Serialization of one cell from a bag of cells](#5-3-6-serialization-of-one-cell-from-a-bag-of-cells) below).
...
3. If $h=1$, the serialization is continued by $l+1$ 32-byte higher hashes of $c$ ([Higher hashes of a cell](/foundations/whitepapers/tvm#3-1-6-higher-hashes-of-a-cell)): $\text{Hash}_1(c)$, ..., $\text{Hash}_{l+1}(c)=\text{Hash}_\infty(c)$.
...
Usually a private key for elliptic curve cryptography described by the data listed in [Data for elliptic curve cryptography](#a-1-9-data-for-elliptic-curve-cryptography) is a "random" integer $0<a<\ell$, called the *secret exponent*, and the corresponding public key is the point $A:=[a]G$ (or just its $x$-coordinate $x_A$), suitably serialized.
...
Notice that $\phi(A)$ is the public key for Curve25519 defined by the same private key $k$ according to [Public and private keys for TON Curve25519 cryptography](#a-2-4-public-and-private-keys-for-ton-curve25519-cryptography) and ([45](#a-3-6-generator-of-ed25519)). In this way, we can convert public keys for Ed25519 into corresponding public keys for Curve25519, and vice versa. Private keys do not need to be transformed at all.
...
<span id="fn8">**8**</span> This example is a bit simplified since it does not take into account the presence of transit messages in InMsgDescr, which are not processed by any explicit transaction. [Back ↑](#1-3-8-example%3A-consistency-condition-for-inmsgdescr)
...
<span id="fn28">**28**</span> For simplicity, we sometimes treat the masterchain as just another workchain with $\mathit{workchain\_id}=-1$. [Back ↑](#4-1-1-account-addresses)
...
<span id="fn33">**33**</span> Notice that this record does not represent a change in the state of the account, because the transaction may still be aborted during the action phase. In that case, the new persistent data indirectly referenced by vm_final_state_hash will be discarded. [Back ↑](#4-3-7-valid-computing-phase)

<span id="fn34">**34**</span> The most common way of creating shared libraries for TVM is to publish a reference to the root cell of the library in the masterchain. [Back ↑](#4-4-3-smart-contract-library-environment)

<span id="fn35">**35**</span> The persistent data of the smart contract need not be loaded in its entirety for this to occur. Instead the root is loaded, and TVM may load other cells by their references from the root only when they are accessed, thus providing a form of virtual memory. [Back ↑](#4-4-4-the-initial-state-of-tvm)

<span id="fn36">**36**</span> Both the global gas limit and the gas price are configurable parameters determined by the current state of the masterchain. [Back ↑](#4-4-4-the-initial-state-of-tvm)
...
<span id="fn39">**39**</span> Notice that exotic cells (with $s=1$) always have $b\geq8$, with the cell type encoded in the first eight data bits (cf. [[4, 3.1.7](#ref-4)]). [Back ↑](#5-3-6-serialization-of-one-cell-from-a-bag-of-cells)
