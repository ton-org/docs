...
Now the *InMsgDescr* itself is defined as an augmented hashmap, with 256-bit keys (equal to the representation hashes of imported messages), values of type [`InMsg`](#3-2-2-descriptor-of-an-inbound-message), and [augmentation](#3-2-7-augmentation-of-inmsgdescr) values of type `ImportFees`:
...
In addition to the [components of a shardchain state](#5-1-1-components-of-the-shardchain-state), the masterchain state must contain:
...
In addition to the [components of a shardchain block](#5-1-2-components-of-a-shardchain-block), each masterchain block must contain:
