...
All participants of the network are known. Each validator knows the ADNL address and the public key of every other validator. Messages can therefore be encrypted and signed with private keys, and every signature can be verified by every other participant.
...
Every 0.1–0.2 seconds a node selects three random neighbors and starts a synchronization round. Synchronization can be performed in two ways:
