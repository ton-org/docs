...
- **frozen**: occurs when the storage debt of an active account exceeds 0.1 TON. Only the hashes of the previous code and data cells are preserved. While frozen, the contract cannot be executed. To unfreeze, send a message with the valid [`StateInit`](/foundations/messages/state-init) and sufficient funds for storage fees. Recovery is complex; avoid reaching this state. A project to unfreeze accounts is available [here](https://unfreezer.ton.org/).
...
- `value`: an amount of nanotons in a message.
  ...
  - 0;
  - \> 0.
- `state_init`: [`StateInit`](/foundations/messages/state-init) structure.
  - any;
  - none;
