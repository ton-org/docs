...
Sending a message is the most frequent actions that a smart contract performs during the [action phase](/foundations/phases#action-phase). It can get into the out action list by:

- The [`SENDRAWMSG`](/tvm/instructions#fb08-sendrawmsg) instruction.
...
- `scr` address is set to address of the sender smart contract;
...
- fill the `scr` field with [`addr_none`](/foundations/addresses/overview#external-addresses). 

If after the transformation the message cell does not fit into the maximum allowed size (1023 bits), the action is repeated up to two times:

- with `init` packed in a reference separate from the root;
- with `init` and `body` packed in references separate from the root.
