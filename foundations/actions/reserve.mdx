...
1. Check that `mode` has flag `ReserveBounceIfActionFail`:
   ...
   - if not, then in case of any failure the action phase will be interrupted without further processing.
...
If the action had flag `16`, then in case of any of the above errors the transaction enters [bounce phase](/foundations/phases#bounce-phase), so a [bounce message](/foundations/messages/internal#bounces) is generated when conditions are met.
