...
- If the `mode` bitmask has more than first five ones positive bits, then the error [`34`](https://beta-docs.ton.org/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
- If `mode` has flag `8` but not flag `4`, then the error [`34`](https://beta-docs.ton.org/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
- If after step 3, `reserve` is negative, then the error [`34`](https://beta-docs.ton.org/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
