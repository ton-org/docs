...
After that, each of the references of each cell might be replaced by the 32-byte [representation hash](/foundation/serialization/cells#standard-cell-representation-and-its-hash) of the cell referred to. Thus a _bag of cells (BoC)_ is obtained.
...
- Choose the smallest number of bytes `s` that can contain the binary representation of `n`. Serialize each cell `ci` in a way similar to [standard representation algorithm](/foundation/serialization/cells#standard-cell-representation-and-its-hash), with exceptions:
