...
- `init` is an optional field of [`StateInit`](/foundations/messages/state-init) type used to initialize the contract state, stored either in-place or in a ref of the cell with a message.
...
- When an incoming external message is processed, a transaction will be created only if the contract [accepts](/foundations/messages/external#accept-message) the message.
...
- [tick-tock transactions](/foundations/messages/tick-tock)
- [split-prepare transactions](/foundations/messages/split-prepare)
- [split-install transactions](/foundations/messages/split-install)
- [storage-tx transactions](/foundations/messages/storage-tx)
