...
Internal messages are the messages sent from one contract to another. They are the most common type of message.

The contents of an internal message are as follows:
...
This is the structure of the message a contract may receive. However, the requirements for this structure differ when the contract sends a message. These requirements are described by the relaxed version of the structure.
...
The process of transforming the relaxed version of the structure into the strict version occurs in the action phase and is performed by the [SENDMSG action](/foundations/actions/send). This action rewrites some fields of the structure with correct values and validates the `StateInit` field against the `StateInitWithLibs` type.

## Deployment of contracts

Thanks to this structure, it is possible to send both the payload in the `body` field and the StateInit in the `init` field. As a result, the contract can be [initialized by StateInit](/foundations/messages/state-init) and process the payload in one transaction.

## Message value

The message may carry a value of Toncoin and extra-currency. The value is transferred from the sender to the recipient of the message. The value of this field is affected by the [send modes](/foundations/messages/modes) the contract uses.

## Bounces

An interesting feature of internal messages is bouncing. A bounce message is generated automatically by the network when the original message results in an error in the [compute phase](/foundations/phases#compute-phase) during processing. It may also occur when the message results in an error in the [action phase](/foundations/phases#action-phase) and mode `SendBounceIfActionFail` (+16) is used.
...
- The `bounced` flag is set to `true`.
- The `bounce` flag is set to `false`.
- `body` is replaced with the concatenation of 32 bits equal to one (0xffffffff) and the first 256 bits of the old `body`. The new `body` is stored in the message in-place, so `Either = 0`.
...
If `(extra_flags & 3) = 3`, the bounced message contains the entire body of the original message. Otherwise, only the bits from the root of the original body are returned.

All other bits in `extra_flags` are reserved for future use and are currently disallowed (internal messages with flags other than `0..3` are invalid).

When a message with the new bounce flag is bounced, the bounced message body has the following format (`new_bounce_body`):
...
- `gas_used` and `vm_steps` are the same as in `TrComputePhase` of the transaction.
