...
_Reserving coins_ is one of actions that a smart contract can perform during the [action phase](/foundations/phases#action-phase). In TVM it is implemented via the [`RAWRESERVE`](/tvm/instructions#fb02-rawreserve)
...
Below we describe how the reservation action is implemented. To see examples of when the reservation is used in practice and learn how to solve related problems, visit the [reserve patterns](/patterns/reserve) page.
