...
When an external-in message is sent to the blockchain, it's being replayed by the validators. This means that the message sent once can be delivered to the contract multiple times. Normally, each external-in message should be accepted only once. There are different techniques of achieving this goal. They are used in different [wallets](/standard/wallets/comparison).
