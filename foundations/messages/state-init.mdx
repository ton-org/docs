...
Assume we have an address `D` starting with `0x5ABC...`. If there are 16 active shardchains, first 4 bits of the address (i.e., `5`) correspond to shard IDs.

Now, if some other contract `S` is deployed that intends to exchange messages with `D`, and computing an address from its `StateInit` results in `0xA123...`, it will be deployed by default into a shard `A`, which is the most distant from shard `5`. Messages from `5` to `A` will take 4 hops and 4 blocks to arrive at the destination.

To put `S` into the same shard as `D`, `fixed_prefix_length` can be set to 4 in a deploy message sent to `0x5123...` address. Thus, TVM will ignore the discrepancy in first 4 bits of a passed address (`0x5123...`) and computed address (`0xA123...`), and it will be deployed into the same shard `5`.
