...
- Generate a mnemonic (a key). It uniquely determines wallet's address, but the wallet doesn't exist on blockchain yet, i.e. is in [`nonexist`](http://localhost:3000/foundations/status#status-variety) status.
- Send some funds to the wallet's account. Now it will be in [`uninit`](http://localhost:3000/foundations/status#status-variety) status, i.e. already with some balance on it, but without any code yet.
- Deploy wallet's code to this address. Some of these funds will be used to pay for the deploy process. Now the wallet is in [`active`](http://localhost:3000/foundations/status#status-variety) status, and can be used for any purpose.
