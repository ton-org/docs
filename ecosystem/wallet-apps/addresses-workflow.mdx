...
At the moment, TON wallets work with [addresses](/ton/addresses/formats) as follows:
...
1. The wallet app retrieve from address bounceable flag.
1. The wallet app check the destination address. If it has `unitialized` status, the app force set `bounce` field of sending message to `false` and ignore bounceable/non-bounceable flag from address representation.
1. If destination is not `unitialized` then the wallet app uses the bounceable/non-bounceable flag from the address representation for the `bounce` field of sending message.
