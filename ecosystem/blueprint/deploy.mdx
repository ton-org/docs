...
```
3. Script can be run using `npx blueprint run <SCRIPT> [arg1, arg2, ...]` command

### Deploying contracts

TODO: example's link
1. You need a deployment script in `scripts/deploy<CONTRACT>.ts` - [example](/example/scripts/deployCounter.ts)
2. Run interactive: &nbsp;&nbsp; `npx blueprint run`
3. Non-interactive: &nbsp; `npx blueprint run deploy<CONTRACT> --<NETWORK> --<DEPLOY_METHOD>`
  * Example: `npx blueprint run deployCounter --mainnet --tonconnect`
