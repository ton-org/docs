...
to customize Blueprint features.
...
- [scaffold](https://github.com/1IxI1/blueprint-scaffold) – creates a simple decentralized application (dApp) using the wrappers’ code.
...
Each property of the `network` object has the same meaning as its corresponding `--custom` flag. See [Blueprint reference → `run`](/ecosystem/blueprint/reference#run) for details.
...
You can also specify these parameters using the command-line interface (CLI):
