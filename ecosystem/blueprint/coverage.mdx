...
<Aside>
    Note, that this article is about coverage, that is calculated on TVM assembler statements. Calculation on paths and source code lines is not yet implemented.
</Aside>
...
<Aside type="caution" title="Library compatibility">
    For this way to work correctly, version of `@ton/sandbox >= 0.37.2` and `@ton/blueprint >= 0.41.0` is needed.
</Aside>
...
<Image src="/resources/images/coverage-report.png" />

Apart from statistics in the header, the most useful part is the line-by-line coverage report. Most of the fields are self-explaining, but the code section is fascinating. The blue number on the left shows the number of times code hit that exact instruction, while the red number shows the gas this instruction costs. This allows one to explore not only coverage, but the overall gas-efficiency.

<Aside>
    To fully understand the assembler output it's heavily recommended to familiarise yourself with [TVM](/ton/tvm.mdx).
</Aside>

## Limitations

Note that when code of other contracts is stored directly in the code of contract ([Tact](/language/tact) does that automatically if a contract system does not contain circular dependencies), that affects the overall code coverage percentage.

Hopefully, this will be fixed in future versions, but if that is the case, add circular dependency for coverage estimation. Importing a file with the following content will be enough.
```tact
contract A {
    receive() {
        let x = initOf B();
