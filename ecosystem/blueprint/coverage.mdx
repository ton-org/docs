...
<Aside type="note">
  This page explains how coverage is calculated for the Telegram Open Network Virtual Machine (TVM) assembly instructions. Path and source-line coverage are not implemented.
</Aside>

There are two main ways to calculate coverage of your `@ton/sandbox` tests.

## Collect coverage (easy)

<Aside
  type="note"
  title="Important"
>
  For this to work correctly, versions of `@ton/sandbox` (>= 0.37.2) and `@ton/blueprint` (>= 0.41.0) are required.
</Aside>

When using Blueprint, run:
...
## Collect coverage (customizable)

There might be some reasons why you don't want to use `--coverage`.
...
## Collect coverage for multiple contracts
...
<Aside type="note">
  To understand the TVM assembly output, read [TVM](../../ton/tvm).
</Aside>

## Limitations

Note that when the code of other contracts is stored directly in a contract's code ([Tact](../../language/tact) does that automatically if a contract system does not contain circular dependencies), that affects the overall code coverage percentage.
