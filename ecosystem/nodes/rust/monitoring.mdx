...
<Aside
  ...
  title="Important"
>
  The chart does not provide external metrics access out of the box. Securing an unauthenticated HTTP endpoint is an operator responsibility.
</Aside>
...
The Grafana dashboard is authored as TypeScript with Grafana Foundation SDK and generated to JSON. Dashboard source is available in [TON Rust Node Grafana source](https://github.com/RSquad/ton-rust-node/blob/master/grafana/src/index.ts). Generated output file name is `ton-node-overview.json`.
...
`PrometheusRule` resources can be created to trigger alerts based on TON node [metrics](/ecosystem/nodes/rust/metrics). Tested alert rule examples are not included yet.
