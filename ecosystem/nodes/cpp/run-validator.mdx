...
- 128 GB RAM
- 2 x 1.92 TB NVMe SSD (250k+ read IOPS, 83k+ write IOPS)
...
<Aside
  type="caution"
>
  After activation, verify the following in a [blockchain explorer](https://tonviewer.com/):
...
Set up monitoring dashboards for RAM, disk, network, CPU usage, and other metrics. For on-chain validator metrics, use [ton-validators-metrics](https://github.com/the-ton-tech/ton-validators-metrics). For system-level metrics, use Prometheus with node\_exporter alongside [MyTonCtrl Prometheus integration](/ecosystem/nodes/cpp/integrating-with-prometheus). For technical assistance, contact [@mytonctrl\_help\_bot](https://t.me/mytonctrl_help_bot).
...
For monitoring setup, see [Set up monitoring](/ecosystem/nodes/cpp/run-validator#5-7-set-up-monitoring).
...
- Check the elector contract status using a [blockchain explorer](https://tonviewer.com/).
