...
<CodeGroup>
  ...
  #     AppKit React      Core packages         TanStack Query React
  npm i @ton/appkit-react @ton/core @ton/crypto @tanstack/react-query
  ```

  ```shell title="TypeScript" icon="globe"
  #     AppKit      Core packages         TanStack Query Core
  npm i @ton/appkit @ton/core @ton/crypto @tanstack/query-core
  ```
</CodeGroup>

If there is an existing dApp that uses TON Connect UI libraries, migrate it to AppKit in a few steps:

- [from `@tonconnect/ui-react` (React)](#migrate-from-%40tonconnect%2Fui-react)
- [from `@tonconnect/ui` (Vanilla)](#migrate-from-%40tonconnect%2Fui)
...
[Basic setup](#initialization) allows for direct API requests but is insufficient for connecting TON wallets via the TON Connect protocol. For that, [configure a connector](#connectors) and host a public [app manifest file](/ecosystem/ton-connect/manifest#app-manifest).
...
<Steps>
  <Step
    title="Install AppKit and peer dependencies"
  >
    ```shell
...
<Steps>
  ...
  >
    ```shell
    npm i @ton/appkit @tanstack/query-core
    ```
  ...
  >
    ...
    const kit = new AppKit({
      connectors: [
        new TonConnectConnector({
          // Pass the existing TonConnectUI instance
          tonConnectUI
        }),
      ]
    });

    // Dynamic configuration, passing the existing TonConnectUI instance
  ...
  >
    Instead of hoisting everything under the instantiated `TonConnectUI` object, AppKit exposes numerous tree-shakable functions that replace and expand existing `TonConnectUI` functionality. Refer to [relevant how-to pages](#quick-start) for concrete examples.
  </Step>
</Steps>
