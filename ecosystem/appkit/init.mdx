...
If there is a dApp that currently uses [TON Connect UI libraries](#relation-to-ton-connect-libraries), migrate that dApp to AppKit in just a few steps:
...
<Steps>
  ...
  >
    Instead of consolidating everything within the instantiated `TonConnectUI` object, AppKit offers several tree-shakable functions that enhance and expand the capabilities of the existing `TonConnectUI` functionality. Refer to [relevant how-to pages](#quick-start) for concrete examples.
  </Step>
</Steps>
