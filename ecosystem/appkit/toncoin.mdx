...
Balances are returned as strings representing fractional Toncoin units. For example, `'0.1'` is a balance of 0.1 Toncoin or 10<sup>8</sup> nanoToncoin. Decimal precision of Toncoin is 9 digits, so the smallest possible balance is `'0.000000001` or 1 nanoToncoin.
...
<CodeGroup>
  ...
  export const BalanceCard = () => {
    const { data: fractionalToncoin } = useBalance();

    return (
      <p>TON wallet balance: {fractionalToncoin ?? '0'}</p>
    );
  };
  ```
  ...
  const fractionalToncoin = await getBalance(kit);
  if (fractionalToncoin) {
    console.log(
      'TON wallet balance in fractional Toncoin:',
      fractionalToncoin,
    );
  }
  ```
...
<CodeGroup>
  ...
  export const SendToncoin = () => {
    ...
    // For example, '0.1' Toncoin.
    const fractionalToncoin = '<FRACTIONAL_AMOUNT>';
