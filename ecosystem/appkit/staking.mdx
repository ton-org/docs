...
[Tonstakers](https://tonstakers.com) is the primary Toncoin liquid staking provider for AppKit. It is configured per network, similar to API clients:
...
<CodeGroup>
  ...
  export const StakingQuoteCard = () => {
    ...
    return (
      <div>
        ...
        <p>Stake amount: {quote?.amountIn} nanoToncoin</p>
        <p>Estimated output: {quote?.amountOut} nanoToncoin</p>
      </div>
    );
  };
  ```
  ...
  async function getStakeQuote(
    ...
    console.log('Stake amount:', quote.amountIn, 'nanoToncoin');
    console.log('Estimated output:', quote.amountOut, 'nanoToncoin');
