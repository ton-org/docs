...
1. The user's off-chain backend calls the API method of the data provider.
1. The provider responds with signed price data, including a timestamp till this data is valid.

3-4. The user sends a message to the on-chain contract that requires price data and other business logic.

5. The user contract sends an internal message to the oracle contract: "Verify that this price is correctly signed and valid".
5. The oracle contract verifies the signature, timestamp, and price feed ID. If valid, it sends a response message with the verified price data.
5. The user contract receives the response from the oracle contract, verifies the sender, and uses the provided data.
