...
Since TON execution model is asynchronous, the classic ways to interact with oracles (get methods during transaction) can't be applied here, you can learn about differences in the [Coming from Ethereum](/guidebook/from-ethereum) article. The pattern to get data from oracle that works the best is [Request-Response pattern](/techniques/carry-value) - you send internal message to oracle contract and verify the response, getting needed data.
