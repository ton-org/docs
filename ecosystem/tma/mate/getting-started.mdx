...
Before deploying the project assets, ensure that you have built your project and are not deploying source files. You should only deploy files that can be successfully opened by the user's browser.

Let’s assume you have a project with ID `48`. Also, you have a `dist` directory with all the built files. To deploy this directory to the CDN, use the following command:
...
> **Important**
>
> Note that your directory must include only the usual files and directories.
> All other types of files (symlinks, for example) are forbidden; the CLI tool will let you know
> about it.
