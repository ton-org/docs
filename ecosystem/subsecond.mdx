...
Sub-second finality is an upgrade in which transactions are processed and finalized in under 1 second. It combines faster block production with low-latency APIs. _The goal of this upgrade_ is to enable end-to-end user interactions to complete in under 1 second.

However, upgrading the blockchain does not automatically improve _the user experience_. To deliver this performance to users, ecosystem projects must adapt their apps to surface transaction updates in real time.

## Current status

<Aside type="note">
  Mainnet validators were updated on February 12, 2026 with Catchain 2.0. Sub-second upgrade is not yet enabled and will be activated after testnet validation. No activation date has been announced.
</Aside>

The testnet has been running stably since the January 23, 2026 update with sub-second upgrade active.
...
MyTonWallet and [tonscan.org](https://tonscan.org) have already switched to Streaming API v2 on both mainnet and testnet. This improves how quickly transaction updates appear in the UI by delivering events in real time, even though sub-second upgrade is not yet enabled on mainnet.
...
### For self-hosted nodes, liteservers, and TON Center instances
...
1. Subscribe to the sender/recipient address through Streaming API before or immediately after sending.
1. Receive `pending` → show "processing" indicator.
1. Receive `confirmed` → optionally show optimistic success.
1. Receive `finalized` → show confirmed success, update state.
1. Receive `trace_invalidated` → discard cached trace, check status manually.
...
- [Public support chat](https://t.me/subsecond_upgrade) for sub-second update
