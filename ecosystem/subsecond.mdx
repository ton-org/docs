...
|---------------------| --- | --- | --- | --- |
| Current mainnet     | ~2.5 seconds | ~0.4 BPS | ~30–100 ms | ~10 s |
...
### Self-hosted lightserver updates

- If a lightserver node is self-hosted, ensure it is updated before mainnet rollout.
...
- SSE connections receive automatic server-side keepalives (`: keepalive`) every 15 seconds; no client action required.
