...
An agent that already has at least one user-created wallet can create additional child wallets from that wallet. In that case, sender verification uses the original operator public key of the parent user-root wallet.

Child wallets created by an agent cannot create additional child wallets. Thus, agents can only create child wallets from the user-created wallets. If a deploy request comes from an agent-created wallet, the contract rejects it.

On the other hand, users can indefinitely deploy new wallets and form child wallet chains. Further, users can revoke access or withdraw funds from any deployed wallet at any time using the dashboard.
