...
`ton_proof` lets your backend verify that a user actually owns the wallet they connected. The client signs a payload with the wallet's private key, your server checks the signature.
...
After wallet connection, the client receives a `TonProofItemReplySuccess` object and sends it to your backend:
...
- **AppDomain** — your app domain prefixed with its length. `lengthBytes` is a 32-bit little-endian integer.
...
Your backend hashes the message, prepends `0xffff ++ "ton-connect"`, hashes again, and verifies with the user's public key.
