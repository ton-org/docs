...
Before you will be able to recieve and handle wallet events you need to setup events handler and enstablish a connection with dApp
...
try await kit.remove(eventsHandler: eventsHandler)
...
To establish a connection with dApp you need to get a URL for connection request from any source (copy/paste, QRCode, depplink and etc.).

Once you have a connection URL you need to send it to Wallet Kit to perform connection request.

```swift
try await kit.connect(url: <connection url>)
```

Once connectiong request is performed, a connection event will be send to your events handler.
...
// To approve
try await request.approve(walletAddress: <wallet address>)
