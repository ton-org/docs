...
<Aside>
  The API client in use is determined by the optional [`apiClient` configuration parameter](./init#param-api-client) during [WalletKit initialization](./init#initialization). By default, the [TON Center APIs](/ecosystem/api/toncenter/overview) are used, which provide rich emulation capabilities.
</Aside>
