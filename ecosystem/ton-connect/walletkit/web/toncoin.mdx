...
Therefore, the most practical use of one-off balance checks is right before approving a transaction request. At this point, the actual wallet contract balance cannot be less than the checked amount, though it might be higher if new funds arrived right after the check.
...
const stopMonitoring = startBalanceMonitoring(
  ...
  // an existing function in your wallet service that refreshes the
