...
/**
 * Starts the monitoring of a given wallet id,
...
```ts title="TypeScript"
kit.onTransactionRequest(async (event) => {
  if (event.preview.data.result === 'success') {
...
<Aside>
  Emulation uses the API client configured during [WalletKit initialization](/ecosystem/ton-connect/walletkit/web/init#param-api-client). The default TON Center APIs provide rich emulation capabilities.
</Aside>
...
```ts title="TypeScript" expandable
// An enumeration of various common error codes
import { SEND_TRANSACTION_ERROR_CODES, type TransactionTraceMoneyFlowItem } from '@ton/walletkit';
...
kit.onTransactionRequest(async (event) => {
  try {
    ...
    if (isEmulationSuccessful) {
      const transfers = event.preview.data.moneyFlow?.ourTransfers || [];
...
async function sendToncoin(
  // Sender's TON wallet id as a string
  walletId: string,
...
Alternatively, you can send the transaction directly without calling `handleNewTransaction()` and without triggering the `onTransactionRequest()` handler:
