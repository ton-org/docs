...
<Aside>
  Despite this check, the transaction may still fail due to insufficient gas if the wallet contract is managed by another wallet service or outside WalletKit. However, in that case, the message will not be sent, and the funds will either remain in the wallet contract or revert to it immediately.
</Aside>
