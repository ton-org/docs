...
<Aside
  type="caution"
>
  To avoid triggering the `onTransactionRequest()` handler and send the transaction directly, use the `sendTransaction()` method of the wallet instead of the `handleNewTransaction()` method of the WalletKit:

  ```ts title="TypeScript"
  const fromWallet = kit.getWallet(walletId);
  if (fromWallet) {
    // This will not route into the normal flow,
    // and will not trigger the onTransactionRequest handler,
    // making this transaction be made directly.
    await fromWallet.sendTransaction(tx);
  }
  ```

  Do not use this approach unless it's imperative to complete a transaction without the user's direct consent.
</Aside>
