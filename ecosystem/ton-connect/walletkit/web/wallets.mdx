...
1. Then, select a wallet adapter — it's an implementation of the `WalletInterface` type for a particular TON wallet contract. Currently, WalletKit provides two: `WalletV5R1Adapter` (recommended) and `WalletV4R2Adapter`.
   ...
   const walletAdapter = await WalletV5R1Adapter.create(signer, {
     ...
     // since it is used to make their addresses unique.
     // Same IDs will lead to the wallets of the same address,
     // i.e., to being the same smart contract.
     walletId: 0,
   });
   ```
...
When creating many wallet adapters, remember to specify a different `walletId` for each. This is because on TON, addresses are deterministically derived from a smart contract's initial code and data.

Setting unique IDs for the wallet's initial data is essential to prevent address collisions, which would result in you creating the same contract repeatedly.
