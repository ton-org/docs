...
Normalization is a standardization process that converts different external-in message representations into a consistent format.

To address this, the ecosystem defines a standard that ensures consistent hash calculation. The normalization rules are specified in detail in the [TEP-467 proposal](https://github.com/ton-blockchain/TEPs/pull/467). This PR is not yet merged, but message lookup by its normalized hash is already implemented in most TON [RPC providers](/ecosystem/rpc/overview).
