...
>
  The `jetton/wallets` endpoint sorts by balance instead of logical time. Results may be inconsistent when paginating with `limit` and `offset` if balances change between requests.
</Aside>
