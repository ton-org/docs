...
- `"actions"` — subscribe to certain actions by setting `action_types` to either of: `"ton_transfer"`, `"jetton_transfer"`, `"nft_transfer"`.
...
<Aside type="note">
  Use `action_types` when subscribing via [SSE](/ecosystem/api/toncenter/streaming/sse) or a [WebSocket](/ecosystem/api/toncenter/streaming/wss) to filter received `actions`.
</Aside>
