...
description: "Reference for the @type discriminator values returned by TON Center API v2."
---
...
The `@type` field acts as a **discriminator**: when a response can return different object shapes, the `@type` value tells you which fields to expect. This is useful for type-safe deserialization in statically typed languages.
...
When you query account information, the `account_state` field uses `@type` to indicate which kind of contract is deployed. The TL schema defines these as variants of `AccountState`:
...
| `ext.accounts.walletInformation` | `WalletInformation`          | Wallet-specific: type, seqno, wallet\_id (TON Center extension) |
...
For background on the TL-B format used across the TON ecosystem, see the [TL-B overview](/languages/tl-b).
