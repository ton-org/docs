...
Return a `2xx` response after successful processing and for validation failures that are intentionally rejected (for example, mismatched amount or unknown reference) to prevent retries. Only return non-`2xx` for requests that fail authentication or are malformed.
