...
Receive real-time notifications about transfer events. TON Pay uses webhooks to notify your application in real-time about transfer events. When a payment is completed, TON Pay sends an HTTP POST request to your configured webhook URL with event details.
...
<Step title="Enter the webhook URL">
  ...
  https://yourdomain.com/webhooks/tonpay ```
</Step>
...
<Warning>
  Always return `200 OK` for validation failures (amount mismatch, unknown order) to prevent retries, and log the failure.
</Warning>

Return a `2xx` response after successful processing and for validation failures that you intentionally reject (for example, mismatched amount or unknown reference) to prevent retries. Only return non-`2xx` for requests that fail authentication or are malformed.
