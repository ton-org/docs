...
<Aside>
  The webhook URL must be publicly accessible and accept `POST` requests from TON Pay backend.
</Aside>

1. A transfer is completed on-chain and indexed by TON Pay.
1. TON Pay generates a webhook payload and signs it using [HMAC-SHA256](https://en.wikipedia.org/wiki/HMAC) with the API key.
...
1. Verify the signature and process the event. Return a `2xx` status code to acknowledge receipt.
...
1. Navigate to <kbd>Developer</kbd> → <kbd>Webhooks</kbd> sections.
1. Enter the webhook endpoint URL (must be HTTPS in production). Replace `<WEBHOOK_URL>` with the HTTPS endpoint that receives TON Pay webhooks.
...
<AccordionGroup>
  <Accordion
    title="Validate critical fields"
  >
  ...
  <Accordion
    title="Return `2xx` after validation and process asynchronously"
  >
    Acknowledge receipt after validation, then process in the background to avoid timeouts.
  ...
  <Accordion
    title="Log all webhook attempts"
  >
...
<AccordionGroup>
  ...
  <Accordion
    title="Amount or asset mismatch"
  >
    Possible causes:
  ...
  >
    ...
    - **Return `2xx`** after validation passes.
