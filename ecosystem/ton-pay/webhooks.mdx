...
<Aside
  ...
  title="Type safety"
>
  Webhook types are exported from [`@ton-pay/api`](link-rot). Use `WebhookPayload` for type safety in webhook handlers.
</Aside>
...
<Aside type="note">
  Return a `2xx` status code only after the webhook is processed successfully. Returning success prematurely can result in missed events.
</Aside>
...
- Use HTTPS endpoints for webhooks in production. TON Pay may reject HTTP endpoints for security.
...
<Accordion title="If webhooks are not received">
  ...
  1. Ensure your endpoint returns a `2xx` status code for valid requests.
