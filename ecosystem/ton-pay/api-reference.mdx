...
TypeScript exports for TON Pay SDK packages [`@ton-pay/api`](https://www.npmjs.com/package/@ton-pay/api) and [`@ton-pay/ui-react`](https://www.npmjs.com/package/@ton-pay/ui-react)
...
- `pay(getMessage)`: receives `senderAddr`, requests `{ message }` from the factory, and sends via TON Connect. Resolves `{ txResult, ...factoryReturn }`.

### TonPayButton

Prebuilt button that handles connect or disconnect flow and calls `handlePay`.
