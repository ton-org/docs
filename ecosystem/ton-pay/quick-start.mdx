...
<Steps>
  ...
  >
    ...
    Store the `reference` as soon as `createTonPayTransfer` returns it. To resume status checks after a reload, persist it outside component state, for example on the server.
