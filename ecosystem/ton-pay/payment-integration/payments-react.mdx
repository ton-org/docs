...
When using `useTonPay` with server-side message building, [the API key can be included](/ecosystem/ton-pay/payment-integration/payments-react#api-key-configuration) in the backend endpoint:
...
A complete description of testnet configuration, including obtaining testnet TON, testing jetton transfers, verifying transactions, and preparing for mainnet deployment, is available in the [Testnet configuration](/ecosystem/ton-pay/payment-integration/\<TESTNET_CONFIGURATION_DOC>) section.
...
NEXT_PUBLIC_RECIPIENT_ADDRESS=<TESTNET_RECIPIENT_ADDRESS>
```
