...
- If transaction fails with `Wallet is not connected`, ensure the wallet is connected before calling `sendTransaction`:
...
- If `sendTransaction` throws `Invalid address format`, verify that:
...
- If transaction expires before user signs. The `validUntil` timestamp is too short or the user is taking too long to sign. Increase the validity period:
...
- If `onStatusChange` not triggering, ensure you're properly subscribing to status changes and that the subscription persists:
...
- If multiple wallet connection prompts. This can happen if you're calling `connectWallet()` multiple times. Implement connection state tracking:
