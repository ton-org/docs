...
<Aside type="note">
  Single nominator pools are excluded from this rule. They fall through to the percentage-based calculation (Priority 5) because single nominator pools behave like direct staking economically.
</Aside>
...
| ---------------------------------- | ------------- | ------------------------------------------------------------- |
| Direct staking (explicit argument) | 10            | Minimal fees for a single stake transaction.                  |
| Direct staking (saved fixed stake) | 0             | The stored value is used as-is; only validation guards apply. |
...
1. Falls through to percentage-based calculation (Priority 5).
...
1. Result: **499,980 TON** is submitted. The node participates in this round but may miss the next one because all funds are locked.
