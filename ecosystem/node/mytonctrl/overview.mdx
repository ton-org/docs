...
| `-h` | Prints installer help and exits. | Sanity-check available options.
...
Set these variables before running `install.sh` (for example `ARCHIVE_TTL=864000 sudo bash install.sh -d`) to preconfigure node behaviour:
