...
| `single-nominator` | Orbs's single nominator pools. |
...
## disable_mode
**Purpose:** Deactivate one of the pluggable modes (validator, pools, liteserver, etc.).
...
- After a successful change the console session exits so that dependant services can restart with the new configuration.
...
- Lists both core modes (validator, liteserver, pools) and optional services (alert-bot, prometheus, collator, etc.).
