...
| Mode               | Description                                                                                                                              |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `validator`        | Validator functions. Participates in elections and staking. If pool and liteserver modes are disabled, stakes from the validator wallet. |
...
- Runs a bundled benchmark script with elevated privileges. The script executes two Flexible I/O Tester (FIO) scenarios (random 4K with queue depths 64 and 1) and a RocksDB stress test.
...
- Without arguments the command aggregates live data from the validator, lite-client, system metrics, and Git repositories. It prints:
  ...
  - Local node metrics: validator index and efficiency, Abstract Datagram Network Layer (ADNL) addresses, wallet balances, CPU/memory/swap/disk utilization, network throughput, service uptimes (`MyTonCore`, `validator`, `btc_teleport` when applicable), virtualization detection, version information for MyTonCtrl, TON binaries, and optional BTC Teleport.
...
- Useful for auditing which modules are active before enabling or disabling more features.
