...
- Reuses configuration stored in the local database (stake amount, elector parameters, and related settings).
...
- Collects validator lists for the current and previous rounds, locates the local Abstract Datagram Network Layer (ADNL), and prints block production statistics.
...
- With `--json`, returns the parsed collator list in JavaScript Object Notation (JSON) format, matching the structure passed to `set-collators-list`.
