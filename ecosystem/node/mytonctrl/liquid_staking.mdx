...
**Behavior**
- Calls the controller’s `update_validator_set` method so it picks up the latest validator ADNLs from the pool.
...
**Behavior**
- Runs the controller’s `get_validator_controller_data` method and prints the resulting fields (state, approval, borrowed amount, stake timing, etc.).
