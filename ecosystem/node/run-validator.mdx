...
- Validator node spends up to 200 TON monthly.
...
2. Run validator installation, installation takes approximately 20 minutes:
...
3.After installation, launch MyTonCtrl with command `mytonctrl`. To check synchronization status using the command:
...
4. Wait until the node is fully synchronized approximately 3 hours (depends on network connection). Check the field `Local validator initial sync status: x`, the value x should become less than 20.
...
Once you install node MyTonCtrl try to perform network actions with node. If you notice this notification, you need to configure your network additionally.

1. Print your configuration file:
```bash
cat /var/ton-work/db/config.json
```
2. Find TCP `control.port` within output
 ...
 "control" : [
      {
         ...
         "id" : "w6P9N..............................H/k=",
         "port" : 60055, #this port should be open
         "allowed" : [
            {
...
3. Find UDP `engine.addr` port number
...
```
4. Make sure that `engine.addr` ports opened with [www.ipvoid.com/udp-port-scan](https://www.ipvoid.com/udp-port-scan/) and [www.ipvoid.com/port-scan](https://www.ipvoid.com/port-scan/).

5. If you find out port filtered, open this port:
...
Top up this address with enough coins to cover operational costs,
ensure you have at least 200 TON/month in your validator wallet for operation fees.

2. Credit validator wallet
