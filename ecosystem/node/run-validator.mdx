...
### 2.6 Test disk latency

Disk latency is the common bottleneck. Benchmark storage with `benchmark` command:
...
<Image
  src="/resources/images/nodes/run-validator/run-validator-27.png"
  darkSrc="/resources/images/nodes/run-validator/run-validator-27.png"
  alt="Test disk latency"
/>

Make sure your `read iops` and `write iops` exceed 64k.

### 2.7 Verify node synchronization
...
Create a [single nominator pool](https://docs.ton.org/v3/documentation/smart-contracts/contracts-specs/single-nominator-pool) for secure stake manegment. As an `owner-address`, specify the beneficiary wallet address that will stake owned funds and receive rewards.
...
### 4.1 Find two las cycles

Get `min_stake` and `max_stake` from [Validation API](https://elections.toncenter.com/getValidationCycles?return_participants=false\&offset=0\&limit=2) for two last cycles.

<Accordion
  title="See expected result..."
>
  ![run-validator-28.png](..%2F..%2Fresources%2Fimages%2Fnodes%2Frun-validator%2Frun-validator-28.png)
</Accordion>
...
av_min_stake = 701 812
```

```bash
// largest within 2 008 570 TON and 2 024 432 TON
av_max_stake = 2 008 570
```

### 4.3 Calculate effective stakes

<Aside type="tip">
  _Effecitive_ stands for winning elections for both odd and even cycles.
</Aside>
...
![run-validator-29.png](..%2F..%2Fresources%2Fimages%2Fnodes%2Frun-validator%2Frun-validator-29.png)
