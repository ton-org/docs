...
>
  ...
  # Check clang version
  clang --version
  # If it is version 16, don't need to do the steps below.
...
Configure network on the server according to the following:
...
Find wallet named `validator_wallet_001` and its address.
...
Make backup of the validator wallet secret key:
...
>
  Insert single nominator pool address to `SINGLE_NOMINATOR_ADDRESS` and send this message from the owner's wallet.
...
Once deposit restored (except 1 TON reserve on single nominator pool) to owner wallet, that means all system set securely and ready for real stake:
...
Top up single nominator pool with optimal stake for validation. In the next elections, MyTonCtrl will automatically use this pool for staking.
...
Make sure pair stakes appropriately circulated, then focus on monitoring validator health and updates.
...
Set up alerting in MyTonCtrl to get notification of critical issues with validator. For more information, see [MyTonCtrl private alerting bot](https://old-docs.ton.org/v3/guidelines/nodes/maintenance-guidelines/mytonctrl-private-alerting) or check [MyTonCtrl public alerting bot](https://t.me/validators).
...
<Aside type="tip">
  ...
  Unlike `checkloadall`, which only works on validation rounds, this API allows to set any time interval to analyze a validator's efficiency.
</Aside>
