...
2. After installation, launch MyTonCtrl with command `mytonctrl`. To check synchronization status using the command:
...
3. Make sure your ports allowed. Print your configuration file:
```bash
cat /var/ton-work/db/config.json
```

4. Find UDP `engine.addr` port:
...
5. Make sure UDP `engine.addr` ports allowed with following command:

```bash
sudo ufw status
```
![run-validator-10.png](..%2F..%2Fresources%2Fimages%2Fnodes%2Frun-validator%2Frun-validator-10.png)

6. If you find out port filtered (absent in allowed), open this port:
...
3. Explore your validator walet by address with [tonviewer](https://tonviewer.com/):
...
<Accordion title="If you’ve already created a pool...">
