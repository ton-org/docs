...
Launch MyTonCtrl with the command `mytonctrl`. Then, check synchronization status using the command:
...
<Aside type="tip">
  Initially, the status shows "Local validator initial sync status: downloading blockchain state xxxx", which indicates that the node has started synchronization.
</Aside>
...
max_expected_effective_stake = 4017140
```
