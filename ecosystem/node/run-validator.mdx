...
3.After installation, launch MyTonCtrl with command `mytonctrl`. Wait until the node is fully synchronized approximately 2 hours(depends on network connection). To check synchronization status using the command:
...
4. Check the field `Local validator initial sync status: x`, the value should become less than 20.
