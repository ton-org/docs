...
<Aside type="tip">
  Initially the status shows _Local validator initial sync status: downloading blockchain state xxxx_, which indicates that the node has started synchronization.
</Aside>
...
<Aside
  type="danger" title="Funds at risk"
>
 Use a cold wallet (e.g., Ledger) for the `owner-address` and store its seed phrase securely to keep funds safe. If access to the owner wallet is lost, the access to stake and rewards funds cannot be recovered.
</Aside>
