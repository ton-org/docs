...
Refer to the [MyTonCtrl overview](/ecosystem/node/mytonctrl/overview#install-mytonctrl) for installer flags and environment variables when you need unattended deployments.
...
Need more wallet operations? See the [wallet command reference](/ecosystem/node/mytonctrl/wallet).
...
- Use the private alert bot once your node is stable: `MyTonCtrl> enable_mode alert-bot` then configure credentials per the [alerting guide](/ecosystem/node/mytonctrl/alerting).
...
Once comfortable with these workflows, dive into the [detailed command references](/ecosystem/node/mytonctrl/overview) for advanced automation.
