...
Installation and configuration expose validator wallets and secrets. Record seed phrases offline, keep terminals private, and rehearse new steps on testnet before running them on mainnet funds.
:::
...
:::caution Use a dedicated operator account
Install MyTonCtrl with `sudo`, then operate it as the non-root user the installer provisions (default `mytonctrl`). Mixing users leads to permission issues under `/var/ton-work` and breaks systemd service ownership.
:::
...
Refreshing the global config or restarting systemd units interrupts validator duties. Schedule these changes during maintenance windows, snapshot the node first, and validate them on testnet whenever possible.
:::
...
```bash
wl
nw <WORKCHAIN_ID> <WALLET_NAME>
iw <WALLET_ADDRESS> <SECRET_KEY_B64>
aw <WALLET_NAME>
ew <WALLET_NAME>
mg <WALLET_NAME> <DESTINATION_ADDRESS> <TON_AMOUNT>
mgtp <WALLET_NAME> <DESTINATION_ADDRESS> <TON_AMOUNT>
```
...
`ew` prints the Base64 secret key. Run it only on secure terminals, store the output offline, and practice with testnet wallets before exposing production keys.
:::
