...
Fift is a stack-based programming language with The Open Network (TON)-specific features that can work with cells. TON Virtual Machine ([TVM](/ton/glossary#ton-virtual-machine-tvm)) assembly is another stack-based language designed for TON that also handles cells. What's the difference between them?
...
Think of Fift as a teaching language where you can introduce high-level concepts to a learner. Just as a trainee programmer gradually absorbs and applies new concepts, Fift allows you to define custom commands and abstractions. The `Asm[Tests].fif` file demonstrates this - it's essentially a collection of TVM opcode definitions.
...
### (Fift) Include large BoCs in contracts
...
### (TVM assembly) Convert integers to strings
...
### Use efficient modulo multiplication (TVM assembly)
...
The `x{A988}` opcode implements an optimized division operation with built-in multiplication (as specified in [instruction A988 (MULMOD)](/tvm/instructions#a988-mulmod)). This specialized instruction directly computes just the modulo remainder of the operation, skipping unnecessary computation steps. The `s,` suffix then handles the result storage - it takes the resulting slice from the stack's top and efficiently writes it into the target builder. Together, this combination delivers substantial gas savings compared to conventional approaches.
