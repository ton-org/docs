...
| **`*>>c`** | _`(x y z – q)`_ | similar to `*/c`, but with division replaced with a right shift. Computes $q := \lceil \fraq{xy}{2^z} \rceil$ for $0 ≤ z ≤ 256$. Equivalent to `1<< */c`.
| **`*>>r`** | _`(x y z – q)`_ | similar to `*/r`, but with division replaced with a right shift. Computes $q := \lceil \fraq{xy}{2^z} \rceil + 1/2$ for $0 ≤ z ≤ 256$. Equivalent to `1<< */r`.
