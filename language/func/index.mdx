...
The method uses the `impure` and `inline` [specifiers](functions#specifiers), which are flags that tell the compiler to process 
...
with the single-line [comment]() `;; nobounce`. Then, function [`store_slice`]() stores the receiving address, followed by the amount to 
send with function [`store_coins`](). The builder finalizes the cell with a call to the [`end_cell`]() function.

Finally, function [`send_raw_message`]() sends the message The ready-to-send message i
