...
and the `amount` to send. The method does not return a value, indicated by [`()`](func/types#tensor-types) to the left of the method name `send_money`.
The method uses the `impure` and `inline` [specifiers](func/functions#specifiers), which are flags that tell the compiler to process
...
with the single-line [comment](func/comments) `;; nobounce`. Then, function [`store_slice`](/language/func/stdlib#store-slice) stores the receiving address,
...
Developers and engineers can use its bitcode, which is structured as a [tree of cells](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage/) like all data in the TON Blockchain, to create smart contracts or run it on a local instance of the TVM.
...
<Columns cols={3}>
  <Card title="TON Blockchain course in English" href="https://stepik.org/course/176754/promo" arrow />

  <Card title="(Chinese)" href="https://stepik.org/course/201638/promo" arrow />

  <Card title="(Russian)" href="https://stepik.org/course/201855/promo" arrow />
</Columns>
