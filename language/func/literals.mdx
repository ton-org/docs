...
- It is **not** the name of a [built-in](reserved-words#builtins).
...
<Aside>
  ...
  [built-in with a non-symbolic name](reserved-words#built-ins-with-non-symbolic-names). 
</Aside>
...
These are the possible string suffixes:

- without suffix - Produces a slice from the ASCII string. The contents of the slice is the binary code of the ASCII string.
- `s`— Interprets the string as an hexadecimal number and produces a slice having the binary representation of the hexadecimal number. 
    If the string is not an hexadecimal number, the compiler signals an error.
- `a`— Interprets the string as an address and creates a slice containing a [`MsgAddressInt` structure]() from the address.

   <Aside type="danger">
       MsgAddressInt structure link is missing!!
   </Aside>

- `u`— Produces the integer representation of the binary code of the ASCII string.
...
all surrounded by triple quotes `"""`. The triple quotes syntax also supports the above suffixes. 

**Examples**

The following strings (with suffixes) produce these results:

```func
const c1 = "hello";    ;; x{68656c6c6f} (slice constant), where 68656c6c6f 
                       ;; is the slice contents in hexadecimal, representing
                       ;; the ASCII code of the string in hexadecimal.

const c2 = "abcdef"s;  ;; x{abcdef} (slice constant), where abcdef is the 
                       ;; slice contents in hexadecimal. 
                       ;; Note that abcdef is a valid hexadecimal number.

const c3 = "Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF"a;
                       ;; x{9FE6666666666666666666666666666666666666666666666666666666666666667_} (slice constant)
...
const c4 = "NstK"u;    ;; 1316189259 (integer constant)
                       ;; Decimal representation of the ASCII code

const c5 = "transfer(slice, int)"h;  ;; 2053302440 (integer constant)

const c6 = "transfer(slice, int)"H   ;; 55356924298749527416066000120313684523410504308849542670649639903159354505593
                                     ;; (integer constant)

const c7 = "transfer(slice, int)"c   ;; 2235694568 (integer constant)
```
