...
The compiler automatically checks files for multiple inclusions. By default, the compiler ignores redundant inclusions if the same file is included more than once. A warning is issued if the verbosity level is 2 or higher.
...
You can specify version constraints using the following operators:
...
- `^a.b.c`—Requires the major compiler version to be equal to the `a` part, and the minor is no lower than the `b` part.
  - `^a.b`—Requires the major compiler version to be equal to the `a` part, and the minor is no lower than the `b` part.
...
In the following example, `x` contains the initial value of `ds`:
...
- The first argument of `f` contains the initial value of `ds`.
- The second argument contains the 8-bit-modified value of `ds`.
...
All `asm` reordering occurs only after computation.

<Aside type="note"> `#pragma compute-asm-ltr` applies only to code after the directive in the file. </Aside>
