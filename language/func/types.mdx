...
- `B` is the output type, which is called co-domain.
...
Like in functional programming, it is possible to declare functional types which have in their domain and co-domain other functional types. 
For example, `(int -> int) -> int` is a function with domain `int -> int` and co-domain `int`. 
Similarly, `int -> (int -> int)` is a function with domain `int` and co-domain `int -> int`.
