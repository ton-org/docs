...
- An [operator](/language/func/operators)
...
In nested scopes, a new variable with the same name can be declared, as in C:
...
The following way of calling `store_uint` (here, [`begin_cell`](/language/func/stdlib#begin_cell) creates a new builder and has type `() -> builder`):
