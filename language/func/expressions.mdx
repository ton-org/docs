...
This article explains how expressions are constructed in FunC. 
An expression is a combination of literals, variables, operators, and function calls that produce a value when evaluated. 
...
As a general rule, all sub-expressions inside an expressions are evaluated from left to right,
...
the [non-modifying notation](#non-modifying-notation), and the [modifying](#modifying-notation) one, which are explained next.
