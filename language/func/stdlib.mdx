...
>
  ...
  1. Message exceeds size limits, see the [maximum message bit and cell size](/ton/limits#message-and-transaction-limits) for details.
...
You can view a detailed example [here](/ton/messages/internal-messages).
...
>
> 1. **As a `Maybe cell (c^?)`**, which is either a cell that contains a serialized `Hashmap(n, X)` same as above or `null`, indicating an empty dictionary. See [null values](/language/func/types#null-values) for details. When using this format, the dictionary is typically referred to as `D`.
>
