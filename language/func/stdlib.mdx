...
For more details on polymorphic functions, refer to the [Polymorphism with forall](/language/func/functions#polymorphism-with-forall) section.
...
Extracts the head and tail of a lisp-style list. It can be used as a [(non-)modifying method](/language/func/statements#methods-calls).
...
The [c7 special register](/tvm/registers) holds useful data about smart contract execution.
...
<Aside title="+2 flag">
  ...
  1. Message exceeds size limits (see the [Message size](/ton/messages/sending-messages#message-size) section for details).
  ...
  1. The outbound message contains invalid libraries in `StateInit`.
...
You can view a detailed example [here](/ton/messages/sending-messages#example-jetton-transfer-pitfall).
...
See also [set_c3](/language/func/stdlib#set_c3).

### Random number generator primitives

The pseudo-random number generator relies on a random seed, a 256-bit unsigned integer, and sometimes additional data stored in [c7](/tvm/registers).
...
<Aside type="caution">
  ...
  Remember that the random numbers generated by the functions below can be predicted unless additional techniques are used. See [random number generation](/v3/guidelines/smart-contracts/security/random-number-generation) for more info.
...
It can be used as a [modifying method](/language/func/statements#modifying-methods).
A primitive _preloads_ data when it returns only the extracted data, leaving the original slice unchanged. It can be used as a [non-modifying method](/language/func/statements#non-modifying-methods).
...
A primitive _stores_ a value `x` in a builder `b` by returning a modified version `b'` with `x` added at the end. It can also be used as a [non-modifying method](/language/func/statements#non-modifying-methods).
...
Checks whether the given `cell c` is `null`. In most cases, a `null` cell represents an empty dictionary. FunC also includes a polymorphic `null?` built-in function. See [built-in](/language/func/built-ins#other-primitives) for more details.
...
>
> 2. **As a `Maybe cell (c^?)`**, which is either a cell that contains a serialized `Hashmap(n, X)` same as above or `null`, indicating an empty dictionary. See [null values](/language/func/types#null-values) for details. When using this format, the dictionary is typically referred to as `D`.
>
...
Additionally, some primitives have counterparts prefixed with `~`, enabling them to be used as [modifying methods](/language/func/statements#modifying-methods).
...
Marks a variable as used, ensuring the code generated isn't removed during optimization, even if it's not impure. See [Impure specifier](/language/func/functions#impure-specifier) section.
