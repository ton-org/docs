...
**Reference:** [`Repeat loop` in docs](/language/func/statements#repeat-loop)
...
**References:**
- [`slice_data_empty?()` in docs](/language/func/stdlib#slice_data_empty)
- [`store_slice()` in docs](/language/func/stdlib#store_slice)
- [`store_ref()` in docs](/language/func/stdlib#store_ref)
- [`begin_cell()` in docs](/language/func/stdlib#begin_cell)
- [`end_cell()` in docs](/language/func/stdlib#end_cell)
...
- You can find more details in the [documentation](/ton/tblkch#message-layout). A direct link to the [layout](/ton/tblkch#message-layout) is also available.
...
**Note:**
- In this example, we use the literal `a` to obtain an address. More details on string literals can be found in the [documentation](/language/func/literals#string-literals).
- The example uses [`mode 3`](/ton/messages/sending-messages#mode-3), which ensures the contract deducts the specified amount while covering the transaction fee from the contract balance and ignoring errors.
...
**Reference:** [`Message layout` in docs](/ton/messages/sending-messages)
...
**References:**
- [`Message layout` in docs](/ton/messages/sending-messages)
...
**Reference:** [`Internal messages` in docs](/ton/messages/internal-messages)
...
- [Is it possible to redeploy code to an existing address, or does it have to be deployed as a new contract?](/resources/faq#is-it-possible-to-redeploy-code-to-an-existing-address)
