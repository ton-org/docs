...
;; Here, x has value 2.
```
although `x` is incremented to `1` inside the `try` block, the modification is **rolled back** due to the exception produced by the [`throw`]() function. 
...
In the above example, [`throw_arg(-1, 100)`]() produces an exception with error code `100` and exception parameter `-1`.
