...
- a convenient `unboc` disassembler of a contract's code compiled into a [bag of cells](/ton/cells/bag-of-cells) `.boc` format.
...
- BoC Disassembler (`unboc`) — Disassembler for [`.boc`](/ton/cells/bag-of-cells) files. Ships as a standalone binary with the compiler. You can invoke it by running `npx unboc` in your Tact projects.
