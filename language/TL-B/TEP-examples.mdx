...
= InternalMsgBody;
```
Thus, a binary string corresponds to this TL-B scheme if it sequentially contains.
- A binary representation of the hexadecimal number `0x5fcc3d14`.
...
This field has an illustrative TL-B scheme that should be analyzed separately.
```tlb
var_uint$_ {n:#} len:(#< n) value:(uint (len * 8)) = VarUInteger n;
```
This combinator is parameterized by a natural number `n`, represented in curly brackets. For a given `n`, `VarUInteger n` 
describes the serialization of a natural number `m`, which in its binary representation contains no more than `n * 8` bits.

First, the number of bytes required for writing `m`, that is called `len`, is serialized on `LenBits(n-1)` bits. Then `m`
itself is serialized as a `uint` on `len*8` bits. Thus, the size of the serialization of a particular `m` through the
...
starts with `0`, then the remaining bits and references stored in the current cell. Otherwise, they are stored in a separate cell.
...
- next, the 8-bits flag which is currently bounced by `1`;
