...
Each line of a TL-B file is either a TL-B scheme (that is, type declaration), a comment, or a blank line.
...
![TL-B scheme examples diagram](/resources/images/tlb-scheme.svg)
...
- **Field definitions**, each of which consists of
...
- **Constraints**: optional expressions that restrict values which are instances of `Nat` type.
...
- **Parameter declarations**: declare fields of types `#` (natural numbers) or `Type` (types of types) that may be used as parameters for parameterized types.
...
- **Combinator name**: the right side of a TL-B scheme that represents the name of the defined combinator. It can be parameterized.
...
From a high-level perspective, the **right-hand side** of each _scheme_ is a type, either simple (such as `Bit` or `True`) or parameterized (such as `Hashmap n X`) and
...
If a tag is not explicitly provided, TL-B parser must compute a default 32-bit constructor tag by hashing with
[cyclic redundancy check (CRC32)](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) the text of the scheme with `| 0x80000000` defining this constructor in a certain fashion. Therefore,
...
For example, each address in TON could be either an internal or an external address; see [addresses — general information](../../ton/addresses/addresses-general-info).
...
It then understands that this address is further serialized as `addr_std` and continues to parse our string relative to the fields defined in this constructor.
...
For example, consider the following scheme, that represents a stack integer value:
...
- `type-expr` is the field's type. Can be a simple type, a parameterized type with appropriate arguments, or a more complex expression.
...
For a real-world example, consider the following `McStateExtra` combinator that describes data stored in each masterchain block.
...
as a field of the combinator. Only identifiers of the `Nat` and `Type` types can be parameters.
...
A good real-world example of parameterized types is the definition of TVM tuples:
...
For a detailed explanation of how it works, see the [complex and non-trivial examples page](./complex-and-non-trivial-examples).

#### Special

TVM allows the following types of cells:
...
Some fields may be **implicit** (the negate operator is written as a tilde `~`). These fields are defined within curly brackets `{}`, as constraints and parameters of the parameterized types,
...
The negate operator is written as a tilde `~`.
...
For a detailed explanation of how these combinators are deserialized, see the [complex and non-trivial examples page](/language/TL-B/complex-and-non-trivial-examples).
