...
- **Constructor** that is consist of
...
Below, we gradually describe each component of TL-B shemes.
...
[crc32 algorithm](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) the text of the scheme with `| 0x80000000` defining this constructor in a certain fashion. Therefore, 
...
For example, each address in TON could be either an **internal** message or an **external**, see [general info page]().
...
```
When parsing the binary string `10...` that should be an instance of `MsgAddress` combinator, the parser extract the initial two bits that determine the tag.
...
- `type-expr` is the field's type. Сan be a simple type, a parameterized type with appropriate arguments, or a more complex expression.
...
```
Similar as above, in the `CondExample` type, the variable `b` is serialized only if the **second bit** of `a` is `1`.
...
as combinator's field. Only indentifiers of the `Nat` and `Type` types can be parameters.

A simple example of a parameterized type is the following definition of a type `A` that parameterized by a natural number `x`:
...
Since parametrs can be natural numbers, one can use arithmetic operations on them:
```tlb
...
```
For a detailed explanation of how it works, see the [Complex and non-trivial examples page](/language/TL-B/complex-and-non-trivial-examples).
...
By default, all cells are classified as `Ordinary`. This applies to all cells described in the TL‑B as well.
...
This example is explained well in the [TL‑B types](/v3/documentation/data-formats/tlb/types#unary) article. The main idea here is that `UnaryChain` recursively deserializes until it reaches `unary_zero$0` because we know the last element of the `Unary X` type by the definition `unary_zero$0 = Unary ~0`. The value of `X` is calculated at runtime due to the definition `Unary ~(n + 1)`.
