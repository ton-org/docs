...
Commonly used in code. See [Cell & Bag of Cells (BoC)](/ton/cells/boc).
...
a program connecting various blockchains to transfer tokens and data from one network to another. [Article](/ecosystem/oracles.mdx).
...
TON applications run on-chain and rely on smart contracts for computation, storage, and communication.
...
a bot service on Telegram through which people can donate money, and content creators can monetize their channels and services in Toncoin. See also [Subscriptions](/v3/documentation/dapps/defi/subscriptions).
...
the main network of a blockchain. [Article](/ton/overview).
