...
This page explains each TON configuration parameter and its purpose.
...
This parameter is the address of the [elector smart contract](/ton/system-contracts), responsible for appointing validators, distributing rewards, and voting on changes to blockchain parameters.
...
- `capabilities`: This field is a set of flags indicating supported features.
...
- `min_tot_rounds`: The minimum number of rounds before a proposal can be applied. This parameter is not used: only `max_tot_round` (when proposals are rejected) and `min_wins` (when proposals are accepted) matter.

- `max_tot_rounds`: The maximum number of rounds, upon reaching which the proposal is automatically rejected.

- `min_wins`: The required number of wins (3/4 of validators by the sum of the pledge must vote in favor).

- `max_losses`: The maximum number of losses, upon reaching which the proposal is automatically rejected.

- `min_store_sec` and `max_store_sec` determine the possible time interval during which the proposal is stored.
...
- `basic`: A boolean flag (1 for true, 0 for false) indicating whether this workchain is basic (handles TON coins, smart contracts based on the TON Virtual Machine (TVM)).
...
- `flags`: Additional flags for the workchain (reserved; set to 0).
...
This parameter defines the cost of filing complaints about incorrect operation of validators in the [elector smart contract](/ton/system-contracts).

[Parameter #13 on mainnet](https://tonviewer.com/config#13)

## Param 14: block reward

This parameter indicates the reward for creating a block in the TON Blockchain. Values are in nanotons, therefore, the reward for block creation in the masterchain is 1.7 Toncoins, while in the basechain, it is 1.0 Toncoins. In the event of a workchain split, the block reward is also divided: if there are two shardchains within the workchain, then the reward for each shard block will be 0.5 Toncoins.
...
For each validation period, there is an `election_id` equal to the Unix time at the start of the validation.
...
![Election timing scheme (Mainnet)](/resources/images/limits/config15-mainnet.png)

#### Calculate periods (mainnet)
...
![Election timing scheme (Testnet)](/resources/images/limits/config15-testnet.png)

#### Calculate periods (testnet)
...
- `flat_gas_limit` and `flat_gas_price`: A certain starting amount of gas is provided at a price of `flat_gas_price` (to offset the costs of launching the TON Virtual Machine (TVM)).
...
## Param 24 and 25: message prices
...
- `shuffle_mc_validators`: A boolean value indicating whether to shuffle the masterchain validators or not. If this parameter is set to 1, the validators are shuffled; otherwise, they are not.
...
For up‑to‑date values, see [Tonviewer config](https://tonviewer.com/config).

[Parameter #29 on mainnet](https://tonviewer.com/config#29)

## Param 31: fee-exempt contracts

This parameter represents the configuration of smart contract addresses from which no fees are charged for either gas or storage and where `TickTock` transactions can be created. The list usually includes governance contracts. The parameter is presented as a binary tree structure — a tree (`Hashmap 256`), where the keys are a 256-bit representation of the address. Only addresses in the masterchain can be present in this list.

[Parameter #31 on mainnet](https://tonviewer.com/config#31)

## Param 32, 34, and 36: validators lists
...
- `list`: A list of validators in the tree format `id->validator-data`: `validator_addr`, `public_key`, `weight`, `adnl_addr`: These parameters provide details about each validator - their 256-bit addresses in the masterchain, public key, weight, Abstract Datagram Networking Layer (ADNL) address (the address used at the network level of the TON).
...
More details about precompiled contracts are on [Precompiled contracts](/ton/precompiled).
...
After thoroughly reviewing this article, consider a more in-depth study of the following documents:

- The original descriptions are present, but they may be limited, in the documents:
  - [The Open Network Whitepaper](/ton/ton)
  - [Telegram Open Network Blockchain](/ton/tblkch)
...
See [Network configurations](/ton/network) for active configurations.
