...
You can view the parameter values in the [current configuration](https://tonviewer.com/config), and the method of writing them into [cells](/ton/cells/bag-of-cells) is outlined in the [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/language/TL-B/overview) format.

<Aside>
Configuration values are TL-B typed cells serialized into Bags of Cells (BoC); see [canonical serialization](/ton/cells/bag-of-cells) and [Bag of Cells](/ton/cells/bag-of-cells).
</Aside>
...
<Aside>
In the configuration parameter, only the hash portion of the address is recorded, as the contract always resides in the [MasterChain](/ton/overview#masterchain) (WorkChain -1). Therefore, the full address of the contract will be written as `-1:<value of the configuration parameter>`.
</Aside>
...
This parameter is the address of the [elector smart contract](/standard/governance/overview), responsible for appointing validators, distributing rewards, and voting on changes to blockchain parameters.
...
- `min_tot_rounds`: The minimum number of rounds before a proposal can be applied. Currently this parameter is not used: only `max_tot_round` (when proposal will be rejected) and `min_wins` (when proposal will be accepted) matter.
...
For up-to-date values, see https://tonviewer.com/config.
...
More details about precompiled contracts are on [this page](/ton/precompiled).
...
On this [page](/ton/network), you can find active network configurations of the TON Blockchain.
