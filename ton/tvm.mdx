...
…along with a completion tag bit equal to `1` (meaning that the completion has been applied), which must be stored separately.
...
Perfect — I’ll keep the **exact wording** from your excerpt, only formatting it in Markdown for readability (headings, lists, code style for inline values). Numbering stays **exactly as in the source**.
...
* **Continuation** — A special value containing TVM code and execution context that can be invoked (executed) later. As such, it generalizes function addresses (i.e., function pointers and references), subroutine return addresses, instruction pointer addresses, exception handler addresses, closures, partial applications, anonymous functions, and so on.
