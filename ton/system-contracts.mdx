...
The **Elector** smart contract manages validator elections, validation rounds, and reward distribution. To become a validator and interact with the Elector, follow the [validator instructions](/ecosystem/node/setup-mytonctrl).

### Data storage

The Elector stores:

- Non-withdrawn Toncoins in the `credits` hashmap
- New validator applications in the `elect` hashmap
- Past election data in the `past_elections` hashmap (including complaints and `frozen` stakes held for `stake_held_for` periods, defined in [Param 15](/ton/config#param-15-elections-timing))
...
- **Case 1**: 9 candidates stake 100,000 TON (`max_factor=2.7`), and 1 candidate stakes 10,000.
  - _Without the 10,000-stake candidate_: TES = 900,000.
  - _With the 10,000-stake candidate_: TES = 9 \* 27,000 + 10,000 = 253,000.
  - **Result**: 10,000-stake candidate is excluded.

- **Case 2**: 1 candidate stakes 100,000 Toncoin (`max_factor=2.7`), and 9 candidates stake 10,000.
...
The fine is deducted from the validator's `frozen` stake in the relevant `past_elections` record if approved. These funds stay locked for the period defined by [Param 15](/ton/config#param-15-elections-timing) (`stake_held_for`).
