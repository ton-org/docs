...
Recall that the TON Blockchain has several so-called “configurable parameters” (cf. [[3]](/ton/ton#2-1-21-configurable-parameters)), which are either certain values or certain smart contracts residing in the masterchain. This section discusses the storage of and access to these configurable parameters.
...
When some amounts of one or several such cryptocurrencies need to be represented, a dictionary (cf. [[4](/ton/tvm#3-3-hashmaps%2C-or-dictionaries), 3.3]) with 32-bit `currency_id`s as keys and `VarUInteger 32` values is used:
...
<a id="footnote-4"></a> **4** Cf. [[4, 3.3.3–4]](/ton/tvm#3-3-3-serialization-of-hashmaps), where an example is given and explained, pending a more complete reference. [Back ↑](#1-1-1-tvm-cells)
...
<a name="footnote-39"></a>**39** Notice that exotic cells (with `s = 1`) always have `b ≥ 8`, with the cell type encoded in the first eight data bits (cf. [[4, 3.1.7]](/ton/tvm#3-1-7-types-of-exotic-cells)). [Back ↑](#5-3-6-serialization-of-one-cell-from-a-bag-of-cells)
...
<a name="footnote-43"></a> **43** The original PDF was referred to early version of the [Telegram Open Network](https://www.editionmultimedia.fr/wp-content/uploads/2019/10/Telegram-Open-Network-2017.pdf). We have redirected these links [3.x.x] to a more up-to-date document — [The Open Network](/ton/ton). [Back ↑](#introduction)
