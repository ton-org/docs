...
Understand the four account states on TON and how they affect transactions and deployment.
...
The main difference between the `uninit` and `frozen` statuses is that in addition to metadata, the `frozen` account contains the hash of the [last account state](/ton/tblkch#4-1-6-account-description). Thus, it becomes possible to restore the last state of an account before it was frozen by sending a `state_init` to it, whose hash matches the one recorded on the account.
