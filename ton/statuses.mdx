...
### Legend
 ...
 - `StateInit`: StateInit structure.
   ...
   - valid last state: must be `state_init` of the last successful transaction before the account changes to frozen.
 ...
 - `zero_bal_after_dest_act`: Did the account balance become zero when sending some of the messages with the flag 32? This field is meaningful only if there's at least one such message during **Action phase**.
   - any;
   - false;
   - true.
