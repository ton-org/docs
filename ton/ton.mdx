...
where all $account\_id$ appearing as indices of this hashmap must begin with prefix $s$, if we are discussing the state of shard $(w, s)$ (cf. [2.1.8](#2-1-8-identification-of-shardchains)).
...
The (virtual) account-chain state is just the state of one account, described by type $AccountState$. Usually it has all or some of the fields listed in [2.3.20](#2-3-20-account%2Fsmart-contract-state), depending on the specific constructor used.
...
The shard configurationi.e., this shard binary tree, or the collection of all active $(w, s)$ for a given $w$ (corresponding to the leaves of the shard binary tree)is part of the masterchain state and is available to everybody who keeps track of the masterchain.[²²](#footnote-22)
...
While Proof-of-Work algorithms are very similar to each other and dier mostly in the hash functions that must be computed for mining new blocks, there are more possibilities for Proof-of-Stake algorithms. They merit a sub-classification of their own.
...
So far, the classification was valid both for single-chain and multi-chain systems. However, multi-chain systems admit several more classification criteria, reecting the relationship between the different blockchains in the system. We now discuss these criteria.
...
Multi-blockchain projects can be classifiedconfiguration according to the supported level of interaction between the constituent blockchains.
...
### 2.9.9. Ushermenshermenniversa; https://universa.io
