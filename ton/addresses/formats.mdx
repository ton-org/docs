...
<Warning>
  This format can be applied only to addresses that are described according to the `addr_std` TL-B scheme, see addresses [general info](/ton/addresses/overview) subpage.
</Warning>
...
- **User-friendly format** is for application-level use, including flags and a checksum.
