...
One can create a library cell that directly references another library reference. However, such cells will not be automatically dereferenced by the CTOS instruction (begin_parse in FunC), and attempting to do so will result in exit code 9, so you need to dereference them manually using the XLOAD or XCTOS instructions.

However, creating libraries that reference a cell whose tree contains other library cells is totally fine.
...
Also, if the account hosting the library becomes frozen, the library becomes inaccessible. So it is very important to keep in mind that storage costs in the masterchain are much higher than in the basechain and to carefully control the balance of the account that hosts the library.

### Other articles

If you want to see practical examples of how to manage libraries, please check out [this article](guidebook/working-with-libraries.mdx).
Library cells are mainly used to minimize storage costs for accounts with identical code. Read more about this pattern [here](techniques/using-libraries.mdx).