...
<Aside type="tip">
The following documents provide details on jetton architecture and core TON concepts, which differ from EVM-like blockchains and other blockchain systems. Understanding these concepts is crucial and will significantly aid in grasping TON's functionality.
</Aside>
...
<Button href="/v3/guidelines/dapps/asset-processing/jettons#accepting-jettons-from-user-deposit-addresses" colorType="secondary" sizeType={'sm'}>
  On-chain processing
</Button>
...
<img
  ...
  src="/resources/images/asset-processing/jetton_contracts.png"
/>

<img
  className="hidden dark:block"
  src="/resources/images/asset-processing/jetton_contracts_dark.png"
/>
...
To prevent fraud, verify the **original jetton address** (jetton master contract) for specific jetton types. Alternatively, **check the project’s official social media** or website for **accurate information**. Use [Tonkeeper ton-assets list](https://github.com/tonkeeper/ton-assets) to verify assets.
...
| `amount`               | coins      | The total amount of jettons being transferred (in jetton units).                                                                                                                         |
...
This transfer requires some Toncoin for **fees** and a **transfer notification message**.
...
to the specified centralized address with a mandatory comment.
...
6. If there is at least one transaction with undecodable output messages (the message body does not contain opcodes for
  ...
  <Tab title="Golang">
    <details>
      ...
      const (
      	...
      	TestnetConfig   = "https://ton.org/testnet-global.config.json"
      ...
      func prepare(api ton.APIClientWrapped, jettonsInfo map[string]jettonInfo) (map[string]jettons, error) {
      	...
      	for name, info := range jettonsInfo {
      		...
      		slice.MustLoadAddr()  // skip owner address
      		adminAddress := slice.MustLoadAddr()

      		if adminAddress.String() != info.address {

			return nil, fmt.Errorf("jetton minter address from jetton wallet doesn't match config")
      		}
...
You can find SDKs for different languages (JS, Python, Golang, C#, Rust, etc.) [here](/v3/guidelines/dapps/apis-sdks/sdk).
