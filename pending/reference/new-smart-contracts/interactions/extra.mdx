...
> THIS PAGE IN TON DOCS: https://docs.ton.org/v3/guidelines/dapps/cookbook
...
TON addresses uniquely identify contracts on the blockchain, indicating their workchain and original state hash. [Two common formats](/documentation/smart-contracts/addresses#raw-and-user-friendly-addresses) are: **raw** (workchain and HEX-encoded hash separated by the ":" character) and **user-friendly** (base64-encoded with certain flags).
...
- Create a wallet wrapper (object) of the correct version, typically v3r2 (see also [wallet versions](/documentation/smart-contracts/contracts-specs/wallet-contracts)), your secret key and workchain (usually 0 for the [BaseChain](/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains#workchain-a-blockchain-with-your-own-rules)).
...
- Form messages you want to send and initiate the transaction. You can send up to 4 messages per request, as detailed in the [advanced manual](/guidelines/smart-contracts/howto/wallet#sending-multiple-messages-simultaneously).

```js JS (@ton) for Wallet V4
...
Although it is a common approach, [calling the GET method](/guidelines/smart-contracts/get-methods#calling-get-methods-from-code) every time to retrieve the wallet address can be slow and resource-intensive.
...
NFT collections on TON are highly customizable. The NFT contract can be defined as any contract that provides a valid get-method to return metadata. The transfer operation is standardized similarly to [Jettons](/guidelines/dapps/cookbook#how-to-construct-a-message-for-a-jetton-transfer-with-a-comment).
...
swap#e3a0d482
    _:SwapStep
    swap_params:^SwapParams = ForwardPayload;
step#_
    pool_addr:MsgAddressInt
    params:SwapStepParams = SwapStep;
step_params#_
    kind:SwapKind
    limit:Coins
    next:(Maybe ^SwapStep) = SwapStepParams;
swap_params#_
    deadline:Timestamp
    recipient_addr:MsgAddressInt
    referral_addr:MsgAddress
    fulfill_payload:(Maybe ^Cell)
    reject_payload:(Maybe ^Cell) = SwapParams;
```
...
swap#ea06185d
    ...
    amount:Coins
    _:SwapStep
    swap_params:^SwapParams = InMsgBody;
step#_
    pool_addr:MsgAddressInt
    params:SwapStepParams = SwapStep;
step_params#_
...
<Accordion title="JS (@ton)" icon="fa-info-circle" iconColor="blue">
...
<Accordion title="Kotlin (ton-kotlin)" icon="fa-info-circle" iconColor="blue">
Build Asset slice:

```kotlin Kotlin (ton-kotlin)
...
<Accordion title="Python (pytoniq)" icon="fa-info-circle" iconColor="blue">
...
- [Message overview](/documentation/smart-contracts/message-management/messages-and-transactions)
- [Internal messages](/documentation/smart-contracts/message-management/internal-messages)

This topic is explored in depth in the [Payments processing](/guidelines/dapps/asset-processing/payments-processing) article.
