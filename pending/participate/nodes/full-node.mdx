...
> THIS PAGE IN TON DOCS: https://docs.ton.org/v3/guidelines/nodes/running-nodes/full-node
...
You can accomplish this by contacting your network provider or [renting a server](/guidelines/nodes/running-nodes/full-node#recommended-providers) to run a node.
...
<Aside>
Prices, configurations, and availability can change. It is recommended to always refer to the official documentation and pricing pages of the respective cloud provider before making any decisions.
</Aside>
...
Please, check this video step-by-step tutorial to start promptly: (none).
...
Otherwise, [check troubleshooting section](/guidelines/nodes/nodes-troubleshooting#status-command-displays-without-local-node-section) and [check node logs](/guidelines/nodes/running-nodes/full-node#check-the-node-logs).
</Aside>
