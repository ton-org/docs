---
title: Get started with TON
hidden: false
---

import { Aside } from '/snippets/aside.jsx';

> THIS PAGE IN TON DOCS: https://docs.ton.org/v3/guidelines/get-started-with-ton
...
<div style={{ width: '100%', textAlign: 'center', margin: '0 auto' }}>
  <video style={{ width: '100%', borderRadius: '10pt', margin: '15pt auto', maxWidth: '90%' }} muted={true} autoPlay={true} loop={true}>
...
2. Next, you need to [enable test mode](/concepts/dive-into-ton/ton-ecosystem/wallet-apps#tonkeeper-test-environment) within Tonkeeper.
...
<Aside>
You need to [sign in](https://github.com/login) to GitHub for further work.
</Aside>
...
- **API provider** to make requests to TON Blockchain. TON has multiple [API types](/guidelines/dapps/apis-sdks/api-types) for different purposes. We will use the testnet version of [toncenter.com](https://toncenter.com/) API.
- **JavaScript SDK**: a JavaScript SDK (recall that an SDK is a Software Development Kit) is needed to parse the smart contract address used and prepare it to create an API request. To better understand TON addresses and why they need to be parsed to carry out this process, please see this [resource](/documentation/smart-contracts/addresses) to understand why we should parse it. To carry out this procedure, we use [`@ton/ton`](https://github.com/ton-org/ton).
...
1. **Wallet address**: a wallet address is required for the miner to receive their mining reward (in this case, we must use the [Tonkeeper Testnet mode](/concepts/dive-into-ton/ton-ecosystem/wallet-apps#tonkeeper-test-environment)).
...
<Aside>
Using an RPC node provider or running your own ton-http-api instance is better for that. Read more at the [TON Center API page](/guidelines/dapps/apis-sdks/ton-http-apis).
</Aside>
...
Furthermore, to run the script, it is necessary to enter the following command [in the terminal](/guidelines/get-started-with-ton#running-scripts):
...
<Aside>
**Warning**: Although this information is highly complex and **not necessary for this tutorial** if you're interested in understanding the complex technical aspects of TON, consider using these resources:

1. To better understand how TON Virtual Machine (TVM) operates and _how TON processes transactions_, check out [TVM overview section](/documentation/tvm/tvm-overview).
2. Secondly, if you want to learn more about how transaction and gas fees work on TON, consider diving into [this section](/documentation/smart-contracts/transaction-fees/fees) of our documentation.
3. Finally, to better understand the exact gas values needed to carry out TVM instructions, see [this section](/documentation/tvm/instructions#gas-prices) of our documentation.
...
<Aside>
Cells are data storage structures on TON that fulfill numerous purposes, including increasing network scalability and smart contract transaction speeds. We won't get into specifics here, but if you're interested in understanding the complexity of cells and how they work, consider diving into [this](/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage) section of our documentation.
</Aside>
...
To ensure that the NFT mining process is carried out correctly and that the user can store their NFT properly, we will use [Blueprint](/documentation/smart-contracts/getting-started/javascript) to simultaneously interact with TON Blockchain and the Tonkeeper wallet.
...
- [Simple: NFT Testnet Mining](/guidelines/get-started-with-ton#simple-nft-testnet-mining)
- [Genuine: NFT Mainnet Mining](/guidelines/get-started-with-ton#genuine-nft-mainnet-mining)

### Simple: NFT Testnet mining

Below are the steps needed to initiate your first Testnet transaction to mine your NFT:

1. Activate [Testnet mode within your Tonkeeper wallet](/concepts/dive-into-ton/ton-ecosystem/wallet-apps#tonkeeper-test-environment)
...
- [Develop smart contracts: Introduction](/documentation/smart-contracts/overview)
...
- [Bot for sales of dumplings](/guidelines/dapps/tutorials/telegram-bot-examples/accept-payments-in-a-telegram-bot-js)
- [Mint Your first Jetton](/guidelines/dapps/tutorials/mint-your-first-token)
- [Step by step NFT collection minting](/guidelines/dapps/tutorials/nft-minting-guide)
- [How to run TON Site](/guidelines/web3/ton-proxy-sites/how-to-run-ton-site)
