...
(physical vs virtual level + image, nodes)
...
(ecosystem images:
    physical:
        frontend -> http sdk
        (frontend -> ton connect) -> (wallet kit -> wallet app) -> http sdk
        frontend -> backend // explorer
        backend -> http sdk // processing
        backend -> adnl sdk -> node
        http sdk -> http api -> node

        tma are frontends
        http api might be self-hosted
        node (liteserver) can be self-hosted
...
(ADNL, HTTP API)
(Cell, @ton/core)
(Bindings/wrappers in general)
(Low-level bindings)
(Wallet/token wrappers, @ton/ton, assets-sdk)
(TON Connect)
