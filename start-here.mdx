...
This documentation is divided into several sections, with lower-level details appearing later in the list. For example, an existing standard contract may already solve the problem, so there may be no need to implement a new one, but lower level sections will describe details about the standard contract implementation.
