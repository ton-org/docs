...
> THIS PAGE IN TON DOCS: https://docs.ton.org/v3/guidelines/dapps/asset-processing/payments-processing
...
<Aside type="tip">
It's recommended to review the [Asset processing overview](/documentation/dapps/assets/overview) before reading this tutorial.
</Aside>
...
<Aside type="tip">
Read more on the [Wallet tutorial](/guidelines/smart-contracts/howto/wallet#-deploying-a-wallet).
</Aside>
...
<Aside type="tip">
Full Address description on the [Smart contract addresses](/documentation/smart-contracts/addresses) page.
</Aside>
...
<Aside type="caution">
If the attached `value` is too small, the transaction may fail with the error `cskip_no_gas`. n this case, Toncoins will be transferred, but no logic will be executed on the recipient's side (the TVM will not launch). Read more [here](/documentation/network/configs/blockchain-configs#param-20-and-21).
</Aside>
...
To understand the required pipeline, carefully read [Send payments](/guidelines/dapps/asset-processing/payments-processing/#send-payments), especially the 7th point.
...
- [Learn more](https://docs.ton.org/v3/guidelines/ton-connect/overview/)
...
The official TON blockchain explorer: [https://tonscan.org](https://tonscan.org).

To generate a transaction link in the explorer, the service needs to get the lt (logical time), the transaction hash, and the account address (the address of the account for which the lt and txhash were obtained using the [getTransactions](https://toncenter.com/api/v2/#/transactions/get_transactions_getTransactions_get) method). Then [https://tonscan.org](https://tonscan.org) and [https://explorer.toncoin.org/](https://explorer.toncoin.org/) can display the page for this transaction in the following format:
...
Note: tonviewer and tonscan also support external-in message hashes instead of transaction hashes for explorer links. This is useful when generating an external message and needing an instant transaction link. Learn more about transactions and messages hashes [here](/guidelines/dapps/cookbook#how-to-find-transaction-or-message-hash).

## Best practices

### Wallet creation

<Accordion title="JavaScript and TypeScript" icon="fa-info-circle" iconColor="blue">
- **toncenter:**
...
<Accordion title="Golang" icon="fa-info-circle" iconColor="blue">
- **xssnick/tonutils-go:**
  - [Wallet creation + get balance](https://github.com/xssnick/tonutils-go?tab=readme-ov-file#wallet)
</Accordion>

<Accordion title="Python" icon="fa-info-circle" iconColor="blue">
- **psylopunk/pythonlib:**
...
When under heavy load, TON Blockchain may split into [shards](/documentation/smart-contracts/shards/shards-intro)to distribute network activity. A shard is similar to a network segment in Web3.
...
A full list of SDKs for various programming languages (JS, Python, Golang, etc.) is available [here](/guidelines/dapps/apis-sdks/sdk).
