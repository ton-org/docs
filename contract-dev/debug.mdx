...
<Aside type="tip">
  ...
  Some instructions in TVM are implemented as instruction groups.
  For example, [LDU](/tvm/instructions#d3-ldu) (`load_uint`), [LDI](/tvm/instructions#d2-ldi) (`load_int`), it's preload versions `(`preload\_uint`and`preload\_int\`).
  So there can be only one `exec_*` function handling multiple TVM instructions. That exactly what how `LDU` [is implemented](https://github.com/ton-blockchain/ton/blob/34823b1ea378edbe3bc59f3bcc48126480a0b768/crypto/vm/cellops.cpp#L981)
</Aside>
