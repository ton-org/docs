...
Include the contract code and data in the vanity deploy message. The vanity contract replaces its own state with the provided code and data. The contract is first deployed with a `StateInit` that produces the desired address (see [Addresses overview](/foundations/addresses/overview#account-id)), then immediately sets its actual state from the payload.
...
It checks whether the message comes from the owner specified in data, then replaces its code and data with the ones provided in the incoming message. The key is a 256‑bit salt stored in this contract’s `StateInit` in addition to five padding bits and the owner address. The salt is skipped in the contract code with `ds~skip_bits(256);`, but it determines the resulting address.
...
Run the command with the desired search parameters, including `-w` for the workchain and the owner address that is allowed to perform the deployment. The example below searches on basechain for the specified suffix.

```bash
python3 run.py -w <WORKCHAIN> --end '<SUFFIX>' --case-sensitive <OWNER_ADDR>
```

Where:
- `<WORKCHAIN>` — workchain id; use `0` for basechain. See [workchain](/foundations/addresses/overview#workchain-id).
