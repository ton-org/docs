...
Incoming external messages do not transfer funds and receiving smart contract must pay for their processing from its balance. Upon receiving an external message, a smart contract can spend some free gas (`gas_credit`) in order to decide whether it is ready to accept it or not and then pay for its further processing.

External messages are usually accepted through the `accept()` instruction, which sets `gas_limit` to its maximum possible value specified in the network configuration and zips `gas_credit`. From this point on, the contract will have to pay for the entire processing of the incoming message. Therefore, the unconditional acceptance of all external messages can quickly spend all the funds on the balance of the contract.
...
There are many additional checks before accepting an external message. The following example is a part of [wallet v3 code](https://github.com/ton-blockchain/ton/blob/53ec9684bd213983e1fe0f7610d3e3453a4ec628/crypto/smartcont/wallet3-code.fc).
