...
1. Forwarding fees. For example, in [jettons](/standard/tokens/jettons/overview), the [`StateInit`](/foundations/messages/state-init) must be forwarded with each transfer, resulting in high forwarding fees. Moving the code into a library cell significantly reduces these fees.
