...
- **circom** and **snarkjs** installed (see installation instructions below)
...
For FunC and Tolk you will also need to generate wrappers manually:

```bash
npx export-ton-verifier import-wrapper ./wrappers/Verifier.ts --force
```

This command generates a TypeScript wrapper file that provides type-safe methods to interact with your verifier contract.
...
> **Note:** Make sure you've built the contracts before running tests. For Tact contracts, run `npx blueprint build --all` first. For FunC/Tolk, ensure the wrappers are generated.
...
From here, you can easily extend this workflow to support more complex circuits and real-world applications.
