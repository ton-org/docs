...
For example, [NFT item contracts](/standard/tokens/nft/how-it-works) reference their collection contract. If the address of a collection contract changes, all of its item contracts would need to be updated to point to the new address. This is not possible, as the item contracts cannot be modified by a collection admin. With upgrades the collection contract address stays the same, so all item contracts continue to reference it without any changes. A collection admin can then upgrade the collection contract code to fix bugs or add features without affecting the item contracts.
