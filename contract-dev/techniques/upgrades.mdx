...
1. When the transaction completes, apply the new code during the action phase.  
1. Process subsequent messages with the new code. 

The upgrade happens immediately in a single transaction. However, new code becomes active after the transaction completes and new data is immediately available when the transaction ends. If the message does not contain enough Toncoin to execute the action phase with the code update, but enough to execute the compute phase with the data update, the contract state changes are reverted. Test the upgrade script to estimate gas requirements, and provide enough Toncoin for complete execution of the upgrade transaction.
...
1. Call `hotUpgradeData()` which now runs with the new code, reads old storage, transforms it, and writes new storage
