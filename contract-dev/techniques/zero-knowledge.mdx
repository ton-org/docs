...
<Steps>
  ...
  >
    Run inside the `circuits/Multiplier` directory.
...
The trusted setup is a one-time ceremony that generates the proving and verification keys for a circuit. It's called trusted because if the setup parameters are compromised, proofs could be forged.
...
- For local tests, perform a simplified trusted setup ceremony.
  ...
  >
    ```bash
      ...
      snarkjs zkey contribute Multiplier_0000.zkey Multiplier_final.zkey \
        --name="1st Contributor" -v -e="some random text"
    ```
  </Step>
...
<Steps>
  ...
  >
    The TypeScript wrappers enable type-safe interaction with the verifier contract.
...
<Aside type="tip">
  Build the contracts before running tests and ensure the TypeScript wrappers are generated.
</Aside>

<Steps>
  <Step
    title="Set up the TypeScript dependencies"
  >
...
[The zk-ton-examples repository](https://github.com/zk-examples/zk-ton-examples/) contains templates for Noname, gnark, and Arkworks. It is possible to generate proofs in any of these stacks, then convert them  to the snarkjs format and verified both locally and on-chain.
...
<Steps>
  ...
  >
    ```bash
    cargo init
    cargo add ark-bls12-381 ark-ff ark-groth16 ark-r1cs-std \
      ark-relations ark-snark ark-snarkjs ark-std rand@0.8.5
    ```
  ...
  <Step
    title="Implement the circuit"
  >
    Implement the circuit logic using Arkworks primitives, similar to how write a Circom circuit.
  ...
  >
    Follow the same workflow as in [the Circom section above](#create-the-circom-circuit).
  </Step>
...
<Steps>
  <Step
    title="Create a new project"
  >
    Use the examples from the following repositories as a starting point.

    - [The gnark repository](https://github.com/Consensys/gnark?tab=readme-ov-file#example) contains an example circuit.
...
- [The zk-ton-examples repository](https://github.com/zk-examples/zk-ton-examples/)
- [The export-ton-verifier repository](https://github.com/mysteryon88/export-ton-verifier)
  [The ark-snarkjs repository](https://github.com/mysteryon88/ark-snarkjs)
  [The gnark-to-snarkjs repository](https://github.com/mysteryon88/gnark-to-snarkjs)
- [The Circom documentation](https://docs.circom.io/)
- [The Noname repository](https://github.com/zksecurity/noname)
- [The gnark repository](https://github.com/Consensys/gnark)
- [The Arkworks homepage](https://arkworks.rs/)
- [The snarkjs repository](https://github.com/iden3/snarkjs)
