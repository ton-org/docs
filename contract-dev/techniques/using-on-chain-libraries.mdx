...
<Aside
  ...
  title="Funds at risk"
>
  Keep enough TON on the account that hosts the library to cover at least 10 years of storage. If the balance becomes too low, the library may freeze and become inaccessible.
</Aside>
...
The key line is `setLibCode(libToPublish, 2);`. This call publishes an ordinary cell with the flag set to `2`, which makes the library public.
