...
Standard upgrade methods fail when a contract receives frequent updates. For example, DEX pools that update prices every second or lending protocols that continuously adjust interest rates. The problem: you cannot predict what data will be in storage when the upgrade transaction executes.

When you send an upgrade message with new code and data, other transactions may execute before your upgrade arrives. By the time your upgrade applies, the data you prepared is stale. For a DEX pool, this means overwriting current price data with outdated values, breaking the protocol.
