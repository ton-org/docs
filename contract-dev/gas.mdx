...
Contracts that receive a user's message are referred to as **receiver** contracts. Other contracts in the same system are called **internal** contracts. This terminology is local to the current article.
Consider a contract system with three contracts: **receiver**, **A** and **B**. In that system the typical trace looks like this (transactions move from left to right):
...
The reason for this requirement is that reverting the contract system state is usually not possible, because the TONs are already spent.

When a contract system’s correctness depends on successful execution of the remaining transaction trace, it must guarantee that an incoming message carries enough attached TONs to cover all fees. This article describes how to compute those fees.
...
There are two kinds of values: gas units and TONs.
...
One can convert to TONs in the contract code using current blockchain config parameters:
...
In general, the message size can be calculated at runtime using `computeDataSize()` which uses [`CDATASIZE`](/tvm/instructions#f941-cdatasize):
The forward fee can then be calculated using `getForwardFee()` which uses [`GETFORWARDFEE`](/tvm/instructions#f838-getforwardfee)

<Aside type="tip">
  Instead of `computeDataSize()` mode 1024 in `sendRawMessage()` can be used. It will consume approximately the same amount of gas.
</Aside>
...
The price for the message root is fixed and equal to [lump\_price](/foundations/config#param-24-and-25%3A-message-price): this is the price for root's bits and the cell itself. The `lump_price` constant is included in `getForwardFee()` function.
...
For this case, use `getSimpleForwardFee(b, a)` which uses [`GETFORWARDFEESIMPLE`](/tvm/instructions#f83c-getforwardfeesimple). This function does not add lump\_price into account.
...
<Aside>
  For calculating storage fees, maximum possible size of the contract in `cells` and `bits` should be known. This might not be the trivial task, especially if the contract is storing a `hashmap` in the data. In any case, the approach is the same here. Write test, that will occupy maximum possible size and calculate that. [Helper function for this](#helper-functions)
</Aside>
...
Always keep a minimum balance on all contracts in the system. Storage fees deduct from this reserve, which replenishes with each user interaction.
...
let otherFees = 0; // compute compute + forward fees here
    require(messageValue() >= totalStorageFees + otherFees, "Not enough TONs");
}
```
...
receive(msg: Operation) {
    ...
    require(messageValue() >= freezeLimit * 3 + otherFees, "Not enough TONs");
}
```
...
receive(msg: SwapRequest) {
    ...
    // It may also be necessary to handle fees on this exact contract if it is not supposed to hold users' TONs.
    // That can be done in either of the two approaches described above.
}
```
