...
Cryptographic signatures are the foundation of access control in TON. Contracts verify signatures on-chain and implement their own authorization policies for wallet operations, server-authorized actions, gasless transactions, multisig, and delegation. See [wallet ownership verification](/standard/wallets/how-it-works#how-ownership-verification-works).

## Ed25519 in TON

TON uses [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) as the standard signature scheme. All [wallets](/standard/wallets/history) (v1–v5) and [highload wallets](/standard/wallets/highload/overview) rely on Ed25519.

Specification:

- Public key size: 256 bits
- Signature size: 512 bits
- Curve: Ed25519 (Edwards form over Curve25519)

See [TON Cryptography — Ed25519](/foundations/whitepapers/tblkch#a-3-10-cryptographic-ed25519-signatures) for cryptographic details.

<Aside
  type="caution"
>
  The public key is **not** the wallet address. The address is derived from the contract's `StateInit` (code + initial data). Multiple contracts can use the same public key but have different addresses. See [Addresses in TON](/foundations/addresses/overview).
</Aside>
...
Ed25519 signatures in TON typically work with hashes rather than raw data:
...
<Aside type="note">
  TVM provides two signature verification methods: `CHKSIGNS` (checks signature of data) and `CHKSIGNU` (checks signature of hash). Hash-based verification (`CHKSIGNU`) is preferred because `CHKSIGNS` only processes data from a single cell (up to 1016 bits) and ignores cell references. For messages containing multiple cells or references, hashing the entire structure first is required.
</Aside>
