...
<Aside
  type="caution"
>
  The public key is _not_ the wallet address. The address is derived from the contract's `StateInit` (code + initial data). Multiple contracts can use the same public key but have [different addresses](/foundations/addresses/overview).
</Aside>
