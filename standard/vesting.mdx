...
This guide explains how vesting works on TON, how to deploy and use vesting contracts, and how to earn staking rewards from locked funds.
...
To verify:
1. Open the vesting contract address in a block explorer (tonscan.org or tonviewer.com)
...
2. [Import](/ecosystem/node/mytonctrl/wallet) the private key and vesting address into MyTonCtrl and use it as a standard wallet-v3.
...
3. Find the jetton wallet address:
   - Open the deposit transaction in explorer (tonscan.org or tonviewer.com)
...
3. Find the `bmTON` jetton wallet address:
   - Open the staking transaction in explorer (tonscan.org or tonviewer.com)
   - Look at the transaction trace and find the newly deployed jetton wallet
   - Verify that:
...
[Standard nominator pools](https://docs.ton.org/v3/documentation/smart-contracts/contracts-specs/nominator-pool) are the original TON staking pools where multiple nominators can pool their stakes together with a validator.
...
4. Withdraw stake by sending text comment "w" (lowercase):
   ...
   - If there are enough Toncoins on the pool balance, withdrawal will be made immediately. All funds will be available on the pool balance when it has completed participation in the validation round but has not yet submitted a request for participation in a new round.
   - If there are not enough Toncoins on the pool balance, a withdraw request will be created, and Toncoins will be withdrawn automatically after the end of the current validation round.
...
- [Run a single nominator pool with vesting](https://docs.ton.org/v3/guidelines/smart-contracts/howto/single-nominator-pool#run-a-single-nominator-pool-with-a-vesting-contract)
- [Single nominator pool documentation](https://docs.ton.org/v3/documentation/smart-contracts/contracts-specs/single-nominator-pool)
