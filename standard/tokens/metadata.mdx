...
The metadata standard covering non-fungible tokens (NFTs), NFT collections, and jettons is outlined in [TEP-64](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md) (TON Enhancement Proposal 64).

On The Open Network (TON), entities can have three types of metadata: on-chain, semi-chain, and off-chain.
...
The Snake encoding allows a portion of data to be stored in a standardized cell, with the remainder stored recursively in child cells. In this scheme, Snake-encoded data is typically prefixed with the `0x00` byte (see the note for exceptions). [TL-B](/language/TL-B/overview) schema:
...
[TL-B complex and non-trivial examples](../../language/TL-B/complex-and-non-trivial-examples).
...
The `amount_style` field supports the following values:
...
[GetGems sample metadata](https://s.getgems.io/nft/b/c/62fba50217c3fe3cbaad9e7f/95/meta.json)
...
On-chain NFT: [On-chain NFT example](https://getgems.io/collection/EQAVGhk_3rUA3ypZAZ1SkVGZIaDt7UdvwA4jsSGRKRo-MRDN/EQBq5z4N_GeJyBdvNh4tPjMpSkA08p8vWyiAX6LNbr3aLjI0)

Semi-chain NFT: [Semi-chain NFT example](https://getgems.io/nft/EQB2NJFK0H5OxJTgyQbej0fy5zuicZAXk2vFZEDrqbQ_n5YW)

On-chain jetton master: [On-chain jetton master example](https://tonscan.org/jetton/EQA4pCk0yK-JCwFD4Nl5ZE4pmlg4DkK-1Ou4HAUQ6RObZNMi)

## Parse metadata

Use the following API to parse metadata: [Get account metadata](/ecosystem/rpc/ton-center-v3/accounts/get-account-metadata). It handles most cases within gas limits. Parsing may fail.
