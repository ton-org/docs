...
Use the following API method to parse the metadata: [API Metadata](/api-reference/accounts/metadata). It parses most cases that are restricted in their gas use, otherwise expect rare failures.
...
3. Anyone can create an NFT or jetton using any `name`, `description`, or `image`. To prevent scams and confusion, apps should clearly distinguish tokens by their address, not names or tickers.
