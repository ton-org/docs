...
| ---------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `transfer`             | `0x5fcc3d14`        | tag                                                                                                                    |
| `query_id`             | `uint64`            | arbitrary request number                                                                                               |
| `new_owner`            | `MsgAddress`        | address of the new owner of the NFT item                                                                               |
| `response_destination` | `MsgAddress`        | address where to send a response with confirmation of a successful transfer and the rest of the incoming message coins |
| `custom_payload`       | `Maybe ^Cell`       | optional custom data                                                                                                   |
| `forward_amount`       | `VarUInteger 16`    | the amount of nanotons to be sent to the new owner                                                                     |
| `forward_payload`      | `Either Cell ^Cell` | optional custom data that should be sent to the new owner                                                              |
...
| Field                | Type                | Description                                      |
| -------------------- | ------------------- | ------------------------------------------------ |
| `ownership_assigned` | `0x05138d91`        | tag                                              |
...
| Field             | Type         | Description              |
| ----------------- | ------------ | ------------------------ |
| `get_static_data` | `0x2fcb26a2` | tag                      |
| `query_id`        | `uint64`     | arbitrary request number |
...
| Field                | Type         | Description                                                               |
| -------------------- | ------------ | ------------------------------------------------------------------------- |
| `report_static_data` | `0x8b771735` | tag                                                                       |
| `query_id`           | `uint64`     | should be equal with request's `query_id`                                 |
...
## [TEP-66](https://github.com/ton-blockchain/TEPs/blob/master/text/0066-nft-royalty-standard.md)

### Get royalty params message layout

| Field                | Type         | Description              |
| -------------------- | ------------ | ------------------------ |
| `get_royalty_params` | `0x693d3950` | tag                      |
| `query_id`           | `uint64`     | arbitrary request number |
...
| Field                   | Type         | Description                               |
| ----------------------- | ------------ | ----------------------------------------- |
| `report_royalty_params` | `0xa8cb00ad` | tag                                       |
| `query_id`              | `uint64`     | should be equal with request's `query_id` |
