...
cNFT (compressed NFT) is a [standard NFT](/standard/tokens/nft/overview) designed for mass [airdrop‑style distribution](/standard/tokens/airdrop). Minting costs shift from the creator to end users using [Merkle‑proof](/foundations/proofs/overview).
...
A single NFT is an item contract deployed without an associated collection. It preserves standard ownership semantics but does not include collection metadata or indexing.
