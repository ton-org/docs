...
Since the deployment process is not specified by the standard, logic can vary, and the recipes on this page might not apply to every contract. The [NFT reference implementation — Deploy single item](/standard/tokens/nft/reference-implementation#deploy-single-item) and most modifications follow the same path: the collection owner sends a message with deploy parameters to the collection, and the collection deploys the item.
...
- `.storeUint(1, 32)` — operation code `1` selects "deploy single item" in the reference collection contract (see [NFT reference implementation — Deploy single item](/standard/tokens/nft/reference-implementation#deploy-single-item)). TEP‑62 does not specify this opcode, so in custom implementations, this can differ.
...
In the reference contract, the body for `op=1` consists of `query_id`, `item_index`, `amount`, and a reference to `content`. See the [NFT reference implementation — Deploy single item](/standard/tokens/nft/reference-implementation#deploy-single-item) and [TL‑B overview](/languages/TL-B/overview).
...
- A deployed calling contract that can send internal messages
