...
In TON there is a collection smart contract and a separate smart contract for every NFT item — each item is its own contract (see the extension pattern).

![overview nft](/standard/tokens/nft/pictures/nftOverview.jpg)

Each NFT knows its collection, its index, and its item-specific part of the metadata. The initial owner is assigned by the collection.
...
Best practices:
Store INDEX.json in the metadata of every NFT item.
