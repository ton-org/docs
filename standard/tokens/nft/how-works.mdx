...
In TON, there is a collection smart contract and a separate smart contract for every NFT item — each item is its own contract, see more [why](/techniques/contract-sharding).
...
To get metadata for an NFT of a particular index, first fetch the NFT item address from the collection, then read the item metadata, and then combine it with the collection metadata to get the final result.
