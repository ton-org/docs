...
There is an [API method](/ecosystem/rpc/toncenter/v3/nfts/get-nft-items?playground=open) that can perform this verification off‑chain. Just provide the addresses of the NFT item and NFT collection.

Because the collection is the source of truth, verification goes through it:

1. Read the NFT item index from the item contract via [get_nft_data()](/standard/tokens/nft/api#get-nft-data).
1. Query the collection with [get_nft_address_by_index(index)](/standard/tokens/nft/api#get-nft-address-by-index).
