...
The NFT standard ([TEP-62](https://github.com/ton-blockchain/TEPs/blob/master/text/0062-nft-standard.md)) describes the required messages and get-methods. The NFT royalty standard ([TEP-66](https://github.com/ton-blockchain/TEPs/blob/master/text/0066-nft-royalty-standard.md)) describes a standard way to implement royalty information for NFT sales. The token data standard ([TEP-64](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md)) describes available formats for metadata of tokens. A widely used [reference implementation](https://github.com/ton-blockchain/token-contract/tree/1182ad99413242f09925d50e70ccb7e0e09f94d4/nft) follows these standards for simple NFTs with only the essential mechanics. This page explains the reference implementation, what you can extend, and what must remain unchanged.

The implementation contains multiple smart contracts written in FunC language, including additional ones for NFT sales. This page describes the collection and item smart contracts, as they are the only necessary ones per standard.
...
| `op=1` (inbound internal message, owner-only)       | Optional — deploy single item                  |
| `op=2` (inbound internal message, owner-only)       | Optional — deploy batch of items               |
| `op=3` (inbound internal message, owner-only)       | Optional — change owner                        |
...
The `content` cell in this contract contains 2 cells in references: `collection_content` containing metadata of the collection itself, and `common_content` containing the common prefix for individual items' metadata.
...
The collection and item contracts implement a classic pattern of parent–child contracts, where the collection acts as the parent and items are children. Therefore, the collection implements functionality to deploy its children, and the contract keeps the `nft_item_code` field in storage.

Helper functions used across the contract: composing stateinit, calculating addresses, and sending a deploy message for NFT items.

First, `calculate_nft_item_state_init` composes a stateinit cell of an NFT item with a given `item_index`. It composes a data cell following the expected storage schema of the item contract. Then the data and code cells are placed in a final stateinit cell following the format required by the TON blockchain. It uses `store_dict` functions but doesn't actually work with dictionaries. This function can be replaced with a more self‑explanatory `store_maybe_ref` function which does the same thing. It stores a single bit `0` in a builder if the cell is null, otherwise bit `1` and a cell as a reference.
...
Then there is a function which is mostly boilerplate: `deploy_nft_item`. It deploys an item with the provided index, code, amount of TON to attach, and the content for metadata. It first calls `calculate_nft_item_state_init` to compose the stateinit, then uses it to calculate the address by calling `calculate_nft_item_address` and using `workchain()` to derive the contract’s hardcoded workchain (`0` for basechain in this implementation). Then it uses both stateinit and address to compose and send a message for deploying that item contract.
...
Then comes the only message handling required by the standard: the implementation of the `get_royalty_params` message. All it does is send a `report_royalty_params` message back to the sender, providing the `royalty_params` field taken from storage. The `send_royalty_params` function is mostly boilerplate. It sends a non‑bounceable message to a desired address with a body following the standard. It carries the value of the incoming message.
...
It parses the incoming message and expects it to contain 3 values: `item_index:uint64`, `amount:Coins`, and `content:^Cell`. It checks the item index to be less than or equal to the `next_item_index` from storage. Then it calls a helper function `deploy_nft_item`, which deploys the new NFT item. Lastly, if the item index equals `next_item_index`, it increments `next_item_index` to update the last item index.
...
This feature is useful when you need to deploy many NFTs at once, for example when minting some initial items. It takes a single `deploy_list` dictionary from the message where keys are item indices and values are cells containing amounts of TON to attach to deployment, and content cells, similar to the values parsed by the singular deploy feature above. It traverses the dictionary in ascending order of indices, up to 249 items, so that it does not hit an actions limit. The actions limit is actually 254 out messages, but this reference contract sets a lower limit, which is an arbitrary choice.

The logic here is straightforward. The loop uses `~udict::delete_get_min(64)` to get and delete the item from the dictionary with the minimum unsigned 64‑bit integer key. It does that on each iteration, eventually iterating through the dictionary, and increments `counter` on each step to catch the message count limit. The deployment logic is the same as for the singular deployment feature above.
...
#### Return collection data (`get_collection_data`)

This get‑method returns `next_item_index` (typically the number minted if minted sequentially), collection content, and an owner address. It loads the contract storage, loads the first reference from the `content` cell (which contains the collection content), and returns those values.
...
#### Return item address by index (`get_nft_address_by_index`)

This get‑method takes an item index and returns the item's address. It uses helper functions `calculate_nft_item_state_init` and `calculate_nft_item_address` to calculate the address.
...
#### Return royalty parameters (`royalty_params`)

This get‑method returns royalty parameters: `numerator`, `denominator`, and `destination`, as required by the standard. These values are stored in a `royalty` cell in data, which we parse here to return.
...
#### Client flow for metadata

The reference implementation uses the off-chain metadata format, but this is not the only one defined by the standard. There are also on-chain and semi-chain formats, and it is possible to use a custom format if needed. The sequence diagram shows the flow for resolving an item's metadata. Note that the reference implementation follows only the tag 0x01 branch.

```mermaid
sequenceDiagram
  participant Client as Client/DApp
  participant Item as NFT Item
  participant Collection
  participant HTTP as Metadata server

  Client->>Item: get_nft_data()
  Item-->>Client: index, collection_address, individual_content
  Client->>Collection: get_nft_content(index, individual_content)
  Collection-->>Client: content cell
  Client->>Client: parse tag
  alt tag 0x01: off-chain
    Client->>Client: extract URL
    Client->>HTTP: GET URL
    HTTP-->>Client: JSON metadata: name, image, attributes
  else tag 0x00: on-chain dictionary
    Client->>Client: read on-chain fields
    opt semi-chain: uri key present
      Client->>HTTP: GET uri
      HTTP-->>Client: JSON metadata
      Client->>Client: add fields from JSON
    end
  else custom tag: contract specific
    Client->>Client: handle contract specific TL-B content
  end
```
...
#### Report static data (`get_static_data`)
