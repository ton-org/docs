...
The contract uses helper functions to compose stateinit, calculate addresses, and send deploy messages for NFT items.

First, `calculate_nft_item_state_init` composes a stateinit cell of an NFT item with a given `item_index`. It composes a data cell following the expected storage schema of the item contract. Then the data and code cells are placed in a final stateinit cell following the format required by the TON blockchain. It uses the `store_dict` builder to encode optional refs; it does not store dictionaries. This can be replaced with a more self‑explanatory `store_maybe_ref` builder which does the same thing. It stores a single bit `0` in a builder if the cell is null, otherwise bit `1` and a cell as a reference.
