...
>
  A lookalike sale contract can return a believable `get_sale_data` tuple but route incoming TON elsewhere. Match the deployed code hash against the canonical v3r3 hash before sending TON.
</Aside>

#### Verify the sale contract

Confirm three things before sending TON: the deployed code matches `nft-fixprice-sale-v3r3`, `get_sale_data` describes an active sale for the expected NFT and price, and the NFT is held by the sale contract.

```ts expandable
import { Cell, TonClient, Address } from '@ton/ton';

// Hash of NftFixPriceSaleV3R3CodeBoc from the GetGems nft-contracts repo
...
async function verifySaleContract(
  ...
  // 1. Deployed code must match the canonical v3r3 contract
  ...
  if (!codeHash.equals(NFT_FIXPRICE_SALE_V3R3_HASH)) {
    throw new Error('Code hash does not match v3r3 — possible lookalike contract');
  }

  // 2. get_sale_data must describe an active sale for the expected NFT and price.
  // Stack order: magic, is_complete, created_at, marketplace_address, nft_address,
  // nft_owner_address, full_price, marketplace_fee_address, marketplace_fee,
  // royalty_address, royalty_amount.
  const sale = await client.runMethod(saleAddress, 'get_sale_data');
  if (sale.stack.readNumber() !== 0x46495850) throw new Error('Not a FIXP sale');
  ...
  const nftAddress = sale.stack.readAddress();
  if (!nftAddress.equals(expectedNftAddress)) throw new Error('Sale points to a different NFT');
  // Skip nft_owner_address; full_price comes next.
  sale.stack.readAddress();
  if (sale.stack.readBigNumber() !== expectedPrice) throw new Error('Price does not match');
  ...
  // Skip init?, index, and collection_address.
...
The canonical BOC and reference verifier ([`isNftFixPriceSaleV3Contract`](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/nft-fixprice-sale-v3/isNftFixPriceSaleV3Contract.ts)) live in the GetGems nft-contracts repo. For v4 sale contracts, use the v4r1 hash from [`NftFixPriceSaleV4.source.ts`](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/nft-fixprice-sale-v4/NftFixPriceSaleV4.source.ts).
