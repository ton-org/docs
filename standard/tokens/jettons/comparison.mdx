...
This version is straightforward - it is a forked Stablecoin contract with removed *governance* functionality and added a burn mechanism. Until recent times, it was the most suitable Jetton for basic on-chain coin use cases.
...
During the time of massive on-chain project airdrops and CEX listings, there was a demand for a cheap and easy way to distribute initial Jetton tokens. You can read more about the history [here](/standard/tokens/airdrop). This version is also made from the Stablecoin contract with removed governance and allowed burn. It provides for Merkle-proof airdrops, enabling the minting of Jettons directly on the Jetton-Wallet contract in a decentralized manner. The implementation is designed to support large-scale airdrops without incurring high costs or placing a significant load on the blockchain.
...
It makes transfers and operations with Jetton 2.0 up to 3 times faster under heavy network load compared to the previous versions of Jettons. This version is recommended for all new projects working with Jettons.
