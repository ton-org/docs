...
<Aside
  ...
  title="Funds at risk"
>
  Each jetton stores a `decimals` parameter in its metadata that determines how amounts are displayed. Transferring without accounting for `decimals` can result in sending 1000 times the intended amount—irreversible on mainnet.
