...
also have an _owner (admin)_ who, by sending suitable messages to them, [mints new tokens](/standard/tokens/jettons/how-to-mint), closes the minting, changes metadata, or transfers the
...
The wallet balance information for specific users (owners), the address of the Jetton master contract (for appropriate minting), and some additional information.
...
<Warning>
  In all schemes below, you will see the `query id` field. Nowadays, the field is almost deprecated, and the protocols itself doesn't need it.
...
<Warning>
  The following data layout is just a possible example. The structure of the Mint message is not specified in any TEPs and left to the choice of the developer.
</Warning>
...
| `mintMessage` | Cell       | The rest of the data is in an internal transfer message, except the `query_id` field. |
