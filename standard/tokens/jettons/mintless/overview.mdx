...
Note that to use mintless jetton to its full extent, you will need to establish off-chain architecture.
...
Airdrop HashMap represent how we store individual users airdrop information.
...
The complete airdrop is stored as a HashMap where key is 267-bit internal TON account address and value is AirdropItem structure.

For Jetton Master contract, standard storage is extended with `merkle_hash`: 256-bit root hash of the Merkle tree containing all airdrop data.
...
Using a custom API:
  ...
  - Parse the data as explained in [Retrieving airdrop information](#retrieving-airdrop-information).
