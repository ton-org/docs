...
A programmatic transfer is usually done with an SDK (for example, [`assets-sdk`](https://github.com/ton-community/assets-sdk)) that handles low-level message serialization details. The provided example uses [TON Center API](/ecosystem/rpc/toncenter/overview) that might [require a key](/ecosystem/rpc/toncenter/get-api-key). Also you'll need a mnemonic of a wallet that will pay for the transfer.
...
async function main() {
    ...
    await jettonWallet.send(sender, receiverAddress, toNano(10));
}
...
const senderRegularWalletAddress = Address.parse(
    "<sender's regular wallet address>",
);
...
async function main() {
    ...
    const messageBody = beginCell()
        ...
        .storeAddress(responseDestinationAddress)
        // no custom payload
        .storeBit(0)
