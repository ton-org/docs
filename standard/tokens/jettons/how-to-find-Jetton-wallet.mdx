...
For instance some contract may want to obtain and store it's jetton wallet for some Jetton to handle transfer notifications from it in specific way.
...
async function main() {
    ...
    const jettonMasterAddress = Address
                    .parse('put the Jetton master address in any format');
    const walletAddress = Address
                    .parse("put owner's address in any format");
    ...
    const el: TupleItemSlice = {type: 'slice', cell: walletAddressCell}; // forming the required type for the stack

    const data = await client.runMethod(jettonMasterAddress,
    'get_wallet_address', [el]); // call the get method with a non-empty stack

    console.log(data.stack.readAddress()); // get the Jetton wallet address
