...
Keep a precomputed mapping of recipient → allocation in the contract. When a user sends a claim message, the contract releases the preassigned amount. This works—until the list becomes too large, starting at roughly 3,000 entries, problems begin to surface.
...
We must also prevent double-claims. The airdrop deploys a small per-user proxy contract that serves as a marker of whether the user has already claimed. This proxy blocks any subsequent attempts.
...
1. The user sends a message that deploys their per-user proxy contract along with their Merkle proof.
...
- (cNFT)[/standard-smart-contracts/mintless-jetton.mdx]
- (Mintless Jetton)[/standard-smart-contracts/mintless-jetton.mdx]