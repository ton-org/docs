...
A wallet account's address is [derived](/foundations/addresses/derive) from [`StateInit`](/foundations/messages/state-init) that stores in its `data` field two values:
