...
## Key pair 
TON uses [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) key pairs.
...
<Tabs groupId="code-examples">
    ...
    // const mnemonicArray = 'your mnemonic phrase here'.split(' '); // mnemonic as an array of words
    
    // derive private and public keys from the mnemonic
    const keyPair = await mnemonicToWalletKey(mnemonicArray); 
    
    // optionally, print the mnemonic
...
## Mnemonic validation 
The first byte of the derived `seed` must be zero. 
...
<Tabs groupId="code-examples">
    ...
    console.log(mnemonicArray); // optionally, print the mnemonic
    ```
    </Tab>
</Tabs> 