...
1. **internal_signed**: Authentication through signature (for gasless transactions)
   - `signed`: SignedRequest containing wallet_id, valid_until, msg_seqno, inner request, and signature.

1. **internal_extension**: Authentication as registered extension
   - `query_id`: 64-bit long integer.
   - `inner`: InnerRequest containing the actual actions to perform.
