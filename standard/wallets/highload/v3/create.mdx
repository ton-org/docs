...
<Aside type="tip">
  **Why non-bounceable?** When funding a `nonexist` account, use the non-bounceable format to prevent funds from bouncing back if the account doesn't exist yet. See [Address formats](/ton/addresses/address-formats) for details.
</Aside>

**Account status: [`nonexist`](/ton/statuses#status-variety)**
