...
Read more about [differences](/guidebook/from-ethereum).
...
As you can see, the main functionality of the wallet is to provide a safe way to communicate with TON Blockchain from the outside world. The `seqno` mechanism protects against replay attacks, and the `Ed25519 signature` provides authorized access to wallet functionality. The payload data consists of up to 4 references to cells and the corresponding number of modes, which will be directly transferred to the `send_raw_msg` method.
...
When working with many messages in a short period, there is a need for a special wallet called a Highload Wallet. Read [the article](/standard/wallets/higload) for more information.
