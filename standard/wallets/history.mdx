...
This wallet's function is to act like a regular wallet, but restrict transfers to only one predefined destination address. You can set the destination when you create this wallet and then you'll only be able to transfer funds from it to that address. Note that you can still transfer funds to validation contracts, so you can run a validator with this wallet.
