...
1. `int seqno()` returns current stored seqno.
2. `int get_public_key()` returns the current stored public key.
3. `int get_subwallet_id()` returns the current subwallet ID.
...
2. `int seqno()` returns the current stored seqno.
3. `int get_wallet_id()` returns the current wallet ID.
4. `int get_public_key()` returns the current stored public key.
5. `cell get_extensions()` returns the extensions dictionary.
