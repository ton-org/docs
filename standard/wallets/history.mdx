...
|--------------------------|------------------------------------------|
| [V1 R1](#wallet-v1) | `oM/CxIruFqJx8s/AtzgtgXVs7LEBfQd/qqs7tgL2how=` |
| [V1 R2](#wallet-v1) | `1JAvzJ+tdGmPqONTIgpo2g3PcuMryy657gQhfBfTBiw=` |
| [V1 R3](#wallet-v1) | `WHzHie/xyE9G7DeX5F/ICaFP9a4k8eDHpqmcydyQYf8=` |
| [V2 R1](#wallet-v2) | `XJpeaMEI4YchoHxC+ZVr+zmtd+xtYktgxXbsiO7mUyk=` |
| [V2 R2](#wallet-v2) | `/pUw0yQ4Uwg+8u8LTCkIwKv2+hwx6iQ6rKpb+MfXU/E=` |
| [V3 R1](#wallet-v3) | `thBBpYp5gLlG6PueGY48kE0keZ/6NldOpCUcQaVm9YE=` |
| [V3 R2](#wallet-v3) | `hNr6RJ+Ypph3ibojI1gHK8D3bcRSQAKl0JGLmnXS1Zk=` |
| [V4 R1](#wallet-v4) | `ZN1UgFUixb6KnbWc6gEFzPDQh4bKeb64y3nogKjXMi0=` |
| [V4 R2](#wallet-v4) | `/rX/aCDi/w2Ug+fg1iyBfYRniftK5YDIeIZtlZ2r1cA=` |
| [V5 R1](#wallet-v5) | `IINLe3KxEhR+Gy+0V7hOdNGjDwT3N9T2KmaOlVLSty8=` |
...
- [ton/crypto/smartcont/wallet.fif](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet.fif)
...
The first issue was fixed in `V1R2` and `V1R3`. The `R` stands for **revision**. Usually, revisions are just small updates that only add get methods; you can find all of those in the changes history of [new-wallet.fif](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/new-wallet.fif). Hereinafter, we will consider only the latest revisions.
...
| Contract version | Hash                                     |
|------------------|------------------------------------------|
| V1 R1       | `oM/CxIruFqJx8s/AtzgtgXVs7LEBfQd/qqs7tgL2how=` |
| V1 R2       | `1JAvzJ+tdGmPqONTIgpo2g3PcuMryy657gQhfBfTBiw=` |
| V1 R3       | `WHzHie/xyE9G7DeX5F/ICaFP9a4k8eDHpqmcydyQYf8=` |
...
As you can see, the main functionality of the wallet is to provide a safe way to communicate with TON Blockchain from the outside world. The `seqno` mechanism protects against replay attacks, and the `Ed25519 signature` provides authorized access to wallet functionality. We will not dwell in detail on each of these mechanisms, as they are described in detail in the [external message](/v3/documentation/smart-contracts/message-management/external-messages) documentation page and are quite common among smart contracts receiving external messages. The payload data consists of up to 4 references to cells and the corresponding number of modes, which will be directly transferred to the [send_raw_message(cell msg, int mode)](/v3/documentation/smart-contracts/func/docs/stdlib#send_raw_message) method.
...
| --------- | ---------------------------------------------- |
| 0x21      | `seqno` check failed, replay protection occurred |
...
| Contract version      | Hash                                     |
|-----------------------|------------------------------------------|
| V2 R1 | `XJpeaMEI4YchoHxC+ZVr+zmtd+xtYktgxXbsiO7mUyk=` |
| V2 R2 | `/pUw0yQ4Uwg+8u8LTCkIwKv2+hwx6iQ6rKpb+MfXU/E=` |
...
| Contract version       | Hash                                     |
|------------------------|------------------------------------------|
| V3 R1 | `thBBpYp5gLlG6PueGY48kE0keZ/6NldOpCUcQaVm9YE=` |
| V3 R2 | `hNr6RJ+Ypph3ibojI1gHK8D3bcRSQAKl0JGLmnXS1Zk=` |


#### Persistent memory layout

- <b>seqno</b>: 32-bit sequence number.
- <b>subwallet_id</b>: 32-bit subwallet ID.
...
| 0x22      | `subwallet_id` does not match the stored one                            |
| 0x0       | Standard successful execution exit code.                                |
...
| Contract version | Hash                                           |
|------------------|------------------------------------------------|
| V4 R1            | `ZN1UgFUixb6KnbWc6gEFzPDQh4bKeb64y3nogKjXMi0=` |
| V4 R2            | `/rX/aCDi/w2Ug+fg1iyBfYRniftK5YDIeIZtlZ2r1cA=` |
...
| 0x21      | `seqno` check failed, replay protection triggered                        |
| 0x22      | `subwallet_id` does not match the stored one                            |
| 0x27      | Plugins dictionary manipulation failed (0x1-0x3 recv_external opcodes) |
...
| Contract version       | Hash                                     |
|------------------------|------------------------------------------|
| V5 R1 | `IINLe3KxEhR+Gy+0V7hOdNGjDwT3N9T2KmaOlVLSty8=` |
...
| 0x85      | `seqno` check failed, replay protection occurred                                   |
| 0x86      | `wallet_id` does not correspond to the stored one                                 |
| 0x87      | `Ed25519 signature` check failed                                                  |
| 0x88      | `valid-until` check failed                                                        |
| 0x89      | Enforce that `send_mode` has the +2 bit (ignore errors) set for external messages. |
| 0x8A      | `external_signed` prefix doesn't correspond to the received one                   |
...
When working with many messages in a short period, there is a need for a special wallet called a Highload Wallet. Read [the article](/v3/documentation/smart-contracts/contracts-specs/highload-wallet) for more information.
