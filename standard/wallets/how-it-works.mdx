...
In TON, an account's balance is not stored inside the smart contract's persistent data. It is part of the protocol-level account state, maintained by validators and updated automatically when value is received/sent and when fees are paid.

Therefore, a wallet contract should not keep a separate `balance` variable in storage. Doing so would be redundant and could drift out of sync with the actual account state.

Remote procedure call (RPC) and blockchain explorers expose the balance as part of the public account state.
...
It also makes retries safe: a client may resend the same signed message until the deadline; after it expires, the client must re-sign with a new `valid_until`. Combined with `seqno` equality (the contract accepts only the exact current `seqno` and increments it on success), this provides robust replay protection.
