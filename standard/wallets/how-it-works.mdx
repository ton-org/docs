...
Wallets are often the entry point of transactions in TON. See [external messages](/ton/whitepapers/ton#2-4-6-external-messages%2C-or-messages-from-nowhere).
...
<Aside
  ...
  title="Important"
>
  The public key is not the wallet address. The address is derived from the contract’s `StateInit` and other parameters. See [address formats](/ton/addresses/formats).
</Aside>
...
>
  Imagine Alice has 100 TON. Alice sends 10 Toncoin to Bob as a gift. Bob, being sneaky, forwards the same message to Alice's wallet and would receive another 10 TON — unless the wallet prevents replays.
</Aside>
