...
TON supports contract upgrades through `setCode` and `setData` operations, it will be complete in [action phase](/tvm/exit-codes#action-phase). Contracts typically store a version number to track upgrades. This pattern is used in [vanity](/techniques/vanity) contracts and when protocols need to upgrade while preserving existing contracts, such as DEXs where liquidity is critical. 
