...
In this contract, we will use only two top values (message body and function selector), other three we will not take into account. After finishing execution, TVM silently drops all remaining values. In comments, we will represent stack values from the bottom to the top (e.g. `s2 s1 s0`, where s0 is the top of stack) to understand current stack layout.
