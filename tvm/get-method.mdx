...
The process of fetching the actual contract state and initializing c7 can be handled by [liteserver](/ecosystem/node/overview) for easier execution. In order to execute a get method via liteserver, a `liteServer.runSmcMethod` request should be sent, according to the [schema](https://github.com/ton-blockchain/ton/blob/f58297f1b668c7b49e8b30b65062951ca7c18acc/tl/generate/scheme/lite_api.tl#L90).
...
The response follows the `liteServer.runMethodResult` [schema](https://github.com/ton-blockchain/ton/blob/f58297f1b668c7b49e8b30b65062951ca7c18acc/tl/generate/scheme/lite_api.tl#L39). Apart from the values used for initialization and proofs, the result is included as `result:mode.2?bytes`, which is a BoC of a serialized `VmStack` object, similarly to the request.
...
The easiest path is using a high-level API, such as [Toncenter](/ecosystem/api/toncenter/v3/legacy-v2-compatible/run-smart-contract-get-method). It has a `/api/v3/runGetMethod` endpoint that takes a smart contract address, a get method name, and arguments and returns the resulting stack. Example usage:
