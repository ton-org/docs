...
- TODO: correct anchor links on this page `#number\) → <...>`.
...
To construct a `cell`, a `builder` primitive is used. If you try to store more than 1023 bits of data or more than 4 references to other cells, an error with exit code 8 is thrown: `Cell overflow`.
...
In Tolk, the `map<K, V>` type is an abstraction over the ["hash" map dictionaries of TVM](/language/func/dictionaries).
...
If the account state (contract storage, essentially) exceeds any of the limits specified in [config param 43 of TON Blockchain](/ton/config#param-43) by the end of the [action phase](#action), an error with exit code 50 is thrown: `Account state size exceeded limits`.
