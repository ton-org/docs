...
While cells are deduplicated by hash inside of every data store on TON, there might be situations when there are cells with the same hash required in several stores. For example, if there is a popular contract with a lot of instances, and there's no need to duplicate and store separately part of its code for every instance.

This is where libraries are needed: they store a single cell that is available to all the contracts. Library reference cells store hash of such a library, and tell TVM to look up the content of such library cell. This might reduce the size of serialized data and enable efficient storage of incrementally updated data.

Library reference has level `0`, so it does not contain any higher hashes.
...
Library allows to extend deduplication mechanism on-chain, enabling the incorporation of the same efficiency into custom smart contracts.
...
Library cells are mainly used to minimize storage costs for accounts with identical code. Read more about this [pattern](/techniques/using-onchain-libraries).
