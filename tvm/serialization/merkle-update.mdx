...
Here are some details: The level of a Merkle update cell, `0 <= l < 3`, is determined as `max{Lvl(c1) - 1, Lvl(c2) - 1, 0}`.

Each Merkle update cell serializes as follows:

- one tag byte with value `0x04`;
- the 256-bit-wide higher hash of the referenced cell `c1` or the [representation hash](/tvm/serialization/cells) of `c` if its level equals zero;
