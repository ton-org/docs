...
Pruned branches cells are cells that represent deleted subtrees of tree of cells, see [the corresponding page](/ton/cells/pruned-tree-cells.mdx).
...
is equal to the number of `1-bits` in mask.
- Next, `h * 32` bytes store [representations hashes](/tvm/serialization/cells.mdx) of roots of the pruned subtrees starting 
...
and the first pruned subtrees in sequatial order, but not the third one.
