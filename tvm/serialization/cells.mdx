...
this is to use the so-called _standard cell representation_, `CellRepr(c)`. The standard representation of a cell `c` is a byte sequence that constructed as follows:
...
- Then the data bits are serialized as $\lceil\frac{b}{8}\rceil$ bytes. If `b` is nota multiple of eight, a binary `1` and up to six binary `0s` are appended to
...
However, the serialization of trees formed by cells is arranged differently, see [bag of cells](./bag_of_cells.mdx) for details.
...
All cell manipulation instructions require transforming values of `Cell` type into either [builder](bulder_page) or [slice](slice_page) types before such cells can be modified or inspected.
...
Bellow there are examples of sequential cell creation, population it with data, and then parsing.

### Creating a cell and storing data
