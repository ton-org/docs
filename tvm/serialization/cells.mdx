...
<Note>
  Circular references are forbidden and cannot be created, because computing [cryptographic hash function](https://en.wikipedia.org/wiki/Cryptographic_hash_function) of data that contain that same hash is impossible.
</Note>
...
However, the serialization of trees formed by cells is arranged differently, see [bag of cells](/tvm/serialization/boc) for details.
