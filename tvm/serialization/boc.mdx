...
## An example of manual serialization
...
Now, let’s calculate the descriptor bytes `b1` and `b2` for each of the three unique cells according to the [cells serialization page](/tvm/serialization/cells#standard-cell-representation-and-its-hash). 
...
const roots_of_second_boc = deserializeBoc(serialized_boc_with_indexes_and_crc32);
```
