...
- Choose the smallest power of two `s` that is larger than `n`. Serialize each cell `ci` in a way similar to [standard representation algorithm](/tvm/serialization/cells#standard-cell-representation-and-its-hash), with exceptions:
...
- Optionally, an _index_ can be constructed that consists of `n + 1` `t`-bit integer entries `L1, ..., Ln`, where `Li` is the total byte length of the representations of cells `cj` with `j ≤ i`, and integer `t ≥ 0` is chosen so that `Ln ≤ 2^t`. If the index is included, any cell `ci` the serialized bag of cells may be easily accessed by its index `i` without deserializing all other cells, or even without loading the entire serialized bag of cells in memory.
