...
Arbitrary data are represented in TON Blockchain by trees of cells. Such a tree of cells is transformed into a [DAG](https://en.wikipedia.org/wiki/Directed_acyclic_graph) of cells by identifying unique cells in the
...
In general, a BoC can be obtained from several trees of cells, thus forming a forest. By convention, the roots of the original trees of
cells are marked elements of the resulting bag of cells, so that anybody receiving this bag of cells and knowing
the marked elements can reconstruct the original forest.
...
This page provides a general serialization algorithm and specification of the corresponding TL-B schemes,
...
differs from the serialization of included cells. Therefore, it is important to be able to identify the type of references.
...
- An optional [CRC32C](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) may be appended to the serialization for integrity
...
- The number of absent cells `l ≤ n − k`, which represent cells that are
  absent from this bag of cells, but are referred to from it. The
...
According to the TL-B scheme above there is the [SDK](https://github.com/ton-org/ton-core/blob/4577e94/src/boc/cell/serialization.ts#L1)
