...
s-bit integer `j` instead of hash `Hash(cj)` to represent internal references to cell `cj`. This algorithm is listed in [cells serialization page](/tvm/serialization/cells#standard-cell-representation-and-its-hash).
