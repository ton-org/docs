...
3. Minimal concepts in task pages. Do not front‑load long explanations in tutorials/how‑tos. Link to a concept page instead. (Why: keeping theory separate avoids cognitive overload; readers can pull background only if they need it.)
...
5. Scannability. Prefer short paragraphs (≤ \~5 sentences) and short sentences (\~15–20 words). Use headings, lists, and tables to chunk information. See §8 for norms and flexibility. (Why: short, well‑chunked text is faster to scan and reduces rereads and errors.)
...
Declaring prerequisites. Pages with advanced requirements **MUST** state them at the top (e.g., “Requires familiarity with TVM exit codes and TLB schemas.”). (Why: up‑front expectations prevent readers from starting a guide they can’t complete and reduce support churn.) You **SHOULD** include a Prerequisites block on tutorials/how‑tos and a one‑line Summary on references to orient the reader. (Why: consistent placement improves scanning and lets readers self‑select quickly.)
...
Rule. Each page **MUST** be one of the following: Tutorial, How‑to guide, Explanation, or Reference. Types **MUST NOT** be mixed in a single page. The chosen type **MUST** be clear from the title and content and **MAY** be declared in frontmatter (`doc_type`). (Why: single‑purpose pages are easier to find and follow; mixing types blurs intent and causes duplication; a `doc_type` flag helps both reviewers and tools apply the right checks.)
...
* Tutorial (first success). Single happy path for newcomers. Hand‑holding is **RECOMMENDED**. (Why: one clear path gets novices to a working result quickly; branching early increases drop‑off.)
  ...
  Sections (suggested): Objective → Prereqs → Steps → Verify → Troubleshoot → Next steps. **SHOULD** avoid theory beyond two short bullets; link to an Explanation page. (Why: predictable structure reduces cognitive load; keeping theory brief prevents blocking progress.)
...
* How‑to guide (goal‑oriented procedure). A recipe to achieve a specific outcome. **MUST** be procedural and concise. (Why: readers come for a recipe; extra narrative slows execution and invites errors.)
  Title style: “Mint a jetton with the CLI”. (Why: imperative, specific titles help readers confirm they’re in the right place.)
...
* Emojis **MUST NOT** appear in technical pages. Use admonitions and clear text instead. (Why: emojis are ambiguous, distract from procedures, and don’t localize well; standardized callouts communicate severity reliably.)
...
* UI and log messages **MUST** appear verbatim in quotation marks; **MUST NOT** add emphasis or change casing/punctuation. (Why: exact copying/grepping depends on matching strings.)
...
* Code, filenames, paths, API endpoints, flags, and literals **MUST** use code formatting (inline backticks or fenced blocks). **MUST NOT** add punctuation inside code that isn’t part of the code. (Why: visual separation prevents misreading and keeps examples copy‑pasteable without edits.)
...
* Headings **MUST NOT** contain styling other than text: no bold, italics, `code`, quotes, or ALL CAPS (acronyms permitted). Keep headings clean; use formatting in body text only. (Why: extra styling harms scanability, clutters the TOC, and causes inconsistent rendering.)
* Exceptions:
...
* Headings **SHOULD** be concise. As a norm, H1 ≤ \~60 characters, H2/H3 ≤ \~70 characters. Exceed only for clarity. (Why: concise titles fit sidebars and search results without truncation; brevity improves scanning.)
...
Objective → Prerequisites → Steps (3–7) → Verify (expected output or result) → Troubleshoot (common errors) → Next steps. (Why: this flow gets newcomers to a working result, confirms success, handles common failures, and points to what to do next.)
...
Recommended sections:
Summary → Parameters/Fields (tables) → Returns/Responses → Errors (codes, meanings) → Examples (minimal usage) → See also (anchors). (Why: a uniform, tabular layout makes lookups fast and supports deep linking and automation.)
...
* Examples **MUST** be copy‑pasteable. **MUST NOT** include shell prompts (`$`, `>`) in command blocks. (Why: prompts get copied accidentally and break commands.)
* Command and expected output **MUST** be presented as separate fenced blocks. (Why: mixing them causes copy errors and makes results hard to recognize.)
* Placeholders **MUST** use `<ANGLE_CASE>` (e.g., `<RPC_URL>`, `<LEDGER_DIR>`), and each placeholder **MUST** be defined on first use. (Why: a consistent, visible pattern prevents pasting unsafe hard‑coded values.)
...
* Secrets (keys, mnemonics, API tokens) **MUST NOT** appear in examples. Use placeholders or test keys that are clearly invalid. (Why: prevents accidental leaks and unsafe copying.)
* Commands that could cause loss of funds or data **MUST** be guarded by a Warning/Caution per §11 and **SHOULD** default to testnet. (Why: explicit risk labeling and safe defaults protect readers.)
...
* Use language‑appropriate comments to mark omissions (`// …`); **MUST NOT** use literal ellipses that change syntax. (Why: literal ellipses can create invalid code.)
...
A Caution or Warning **MUST** appear when a page or step: (Why: visible callouts make risk obvious before a reader runs a command.)

* moves funds or changes fee/withdrawal behavior; (Why: users can lose money or block withdrawals.)
* exposes, stores, or transmits private keys/mnemonics; (Why: leaked secrets allow account takeover and theft.)
* modifies validator configuration, networking, or consensus‑affecting parameters; (Why: misconfiguration can halt or fork nodes and degrade the network.)
* performs chain‑affecting operations (e.g., resharding, pruning, halting, replay). (Why: these actions can corrupt state, reduce availability, or cause data loss.)

### 11.2 How to write the callout

A safety callout **MUST** include: (Why: readers need to know the risk, scope, recovery, and safe environment before proceeding.)
...
3. Rollback/mitigation steps, and (Why: gives a path to recovery or damage control.)
...
* Destructive flags (e.g., `--purge`, `--force`) **MUST** be opt‑in and accompanied by a callout. (Why: explicit opt‑in and warnings reduce accidental data loss.)

### 11.4 Key handling and storage

* Keys **MUST** be represented as placeholders or generated ephemeral testing artifacts. (Why: prevents real secrets from being copied into scripts or screenshots.)
* In guides, prefer environment variables or keystores; never inline secrets. (Why: avoids persisting secrets in history, logs, or source control.)
* Reference pages **MAY** document secure storage options but **MUST NOT** encourage unsafe patterns. (Why: guidance is helpful, but normalizing bad practices leads to leaks.)
...
* Internal links **SHOULD** be relative and stable; anchors **MUST** resolve correctly. (Why: relative links survive domain/structure changes; broken anchors waste time.)
* Cross‑section links **SHOULD** jump to precise anchors instead of page tops. (Why: deep links land readers on the exact answer.)
* A See also section **SHOULD** close longer pages with 3–7 highly relevant links. (Why: curated exits guide readers without overwhelming them.)
...
* **MUST NOT** link to outdated or superseded pages except when documenting historical behavior. (Why: stale links propagate errors and erode trust.)
...
* Code identifiers (types, fields, flags) **MUST** appear in code font with exact case. (Why: preserves copy/paste fidelity and distinguishes code from prose.)
...
* BoC (abbrev.) / bag of cells (common noun). Abbreviation **MUST** be “BoC” (not “BOC”).
  Note: MasterChain is an official TON term and **MAY** be used as a proper noun. **MUST NOT** introduce “master/slave” metaphors in new text; use primary/replica or similar. (Why: respects protocol naming while maintaining inclusive language elsewhere.)
...
* **MUST NOT** use `{curly}` or `[square]` placeholder syntax in copy‑pasteable commands, as they are easy to misread as literal characters. (Why: braces/brackets are often treated as literals by shells and new users.)
...
* **MUST NOT** add separators to codes/IDs (e.g., block hashes, transaction IDs, addresses), even if they contain digits; keep copyable values intact. (Why: added punctuation breaks copy/paste and changes identifiers.)
...
* Currency amounts **MUST** specify the unit (e.g., TON, USDT). When precision matters, show decimals explicitly (e.g., 0.000001 TON). (Why: omitting units or precision leads to costly misinterpretation.)
* Address/hash values **MUST NOT** be silently truncated. If truncation aids readability, use a consistent truncation pattern (e.g., first 4 / last 4 with an ellipsis: `EQC…9gA`) and state clearly that the value is truncated. (Why: silent truncation causes copy/paste failures and verification mistakes.)
...
* Each page **MUST** have exactly one H1 that is the first visible content on the page and matches the page title. Frontmatter and MDX imports **MAY** appear above the H1 in source. (Why: a single, top H1 ensures correct anchors/TOC and consistent rendering while allowing MDX setup code.)
...
* Sidebar labels **MAY** be concise aliases of the H1; they **SHOULD** preserve the H1’s meaning even if wording is shortened. (Why: sidebars have less space; preserving meaning avoids confusion.)
...
* Frontmatter (recommended, not mandatory): each page **MAY** declare `doc_type: tutorial | how-to | explanation | reference` for tooling and reader orientation. Pages **MAY** also declare `audience: beginner | advanced` and `status: experimental | deprecated` (see §16.4). (Why: metadata improves filtering, search facets, and reader expectations without changing prose.)
...
* Pages and features **MAY** declare a status in frontmatter: `experimental` or `deprecated`. (Why: lifecycle signals set reader expectations.)
* If deprecated, the page **MUST** include a top‑of‑page Important callout that states the replacement (link to successor) and the removal timeline (date or version). (Why: clear migration guidance reduces breakage and support load.)
...
* A safety requirement (§11) **MUST NOT** be waived. (Why: reader safety outweighs convenience.)
