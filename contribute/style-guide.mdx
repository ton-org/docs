...
- Internal links **MUST** be root‑absolute (start with `/`) and use precise anchors. **Do not** use relative segments like `./` or `../` in any Markdown/MDX link. (We move content frequently; root‑absolute links survive reorganizations.)

## Images

- Use the `<Image>` component for all images. Do not use Markdown image syntax `![...](...)` or raw `<img ...>`.
- Store images under `resources/images/` and reference them with root‑absolute paths, e.g., `<Image src="/resources/images/<group>/<file>.png" />`. Do not use relative `src` such as `./` or `../`.
- Provide meaningful `alt` text; use `darkSrc` when needed for dark theme contrast. Keep sizes/aspect ratios consistent across a page.
