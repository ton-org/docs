...
Content of this section mostly describes contents of `docs.json` and `[...](...)` markdown links in `.mdx` files. In case one of the rules is broken, and there are duplicate reports of the same issue to `docs.json` and to articles, the issue should be reported to `docs.json`.
