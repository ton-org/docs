...
- Images **MUST** be embedded using the `<Image>` MDX component. Markdown image syntax `![...](...)` and raw `<img ...>` are **PROHIBITED**. (Why: the component ensures consistent light/dark handling, sizing, and future‑proof layout control.) \[HIGH]
- Image sources **MUST** be root‑absolute and live under `/resources/images/` (e.g., `<Image src="/resources/images/<group>/<file>.png" />`). **MUST NOT** use relative `src` such as `./` or `../`. (Why: content moves often; root‑absolute paths remain valid.) \[HIGH]
- Prefer paired themes with `darkSrc` when contrast or readability differs. Keep aspect ratios consistent across a page to avoid layout shift. (Why: visual stability improves reading flow.)
- Provide meaningful `alt` text. Use `alt=""` only when the image is purely decorative and the surrounding text already conveys the same information. (Why: accessibility and screen‑reader support.)
- Use component `width`/`height` props for predictable layout when needed; avoid arbitrary inline styling. (Why: predictable dimensions prevent layout shift and maintain consistency.)
...
- Internal links **MUST** be root‑absolute (start with `/`) and **MUST NOT** include `./` or `../`; ensure anchors are stable. (Why: root‑absolute links survive file moves and IA changes; relative segments break during reorganizations.) \[HIGH]
