...
- You **MUST NOT** use marketing‑style language (hype, superlatives, vague claims) anywhere in technical content, including titles, headings, steps, tables, references, and callouts. \[HIGH] (Why: hype erodes trust, slows scanning, and obscures the required action.)
- Replace subjective claims with measurable facts (numbers, constraints, conditions) or remove them; prefer precise, testable statements. (Why: facts enable correct implementation and reproducible results.)
- Examples (non‑exhaustive): “blazingly fast”, “revolutionary”, “world‑class”, “seamless”, “best‑in‑class”, “powerful”, “simple” (when used as hype). Use specific metrics or omit the claim. (Why: concrete data communicates value without marketing tone.)
...
- House rule — quoting style: Use the international style with American spelling. Quotation punctuation **MUST** remain outside closing quotes unless it is part of the quoted text. This differs from American book style; keep it consistent. (Why: keeps code‑adjacent strings accurate and avoids adding punctuation that isn’t part of the literal.) \[HIGH]
  ...
  - Use double quotation marks for log/error strings and literal UI text by default; place punctuation outside unless it’s part of the string. For interactive controls and key names, you **MAY** use `<kbd>` instead of quotes. (Why: quotes remain the convention for messages/text; `<kbd>` can improve readability for controls/keys.)
...
- UI/menu labels and keyboard keys **MAY** use `<kbd>` when referring to literal controls or key names; the label **MUST** be verbatim. Quotation marks are also acceptable for UI text; prefer one style consistently within a page/section. (Why: `<kbd>` is intended for controls/keys and aids scanning; quotes keep a single convention—both are readable when used consistently.)

  - Good (alternative): Click `<kbd>Settings</kbd>`, then select `<kbd>Network</kbd>`.

  - For quoted UI strings, follow the house quoting style above. Punctuation follows normal sentence rules outside `<kbd>`. (Why: consistent punctuation keeps text readable and copyable.)
...
- You **MUST NOT** use bold to style tokens, flags, filenames, or UI/log messages; use code font for tokens, quotation marks for log/error strings and UI text, and you **MAY** use `<kbd>` for UI/menu labels and key names. (Why: consistent, literal styling keeps examples copy‑pasteable, searchable, and accessible.) \[HIGH]
...
- In runnable language examples that show a `main` (or similar entry function), configuration values and other parameters that the reader is expected to change **SHOULD** be declared as top‑level constants, not buried inside `main` or helper functions. When the language forbids top‑level declarations, keep these parameters grouped near the start of the entry function. (Why: collecting adjustable values in one visible place makes examples easier to understand and update.)
...
- Explanatory comments in examples **MUST** appear on their own line immediately above the code they describe; **MUST NOT** use trailing end‑of‑line comments such as `foo(42); // sends a test message` in documentation snippets. (Why: stacked comment‑then‑code is easier to scan and keeps copy‑pasted lines clean.)
...
- Link text **MUST** be descriptive. Avoid generic labels like “click here,” “here,” “this,” or bare URLs as link text. (Why: meaningful text helps scanning, accessibility, and search.)

  - Good: `Read the [validator flags reference](/ecosystem/node/overview#validator-node).`
  - Bad: `Click [here](/ecosystem/node/overview#validator-node) for flags.`

- Link labels **SHOULD** be plain text or inline code for identifiers; **MUST NOT** use bold or italics in link text (see [§6.2](#62-quotation-marks-and-emphasis)). (Why: unstyled labels keep links readable and avoid redundant visual emphasis.)

- You **MUST NOT** add standalone sentences whose only job is to say “See …” or “Read here …” followed by a link (for example, `[Read here](/standard/wallets/mnemonics) to learn more.` or `See [mnemonics](/standard/wallets/mnemonics) for more details.`). Instead, attach the link to the relevant term or phrase in the main sentence (for example, `The [mnemonics guide](/standard/wallets/mnemonics) explains why.`). (Why: inline links keep the text flowing and avoid postponing useful links to trailing meta‑sentences.)

### 12.2 What to link (and what not)

- In a guide, the first useful mention of a flag, parameter, error code, or data type on the page **SHOULD** link to the canonical reference anchor. You **MAY** also link its first mention in other H2/H3 sections on long pages that are frequently deep‑linked or read out of order. (Why: readers see the spec at the moment they need it, even when entering mid‑page.)
...
- When linking to the same internal target multiple times on a page, you **SHOULD** prefer reference‑style Markdown links so the URL appears once in a shared definition block. For example:
  ...
  ```md
  Use the [jetton wallet][jetton-wallet] contract where applicable.
  See the [jetton wallet][jetton-wallet] reference for details.
...
- Columns whose cells are primarily numeric values (for example, counts, sizes, fees, percentages, timings) **MUST** be right‑aligned so digits line up vertically. (Why: aligned digits make numeric comparisons easier and reduce misreads.) \[HIGH] Identifiers such as addresses, hashes, transaction IDs, or error codes are not numeric columns and **MAY** remain left‑aligned.
...
- Within each sidebar group, child page labels **SHOULD** rely on the group label for context; child `sidebarTitle` values **SHOULD** avoid repeating the group name when that word adds no new information (see [§16.3](#163-navigation-labels)). (Why: group‑aware labels stay short and distinct without losing meaning.) \[HIGH]
...
- Child pages inside a sidebar group **SHOULD NOT** repeat the group name in `sidebarTitle` when that word adds no new meaning. Rely on the group label as context and choose a short, distinct label instead. (Why: avoiding redundant group nouns keeps nested labels concise and scannable.) \[HIGH]
  ...
  - Exception: if the group name is part of a proper noun or needed for clarity, you **MAY** keep it (for example, `sidebarTitle: "Telegram Wallet"` under `Standard contracts`).
