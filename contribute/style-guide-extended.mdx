...
Declaring prerequisites. Pages with advanced requirements **MUST** state them at the top (e.g., “Requires familiarity with TVM exit codes and TL-B schemas.”). (Why: up‑front expectations prevent readers from starting a guide they can’t complete and reduce support churn.) You **SHOULD** include a Prerequisites block on step-by-step/how‑tos and a one‑line Summary on references to orient the reader. (Why: consistent placement improves scanning and lets readers self‑select quickly.)
...
- This rule applies to TON documentation pages. This style guide itself **MAY** use parentheses more freely when explaining rules and rationale. (Why: meta‑documentation sometimes needs extra commentary that would be distracting in task pages.)
...
- Bold **SHOULD** mark scan points: a skimming reader should be able to bounce between bolded words and still understand the key actions or constraints in a section. (Why: using bold as a scan path keeps pages skimmable.)
...
- Draft new pages in plain text with minimal styling. Focus on content and structure first; use headings, lists, and code blocks without worrying about emphasis. (Why: drafting plain keeps attention on information, not formatting.)

- During editing, apply the bold and italics rules above: add emphasis only where it meaningfully improves scanning or clarity, and remove any emphasis that does not change what the reader learns. (Why: editing is the right time to tune emphasis.)

- Apply the same minimal‑first approach to headings, `<Aside>` callouts, tables, and visual components such as `<Card>` or `<Image>`: if the content is clear without extra framing, prefer the simpler version. (Why: reducing decorative chrome makes structure and behavior stand out.)
...
- Link text **MUST** be descriptive. Avoid generic labels like “click here,” “here,” “this,” or bare URLs as link text. (Why: meaningful text helps scanning, accessibility, and search.)
  ...
  - Good: `Validator flags are listed in the [validator flags reference](/ecosystem/node/overview#validator-node).`
...
- Flags, parameters, error codes, and data types documented in guides **MUST** be included in this link‑worthy set so first‑use links in [§12.2](#122-what-to-link-and-what-not) always have a canonical target. (Why: treating these technical items as link‑worthy ensures readers can jump straight to their reference definitions.) \[HIGH]
...
- New content **MUST NOT** introduce `<Card>` components on normal documentation pages; prefer headings, lists, and links for navigation and structure. (Why: cards add visual chrome that is rarely needed for comprehension.)

- Existing `<Card>` usages **SHOULD** be treated as style exceptions and either removed over time or confined to top‑level index pages where the card layout directly improves navigation. Any remaining usage **SHOULD** be documented as an explicit exception with scope, owner, and review date per [§18](#18-style-exceptions-and-edge-cases). (Why: limiting exceptions keeps the overall style consistent and maintainable.)

- When an index‑style page genuinely needs card‑like tiles, you **MAY** use `<Card>` as a documented exception; keep the content inside each card minimal and avoid duplicating information from the linked pages. (Why: cards should support navigation, not introduce a second layer of prose.)
