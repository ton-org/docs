...
It is a wrapper over built-in [Callout components](https://www.mintlify.com/docs/components/callouts) provided by Mintlify, but with an unified interface and a way to specify a title.
...
**Implementation:** [`aside.jsx`](https://github.com/tact-lang/mintlify-ton-docs/blob/main/snippets/aside.jsx)
...
The title of the aside to display. If `title` is not set, the default title for the current aside `type` will be used, if any.
...
- Path to an icon file inside the `resources` directory

For custom SVG icons, see [recommendations in Mintlify docs](https://www.mintlify.com/docs/components/icons#param-icon).
