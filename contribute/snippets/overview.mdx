...
Use a component by importing it into the MDX file and then rendering it as a JSX tag. These look like HTML tags but start with an uppercase letter matching the name in the import statement:
...
>
  Highlight content with customizable containers and icons. Prefer using regular ordered or unordered markdown lists whenever possible and resort to cards only when the relevant links and content should stand out.
</Card>
...
When explaining a complex topic or writing a tutorial, prefer adding concise infographic and visual diagrams.
...
1. `<View>` — instead, use `<CodeGroup>`, organize content in sub-headers, or consider making distinct pages per environment or language.
