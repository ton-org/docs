/*
  ...
  ```tolk
  // automatically highlighted if `<TolkHighlight />` exists on a page
  ```

  Visual appearance is defined in `extra.css`.

  Note: `<TolkHighlight>` must be placed on a page to enable highlighting!
  Now it's imported in all "/languages/tolk/..." pages, but not throughout the rest documentation.
 */

export const TolkHighlight = () => {
  const TOLK_GRAMMAR = {
    'comment': [
      { pattern: /\/\/.*/, greedy: true },
  ...
  // that's why even ```text snippets are highlighted with Tolk grammar
  // (there are a few in "/languages/tolk": Fift output and compilation errors)
  function isTolkBlockCode(/**Element*/ codeElement) {
    return codeElement.getAttribute('language') === 'text';
  }

  // check if Prism was called for <code>
  function isCodeHighlighted(/**Element*/ codeElement) {
    return codeElement.classList.contains('language-tolk');
  }

  // call Prism for <code>
  function highlightCode(/**Element*/ codeElement, Prism) {
