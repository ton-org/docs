...
1. Forwarding fees. For example, in [jettons](/standard/tokens/jetton), the StateInit must be forwarded with each transfer, resulting in high forwarding fees. Moving the code into a library cell significantly reduces these fees.
...
In that case you will need to pull account state and resolve all library cells (in most cases if the contract is using libs, the whole code of the contract is the library).
To resolve aa library, the only thing you need is to call [getLibraries method](/api-reference/blocks/getLibraries)
